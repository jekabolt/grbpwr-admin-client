...
// --mutate-scroll краснеет ТОЛЬКО потому, что скроллер стенда несёт `overflow-anchor: none`
// (правка ревьюера): без него Chromium прячет подрезку своим scroll anchoring, и мутация была
// слепа — см. цитату М, там замеры обеих конфигураций.
//
...
head('ЦИТАТА М — прокрутка после пересчёта высоты (страховка примитива под overflow-anchor: none)');
{
  // СИЛА ЭТОЙ ПРОВЕРКИ КУПЛЕНА ОДНИМ СВОЙСТВОМ СТЕНДА. С обычным скроллером --mutate-scroll был
  // ЗЕЛЁНЫМ: подрезка настоящая (замерено: scrollTop 1081 → (height:0, форс-лейаут) 946 →
  // (высота возвращена) 1081), но Chromium возвращает её сам — scroll anchoring, пока смена
  // высоты уложилась в одну задачу. Страховка же в примитиве написана на движки и раскладки, где
  // anchoring не сработает (`overflow-anchor: none`, Safari/Firefox) — поэтому scroll-box стенда
  // несёт `overflow-anchor: none` (правка ревьюера): это не подгонка под ответ, а ровно та
  // конфигурация, ради которой страховка существует. С ним --mutate-scroll краснеет по-настоящему
  // (замерено: 1092 → 948), то есть проверка стала доказательством, а не сторожем.
  //
