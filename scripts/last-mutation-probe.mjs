...
//
// Плюс одиннадцать точек сброса: пропуск любой оставляет протухшую формовую запись, и устаревший ⌘Z
...
function stand(initialIds = [], pieces = []) {
  ...
  const api = {
    ...
    /**
     * «RESET LAYOUT» — единственное подтверждение в системе (R8). Уносит ВСЕ ручные позиции и
     * раскладочную историю разом: отмена одного давнего перетаскивания после него вернула бы одну
     * ноду в место, которого на экране больше нигде нет.
     */
    resetLayout() {
      hist = dropMove(hist);
      pos = {};
    ...
    /**
     * РЯД СТРОК УЕХАЛ, А ТОЧКА СБРОСА НЕ СРАБОТАЛА. Отдельный глагол, потому что случай реальный
     * и узкий: щит `settling` (подтверждение переименования уходом фокуса) на один переход держит
     * `clearFormHistory` немым, и ресет, пришедший ровно в это окно, записи не хоронит. Тогда их
     * убивает щит по `fieldId` — СЛОВАМИ, потому что запись БЫЛА.
     */
    reIdRows() {
    ...
    /**
     * УСПЕШНЫЙ SAVE: форма ресетится, RHF раздаёт строкам НОВЫЕ id — И ЭТО ТОЧКА СБРОСА,
     * одиннадцатая. В компоненте на неё подписан `watch` (безымянное изменение = `reset()`), и
     * зовёт она тот же единственный `dropForm`, что и остальные десять.
     *
     * ДО ЭТОГО ПРОВОДА ПОЛОВИНА ОБЕЩАНИЯ Т5 ДЕРЖАЛАСЬ САМА СОБОЙ, А ПОЛОВИНА НЕ ДЕРЖАЛАСЬ ВОВСЕ.
     * Отмену после save останавливал щит по `fieldId`. ПОВТОРУ этот щит недоступен по построению:
     * отменённой строки в форме нет, тождества спрашивать не у чего, и судить остаётся по длине —
     * а она после сохранения совпадает ровно. «Создал → ⌘Z → save → ⇧⌘Z» молча дописывало шаг в
     * сохранённую карточку, у ОБОИХ формовых родов одинаково.
     */
    saveAndReset() {
      api.reIdRows();
      api.clearForm();
...
{
  // РЯД СТРОК УЕХАЛ МИМО ТОЧКИ СБРОСА — вставка не исключение: id строк другие, и удаление по
  // адресу вынесло бы ЧУЖОЙ шаг. Держит щит по `fieldId`, и отказывает он СЛОВАМИ: запись БЫЛА.
  const s = stand();
  s.create('SHELL');
  s.insertAt(0, 'COLLAR');
  s.reIdRows();
  s.undo();
  is('ре-id мимо точки сброса — отмена вставки отказывает словами', s.said.length, 1);
  is('и карточка цела', s.rows.length, 2);
}

{
  // А ЧЕРЕЗ SAVE ТОГО ЖЕ НЕ ПРОИСХОДИТ ВОВСЕ: сохранение — точка сброса, записи уже похоронены,
  // и отменять нечего. Молчание тут правильное, ровно как у остальных десяти: ни одна из
  // них не говорит ни слова, и save не имеет права быть одиннадцатой особенной.
  const s = stand();
  s.create('SHELL');
  s.insertAt(0, 'COLLAR');
  s.saveAndReset();
  s.undo();
  is('после сохранения отмена вставки МОЛЧИТ', s.said.length, 0);
  is('и карточка цела', s.rows.length, 2);
...
{
  // РЕ-ID СТРОК МИМО ТОЧКИ СБРОСА убивает и эту запись: адреса те же, id новые.
  const s = chain();
  s.rename(0, 'CARCASS');
  s.reIdRows();
  s.undo();
...

{
  ...
  s.saveAndReset(); // ← успешный save: форма ресетится, и это ТОЧКА СБРОСА

  // ХОРОНИТ САМО СОХРАНЕНИЕ, А НЕ ПЕРВОЕ НАЖАТИЕ ПОСЛЕ НЕГО. Раньше записи доживали до ⌘Z и там
  // отказывали словами; на отмене это выглядело прилично, а на ПОВТОРЕ обернулось дырой — щита по
  // `fieldId` у него нет. Сброс на самом ресете закрывает обе стороны разом и делает save таким
  // же, как остальные десять: ни одна из них не произносит ни слова.
  is('формовых записей не осталось', s.history.undo.filter((e) => e.kind !== 'move').length, 0);
  is('раскладочная жива', s.history.undo.length, 1);
  is('и она — та самая перестановка', peekUndo(s.history).kind, 'move');
  is('сохранение похоронило формовые записи МОЛЧА', s.said.length, 0);

  // …и раскладка РАБОТАЕТ: ресет формы её не касался.
  s.undo();
  is('перестановка отменилась после save', s.pos, {});
  is('и сделала это молча', s.said.length, 0);
  is('карточка цела — ни одного шага не удалено', s.rows.length, 2);
}

console.log('\nПОВТОР ПОСЛЕ СОХРАНЕНИЯ — обе стороны и оба формовых рода');

// ЗАЧЕМ ЭТОТ РАЗДЕЛ. Отмену после save щит по `fieldId` останавливал сам, и потому дыру не
// замечали годами: у ПОВТОРА этого щита нет по построению — отменённой строки в форме нет,
// тождества спрашивать не у чего, судить остаётся по длине. А длина после «сделал → ⌘Z → save»
// совпадает ровно, и повтор писал в ТОЛЬКО ЧТО СОХРАНЁННУЮ карточку. Симметрично у обоих родов,
// поэтому и закрыто одним проводом, а не веткой в каждом.

{
  // РОД `append`.
  const s = stand();
  s.create('SHELL');
  is('шаг создан', s.rows.length, 1);
  s.undo();
  is('⌘Z снял создание', s.rows.length, 0);
  is('и запись переехала в стопку возврата', s.history.redo.length, 1);
  s.saveAndReset();
  is('сохранение похоронило и стопку возврата', s.history.redo.length, 0);
  s.redo();
  is('⇧⌘Z не дописал шаг в сохранённую карточку', s.rows.length, 0);
  is('и промолчал: возвращать нечего', s.said.length, 0);
}

{
  // РОД `insert` — тот же замер, потому что дыра у них общая.
  const s = stand();
  s.create('SHELL');
  s.create('SLEEVES');
  s.insertAt(1, 'COLLAR');
  is('вставка легла в середину', s.rows.length, 3);
  s.undo();
  is('⌘Z снял вставку', s.rows.length, 2);
  is('и запись переехала в стопку возврата', s.history.redo.length, 1);
  s.saveAndReset();
  is('сохранение похоронило и стопку возврата', s.history.redo.length, 0);
  s.redo();
  is('⇧⌘Z не вставил шаг в сохранённую карточку', s.rows.length, 2);
  is('и промолчал', s.said.length, 0);
}

{
  // МЕРА ДЛИНЫ САМА ПО СЕБЕ ДЫРУ НЕ ЗАКРЫВАЕТ — вот доказательство, что провод несущий, а не
  // страховочный: щит повтора у ОБОИХ родов после ресета отвечает «можно», потому что длина
  // сошлась. Держит только то, что записей к этому моменту уже нет.
  const APP = { kind: 'append', index: 1, fieldId: 'old', row: {}, label: appendLabel(1) };
  const INS2 = { kind: 'insert', index: 0, fieldId: 'old', row: {}, lengthAfter: 2, label: insertLabel(0) };
  yes('append: длина после ресета сошлась — щит пропускает', canRedo(APP, rows('n1'), noUnits));
  yes('insert: та же длина, тот же ответ', canRedo(INS2, rows('n1'), noUnits));
  no('а отмену тех же записей щит по fieldId останавливает', canUndo(APP, rows('n1', 'n2'), noUnits));
}

{
  // РАСКЛАДКА ПЕРЕЖИВАЕТ СОХРАНЕНИЕ И В СТОПКЕ ВОЗВРАТА ТОЖЕ: условие 1 Т5 — одна запись не
  // трогает оба хранилища, и хоронить презентацию вместе с формой не за что.
  const s = stand();
  s.move([{ key: 'SHELL', at: { x: 7, y: 7 } }]);
  s.undo();
  is('перестановка отменена', s.pos, {});
  is('запись ждёт в стопке возврата', s.history.redo.length, 1);
  s.saveAndReset();
  is('сохранение её НЕ тронуло', s.history.redo.length, 1);
  s.redo();
  is('⇧⌘Z вернул раскладку после сохранения', s.pos, { SHELL: { x: 7, y: 7 } });
  is('молча', s.said.length, 0);
}
...
console.log('\nодиннадцать точек сброса — формовое умирает, раскладочное остаётся');
...
console.log('\nсброс раскладки против переименования — у него ДВЕ половины, и уносится одна');

// ПЕРЕИМЕНОВАНИЕ — ЕДИНСТВЕННАЯ ЗАПИСЬ, ТРОГАЮЩАЯ ОБА ХРАНИЛИЩА НАМЕРЕННО (оверрайды ключуются
// кодом узла, то есть тем самым, что жест переписывает). Поэтому «формовая или раскладочная» на
// ней ломается: `dropMove` судил её как чисто формовую и оставлял целиком — вместе с `posBack`,
// который на ближайшем ⌘Z писал в хранилище оверрайд, только что снесённый подтверждённым
// сбросом. Условие 1 Т5 сформулировано именно про это: одна запись не имеет права трогать оба
// рода. Значит сброс раскладки обязан снять у неё ПОЗИЦИОННУЮ половину, а формовую оставить.

{
  const s = chain();
  s.move([{ key: 'SHELL', at: { x: 40, y: 40 } }]);
  s.rename(0, 'CARCASS');
  is('оверрайд переехал на новый код', s.pos, { CARCASS: { x: 40, y: 40 } });
  s.resetLayout();
  is('подтверждённый сброс забыл все позиции', s.pos, {});
  is('формовая половина переименования жива', peekUndo(s.history).kind, 'rename');

  s.undo();
  is('⌘Z вернул ссылки на старый код', s.card[1].ins, ['SHELL', 'SLEEVE']);
  is('и НЕ воскресил снесённую позицию', s.pos, {});
}

{
  // ЗЕРКАЛО: повтор той же записи тоже не имеет права вернуть оверрайд.
  const s = chain();
  s.move([{ key: 'SHELL', at: { x: 40, y: 40 } }]);
  s.rename(0, 'CARCASS');
  s.resetLayout();
  s.undo();
  s.redo();
  is('⇧⌘Z вернул ссылки на новый код', s.card[1].ins, ['CARCASS', 'SLEEVE']);
  is('и раскладка осталась забытой', s.pos, {});
}

{
  // СИММЕТРИЧНАЯ ДЫРА НЕ ЗАВЕЛАСЬ: у `dropForm` переименование умирает ЦЕЛИКОМ, вместе со своей
  // раскладочной половиной, — воскрешать оверрайд после этого некому. Проверено потому, что
  // «зеркальная дыра» тут была бы ровно того же рода.
  const s = chain();
  s.move([{ key: 'SHELL', at: { x: 40, y: 40 } }]);
  s.rename(0, 'CARCASS');
  s.clearForm();
  is('формовых записей не осталось', s.history.undo.filter((e) => e.kind !== 'move').length, 0);
  is('раскладочная жива', s.history.undo.map((e) => e.kind), ['move']);
  s.undo();
  is('⌘Z отменил перестановку, а не переименование', s.card[1].ins, ['CARCASS', 'SLEEVE']);
  // Оверрайд остался под НОВЫМ кодом — там, куда его законно увёз жест, который никто не отменял.
  // Воскрешения снесённого тут нет и быть не может: записи с `posBack` в стопке уже нет.
  is('старый код в хранилище не воскрес', Object.keys(s.pos), ['CARCASS']);
}

{
  // ДОГОВОР ОБ ИДЕНТИЧНОСТИ ОБЪЕКТА НЕ СЛОМАН: снимать нечего — тот же объект, без ре-рендера.
  const noPos = record(emptyHistory(), {
    kind: 'rename', index: 0, fieldId: 'f1', from: 'A', to: 'B',
    outputs: [], inputs: [], posBack: [], posForward: [], label: renameLabel('A', 'B'),
  });
  yes('переименование без раскладочной половины отдаёт тот же объект', dropMove(noPos) === noPos);
  const withPos = record(emptyHistory(), {
    kind: 'rename', index: 0, fieldId: 'f1', from: 'A', to: 'B',
    outputs: [], inputs: [],
    posBack: [{ key: 'A', at: { x: 1, y: 1 } }], posForward: [{ key: 'B', at: { x: 1, y: 1 } }],
    label: renameLabel('A', 'B'),
  });
  const stripped = dropMove(withPos);
