...
//
// Плюс десять точек сброса (девять по спеке + граница визита фулскрина из ревью): пропуск любой
// оставляет протухшую запись, и устаревший ⌘Z сносит не то. Точки моделируются вызовами
// reset-функции — в компоненте это ровно один и тот же `clear`.
//
...
is(
  ...
  resolvePending(
    { kind: 'append', index: 0, expectedLength: 1, label: appendLabel(0), clearedBefore: true },
    rows('r1'),
  ),
  { kind: 'append', index: 0, fieldId: 'r1', label: 'create step 10', clearedBefore: true },
);
...
is(
  ...
  'clearedBefore' in
    resolvePending(
      { kind: 'append', index: 0, expectedLength: 1, label: appendLabel(0) },
      rows('r1'),
    ),
  false,
);
...
// --- десять точек сброса --------------------------------------------------------------------------
console.log('\nдесять точек сброса — пропуск любой оставляет протухшую запись');
