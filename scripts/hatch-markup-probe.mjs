...
//   * `strokeWidth` не равный `0.7u` — доля контура уезжает: выше — штрих на zoom ≤ 1 догоняет
//     контур по жирности (кламп `hatchK` там спит) и знак ткани начинает спорить с формой детали;
//     ниже 0.6u — линия на DPR 1 становится суб-полупиксельной и рисуется полупрозрачной рванью;
...
for (const state of CLOTH_RAMP) {
  ...
  for (const u of SCALES) {
    ...
    // ТОЛЩИНА ШТРИХА ВСЕГДА `0.7 * u` — постоянная ДОЛЯ контура детали (тот 1 CSS-пиксель, что
    // рисует SILHOUETTE_INK) независимо от габарита. Проверяется на КАЖДОЙ линии: у креста вторую
    // легко оставить с чужой толщиной.
    //
    // Число здесь написано ЛИТЕРАЛОМ, а не взято из модуля: проба, считающая ожидание тем же
    // выражением, что и код, перестаёт быть гейтом — она согласится с любой правкой молча.
    const widths = [...svg.matchAll(/stroke-width="([^"]+)"/g)].map((m) => m[1]);
    is(
      `${tag}: strokeWidth = 0.7u на каждой линии`,
      widths,
      Array(widths.length || 1).fill(String(0.7 * u)),
    );
    is(`${tag}: линий с толщиной столько же`, widths.length, g.kind === 'cross' ? 2 : 1);

    // И ОТДЕЛЬНО — КОРИДОР, ЧИТАЯ ЧИСЛО ОБРАТНО ИЗ РАЗМЕТКИ. Литерал выше поймает опечатку, но не
    // осознанную правку доли; коридор говорит, ЧЕМ доля ограничена с двух сторон и почему. Сверху
    // строго: штрих обязан быть ТОНЬШЕ контура, иначе на zoom ≤ 1 (где кламп спит и обе линии
    // едут с миром одинаково) знак ткани сравнивается по весу с формой детали. Снизу 0.6u: на
    // DPR 1 суб-полупиксельная линия рисуется полупрозрачной рванью вместо линии.
    for (const w of widths.map(parseFloat)) {
      checks++;
      if (!(w < u)) fail(`${tag}: штрих строго тоньше контура`, `${w} ≥ ${u}`);
      checks++;
      if (!(w >= 0.6 * u)) fail(`${tag}: штрих не ниже 0.6u`, `${w} < ${0.6 * u}`);
    }
...
is(
  ...
  '<svg><pattern id="probe" class="hx45" patternUnits="userSpaceOnUse" width="12" height="12" ' +
    ...
    'stroke-width="1.4"></line></pattern></svg>',
);
