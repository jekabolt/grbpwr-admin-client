      ...
      for (const fb of [0, 1] as Flip[]) {
        for (const ra of opts.rots) {
          for (const rb of opts.rots) {
            ...
            // Допуск 2 целых единицы. Единица — 1/SCALE = 1e-4 см, то есть МИКРОН, так что
            // допуск это 2 мкм. Столько стоит округление: движок округляет к целым единицам ДО
            // отражения, эталон — ПОСЛЕ, а Math.round(−x) ≠ −Math.round(x) ровно на половинках.
            // Любая ошибка в тождествах даёт сдвиг на сантиметры, то есть на десятки тысяч единиц.
            if (!(worst <= 2)) {
              out.mismatched++;
              if (!out.firstBad) {
