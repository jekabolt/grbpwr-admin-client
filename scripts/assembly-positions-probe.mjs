...
// Один узел, свободные детали и шаг вне узлов: на полотне есть бокс, хвост и плитки обоих
// состояний — то есть все три вида нод разом.
//
// ШАГ ВНЕ УЗЛОВ ВЗЯТ ПО ДВУМ ДЕТАЛЯМ НАМЕРЕННО, И ЭТО НЕСУЩЕЕ УСЛОВИЕ ФИКСТУРЫ. С Т9а обработка
// РОВНО ОДНОГО предмета уезжает строкой на его плитку и из хвоста уходит; будь здесь `in: ['FLAP']`,
// хвоста бы не стало вовсе, а две проверки ниже — сдвиг хвостового бокса по зарезервированному
// ключу и попадание в него курсором — разыменовывают `layout.tail` без страховки и падают
// TypeError. Двум предметам приписать один нельзя, поэтому такой шаг остаётся в хвосте по
// построению, и геометрия фикстуры не меняется.
const A = fixture(
  ['FR', 'BK', 'FLAP', 'TAB'],
  [{ in: ['FLAP', 'TAB'] }, { in: ['FR', 'BK'], out: 'SHELL', name: 'корпус' }],
);
