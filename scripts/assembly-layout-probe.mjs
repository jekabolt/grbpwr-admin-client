...
// и без блоков вовсе — на них и пришлись обе дельты T-28, а потом обе дельты Т9. Последние три
...
const CASES = [
  ...
  {
    // ДОБАВЛЕН В Т9 после мутационной проверки, которая нашла здесь дыру: снятие дубля индекса не
    // держал ни один кейс. Шаг 0 берёт A входом ДВАЖДЫ (нарушение правила 7, карточка невалидна,
    // а рисовать её всё равно надо), и обработка у A обязана остаться одна: две одинаковые строки
    // соврали бы о числе обработок и вырастили бы плитку на лишнюю строку.
    name: 'невалидный граф: деталь взята входом дважды — обработка всё равно одна',
    pieces: ['A', 'B'],
    steps: [{ in: ['A', 'A'] }, { in: ['A', 'B'], out: 'U', name: 'узел' }],
    since: 'Т9',
    invalidOk: true,
  },
];
...
const EXPECT = {
  ...
  // Дубль входа виден в `consumers` (шаг 0 дважды) и НЕ виден в высоте плитки: 60 — это одна
  // строка, а не две. Второе и есть утверждение кейса.
  "невалидный граф: деталь взята входом дважды — обработка всё равно одна": {
    "width": 308,
    "height": 162,
    "unassigned": [],
    "boxes": [
