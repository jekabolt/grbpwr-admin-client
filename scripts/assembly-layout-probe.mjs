...
//
// ЭТАЛОНЫ ПЕРЕСНЯТЫ 2026-08-20 под Т9г (стопка резервируется по НАСТОЯЩИМ высотам плиток, а не по
// их головам). Дельта посчитана прогоном старой и новой раскладки по одним и тем же фикстурам,
// поле за полем:
//
//   • девять фикстур из четырнадцати не изменились НИ НА ЧИСЛО;
//   • в четырёх переехал ровно ОДИН `y` бокса и больше ничего — бокс заново отцентрован в
//     подросшем резерве стопки: SHELL 32→56 («три, одна и ни одной»), U 26.5→32.5 («деталь взята
//     входом дважды»), U 19→31 («дубль входа у детали, которую уже обрабатывали»), U 47→53
//     («выросла последняя плитка стопки»). Плитки в этих кейсах не сдвинулись ни на пиксель, и
//     габариты полотна тоже: стопка перестала свисать ниже бокса, но её низ остался там же;
//   • ГАБАРИТЫ ПОЛОТНА изменились в ОДНОЙ фикстуре — заведённой этой же задачей, где стопок в
//     колонке две: высота 272→296 (+24 = две строки обработки × 12). ШИРИНА не изменилась НИ В
//     ОДНОЙ фикстуре.
//
// Ноль расхождений вне обработок проверен не только этими фикстурами: старую и новую раскладку
// прогнали на 600 случайных графах без обработки одной детали (727 боксов, 2938 плиток, 74
// карточки с двумя и более боксами в ОДНОЙ колонке) — совпадение полное, 0 расхождений. Что
// сравнение при этом не выродилось, показал положительный контроль: те же 600 графов плюс одна
// обработка детали в каждом дают 438 расхождений из 600.
//
// НА РЕВЬЮ Т9а ДОБАВЛЕНЫ ДВА КЕЙСА — эталоны прежних не тронуты, дельты нет. Оба закрывают дыру,
// найденную мутацией, которую проба переносила зелёной: «дубль входа у детали, которую уже
// обрабатывали» отличает снятие СОСЕДНЕГО дубля от снятия всего, что равно первому индексу
// (прежний кейс на обеих редакциях давал один ответ); «выросла последняя плитка стопки»
// удостоверяет проход, которым полотно вмещает съехавшую стопку — в кейсе «три, одна и ни одной»
// последняя плитка голая, и низ стопки там совпадает с ответом заведомо неверного прохода.
//
...
// и без блоков вовсе — на них и пришлись обе дельты T-28, а потом обе дельты Т9. Последние пять
// заведены в Т9 и на ревью Т9а и сняты уже с новой раскладки: «до» у них нет.
...
const CASES = [
  ...
  {
    // ДОБАВЛЕН НА РЕВЬЮ Т9а. Кейс выше снимает дубль у детали, которую до того не трогали, и
    // потому не отличает «снять СОСЕДНИЙ дубль» от «снять всё, что равно ПЕРВОМУ индексу» — обе
    // редакции дают на нём один ответ, и мутация второй проходила зелёной. Здесь A сначала
    // обработана честно (шаг 0), и только потом взята входом дважды (шаг 1, правило 7): у
    // соседней редакции обработок две, у «по первому индексу» — три.
    name: 'невалидный граф: дубль входа у детали, которую уже обрабатывали',
    pieces: ['A', 'B'],
    steps: [{ in: ['A'] }, { in: ['A', 'A'] }, { in: ['A', 'B'], out: 'U', name: 'узел' }],
    since: 'ревью Т9а',
    invalidOk: true,
  },
  {
    // ДОБАВЛЕН НА РЕВЬЮ Т9а. Полотно обязано вмещать стопку, ушедшую ниже своего бокса, и в
    // раскладке под это стоит отдельный проход по плиткам. Кейс «три, одна и ни одной» его НЕ
    // удостоверяет: последняя плитка стопки там голая, и низ стопки совпадает с низом того же
    // прохода, считай он хоть по голове плитки. Здесь обработана деталь, стоящая в стопке
    // ПОСЛЕДНЕЙ (B упомянута позже A и C — поглощение приводит её в блок после них), и высота
    // полотна становится единственным местом, где её строки вообще учтены.
    name: 'выросла последняя плитка стопки — полотно обязано её вместить',
    pieces: ['A', 'B', 'C'],
    steps: [
      { in: ['A', 'C'], out: 'U', name: 'узел' },
      { in: ['B'] },
      { in: ['U', 'B'], out: 'U' }, // поглощение: B входит в U после обработки
    ],
    since: 'ревью Т9а',
  },
  {
    // ДОБАВЛЕН В Т9г, И ДО ПРАВКИ Т9г ОН КРАСНЕЛ. Рост плитки учитывают двое: внутри стопки —
    // `grown`, в колонке у левого края — `cursor`. Не учитывал его ровно один — `stackH` третьего
    // прохода, а это и есть расстояние до СЛЕДУЮЩЕЙ стопки в той же вертикали. Ни один из
    // тринадцати кейсов выше такого шва не держал: границ «стопка → другая стопка» в них пять, и
    // над всеми пятью стоит голая плитка, — поэтому инвариант «плитки в одной вертикали не
    // наезжают» был зелен не потому, что раскладка верна, а потому, что его не о чем было
    // спросить.
    //
    // Геометрия — «ромб» с двумя обработками на детали A: две обработки на одной детали (зафьюзить
    // и отстрочить) — норма, а не экзотика. На старой раскладке стопка L выезжала ниже своего
    // слота на 24 (2 строки × 12), а до стопки R оставалось 18 (GAP_Y), и плитка B наезжала на C
    // ровно на 6.
    name: 'две стопки в одной колонке: рост первой отодвигает вторую, а не наезжает на неё',
    pieces: ['A', 'B', 'C', 'D'],
    steps: [
      { in: ['A'] },
      { in: ['A'] },
...
const EXPECT = {
  ...
  // Высота 254 — это НЕ бокс: бокс кончается на 184, а полотно тянет до 224 стопка, ушедшая ниже
  // него на строки обработок FR и BK. Дельта Т9г: `y` бокса 32→56 — резерв стопки вырос со 160 до
  // 208 (настоящие высоты 84+60+48 вместо трёх голов по 48), и бокс заново отцентрован в нём.
  // Плитки и габариты полотна не сдвинулись: низ стопки как был 224, так и остался, — изменилось
  // лишь то, что теперь он совпадает с низом резерва, а не свисает из него на 48.
  ...
  // строка, а не две. Второе и есть утверждение кейса. Дельта Т9г: `y` бокса 26.5→32.5 (резерв
  // стопки 104→116), больше ничего.
  ...
  // Дельта Т9г: `y` бокса 19→31 (резерв стопки 104→128), больше ничего.
  "невалидный граф: дубль входа у детали, которую уже обрабатывали": {
    "width": 308,
    "height": 174,
    "unassigned": [],
    "boxes": [
  ...
  // Дельта Т9г: `y` бокса 47→53 (резерв стопки 160→172), больше ничего. Высота полотна 218 не
  // изменилась, но пришла ОТ ДРУГОГО: до Т9г её держал отдельный проход по плиткам (стопка
  // свисала из своего резерва), теперь низ стопки и низ резерва совпали, и хватило бы бокса.
  "выросла последняя плитка стопки — полотно обязано её вместить": {
    "width": 308,
    "height": 218,
    "unassigned": [],
    "boxes": [
  ...
  // ЕДИНСТВЕННАЯ ФИКСТУРА, ГДЕ Т9г ТРОНУЛА ГАБАРИТЫ ПОЛОТНА: высота 272→296, ровно две строки
  // обработки по 12. Ширина 584 — та же. До правки числа были: высота 272, L.y 19, R.y 156 при
  // stackTop 138, TOP.y 95 при stackTop 129, — и при них плитка B (96+48=144) лезла на C (138).
  "две стопки в одной колонке: рост первой отодвигает вторую, а не наезжает на неё": {
    "width": 584,
    "height": 296,
    "unassigned": [],
    "boxes": [
...
const EXPECT_F7 = {
  ...
  // ростом соседки C. (Комментарий стоял на кейс выше — тот, у которого `tail: null`; на Т9г
  // возвращён к записи, которую описывает.)
  "хвост сжимается: уехала обработка детали, остались обработки двух деталей и узла": {
    "tail": {
      "x": 456,
      "y": 16,
  ...
  // Шов двух стопок в одной вертикали: A выросла на две строки (48→72), B уехала вниз на них же
  // (72→96), и низ B (144) обязан остаться ВЫШЕ верха C — то есть выше `stackTop` бокса R. До
  // правки Т9г верх C стоял на 138, и инвариант «плитки не наезжают» называл наезд ровно в 6px;
  // теперь C начинается на 162, и до неё 18 — тот же GAP_Y, что между любыми двумя стопками.
  "две стопки в одной колонке: рост первой отодвигает вторую, а не наезжает на неё": {
    "tail": null,
    "tiles": [
      {
...
function invariants(c, layout, loose) {
  ...
  for (const t of layout.tiles) {
    ...
    // АВТОРСКИЙ ПОРЯДОК И НИ ОДНОГО ПОВТОРА. Список строк плитки — это последовательность, в
    // которой технолог их завёл, а не множество: две строки с одним номером соврали бы о числе
    // обработок, а перевёрнутый порядок — о том, что за чем делают. Эталон ловит и то и другое
    // только там, где обработок на одной детали больше одной; здесь — на любой плитке.
    for (let n = 1; n < t.processing.length; n++) {
      if (t.processing[n] <= t.processing[n - 1]) {
        msgs.push(`плитка ${t.key}: обработки не по возрастанию — ${JSON.stringify(t.processing)}`);
        break;
      }
    }
