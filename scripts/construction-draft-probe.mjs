#!/usr/bin/env node
// ══════════════════════════════════════════════════════════════════════════════════════════════
// ПРОБА ОТМЕНЕНА. СУБЪЕКТА НЕТ. Дата отмены: 2026-09-05, круг 20.
//
console.error(
  'construction-draft-probe.mjs ОТМЕНЕНА (2026-09-05): B-13 снял предложение указаний, B-14 заменил ' +
    'построчный приём само-заполнением — сторожить построчный приём больше нечего. Гейт черновика: tmp/dsgprobe/b1325/run.sh',
);
process.exit(2);

// ══════════════════════════════════════════════════════════════════════════════════════════════
// ПОЧЕМУ ОТМЕНА, А НЕ ПЕРЕПОЙНТИРОВКА — ЗАМЕРЕНО, А НЕ ВЫВЕДЕНО.
//
// 1. ОТМЕНИЛО ЕЁ ДВА ТРЕБОВАНИЯ, А НЕ ОДНО, и это важно для следующего читателя:
//      B-13 «DRAFT OF THE CONSTRUCTION не должен добавлять коллауты» — умерла ВСЯ цитата D
//           (18 проверок: дедуп указания по имени, `same` без чипа, рождение строки, `number:0`,
//           `clientRef`, пара `part`/`parts[0]`);
//      B-14 «оно должно САМО ВСЁ ЗАПОЛНЯТЬ и подсвечивать что заполнило» — умерли цитаты A и E и
//           инструмент цитаты B. Проба меряет мир, где КАЖДОЕ поле приезжает СТРОКОЙ с чипом
//           «принять»; в сегодняшнем мире пустое поле пишется само, строки не рождает вовсе, а
//           чип остаётся только у СПОРА. Счётчик тоже другой: было «N proposed · N taken ·
//           N dismissed», стало «8 written · 1 to decide».
//
// 2. ЦИТАТНАЯ ПОЛОВИНА ПАРЫ МЕРТВА, И ЭТО ИЗМЕРЕНО, А НЕ ВЫВЕДЕНО ИЗ ЧТЕНИЯ ИСХОДНИКА. Прогон
//    2026-09-05 на ветке feat/design-band-ui: предпосылка (5 проверок) ЗЕЛЁНАЯ — оба органа
//    смонтированы, фикстура легла, — и на седьмой проверке прогон обрывается: строки
//    `[data-c19-draft-row="general:silhouette"]` нет, потому что силуэт УЖЕ НАПИСАН. До мутаций
//    дело не доходит ни разу. Три их якоря при этом ЖИВЫ (`upsertDetailText(getValues, setValue,`
//    в органе, `const next = empty` в `form-writers.ts`, `foldToken(proposed) === foldToken(...)`
//    в модели) — и это ровно та ловушка, ради которой здесь `exit 2`: мутационная половина без
//    работающей цитатной не может покраснеть НИ ОДНОЙ иглой, то есть её «зелено» не значит ничего.
//
//    ⚠ ЕЩЁ ОДНО ИЗМЕРЕНИЕ, ЧТОБЫ НЕ ПОВТОРЯЛИ: до переезда блока колорвеев (D5) стенд не мон-
//    тировался ВООБЩЕ — `ColourwayProposals` стоял внутри черновика и звал `useSearchParams`, а
//    роутера в `construction-draft-probe-entry.tsx` нет. Падение выглядело как «орган сломан»;
//    на деле сломан был стенд. Переезд это вылечил — и обнажил настоящую причину, описанную выше.
//
// 3. КУДА УЕХАЛИ ВЫЖИВШИЕ УТВЕРЖДЕНИЯ — ЧТОБЫ СЛЕДУЮЩИЙ ГЕЙТ ИСКАЛ ИХ ПО АДРЕСУ, А НЕ ПО ПАМЯТИ.
//    Гейт черновика теперь ОДИН: `tmp/dsgprobe/b1325/` (23 исхода, 8 мутаций, `run.sh`).
//      · спор со словами человека требует СВОЕГО клика и печатает `was: …`   (цитата C) → 6, 12
//      · принятое доходит до строки `details[]` и НЕ заменяет массив          (цитата A) → 3, 4
//      · стирание невозможно: занятое поле переживает прогон побайтово        (цитата B) → 5, 13, 21
//      · молчание модели не заводит ключа (`estUsage`)                        (цитата B) → 9b
//      · список только дополняется: рождён слот, существующий не тронут       (цитата D) → 9
//      · ответ несёт указания, а `callouts[]` побайтово тот же               (B-13 наоборот) → 7
//      · дверь называет повод, а не молча гаснет                              (отказы)   → 16, 18
//
// 4. ОДНА ИГЛА НЕ ПЕРЕЕХАЛА, И ЭТО НАЗВАНО ЧЕСТНО: «значение доехало до TEXTAREA, которую рисует
//    ДРУГОЙ орган» (`[data-c19-detail=silhouette]` у `ConstructionGeneralInfo`). b1325 читает
//    ФОРМУ, а не второй орган, и стенда с общими сведениями у него нет. Утверждение живо и стоит
//    гейта; чтобы его вернуть, в стенд b1325 нужно домонтировать `ConstructionGeneralInfo` и
//    сверить `value` его textarea с написанным — одна проба, а не переписанные 51.
//
// ══════════════════════════════════════════════════════════════════════════════════════════════
// НИЖЕ — АРХИВ ПРОБЫ КАК ОНА БЫЛА. Он НЕ ИСПОЛНЯЕТСЯ (выход выше) и держится ради каталога игл
// и фикстуры: `construction-draft-probe-entry.tsx` рядом — тот же стенд, и он ещё монтируется.
// ══════════════════════════════════════════════════════════════════════════════════════════════
//
