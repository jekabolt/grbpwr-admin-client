...

{
  ...
  for (const key of Object.keys(BASELINE.after)) {
    ...
    else shown.push(`${key} — ${explain(was, now).slice(0, 4).join('; ')}`);
  }
  ck(
    sameAfter === DOM_BATTERY.length,
    `снимок ПОСЛЕ побайтно совпал во всех ${DOM_BATTERY.length} случаях с деревом ${BASE_SHA}`,
    `совпало ${sameAfter}`,
  );
...
//
// ⚠️ ШЕСТЬ СЛУЧАЕВ ТОЖДЕСТВУ НЕ ПОДЧИНЯЮТСЯ, И ЭТО ИЗМЕРЕННЫЙ ФАКТ О РЕДАКТОРЕ, А НЕ ОБ ЭТОЙ
// ПРАВКЕ. На шаге с ЗАПОЛНЕННЫМ классом шва запись писателя доезжает до формы, а дальше её правит
// сам редактор: у трёх случаев машинка оказывается ПУСТОЙ строкой (шаг MACHINE без машинки сервер
// отвергает), у трёх — не доезжает ссылка на профиль парка. Список этих случаев НЕ ВЫПИСАН РУКАМИ:
// он снят тем же кодом с дерева `a9ca7819` и лежит в слепке (`unexplained`). Правка R7-Б их не
// создавала — Ц-РАВЕНСТВО показывает, что форма и до, и после даёт ОДИН И ТОТ ЖЕ ответ, — и не
// чинила: чинить чужой дефект внутри экстракции значило бы спрятать его под словом «рефакторинг».
// Появление СЕДЬМОГО (или исчезновение одного из шести) красит эту цитату немедленно.
head('Ц-ТОЖДЕСТВО · снимок ДО, переписанный намерением, равен снимку ПОСЛЕ');
{
  ...
  ck(
    held === DOM_BATTERY.length - Object.keys(was).length,
    `тождество держится в ${DOM_BATTERY.length - Object.keys(was).length} случаях из ${DOM_BATTERY.length}`,
    `держится ${held}`,
  );
  ck(
    canon(Object.keys(now).sort()) === canon(Object.keys(was).sort()),
    'а вмешивается редактор ровно в тех случаях, в каких вмешивался до правки',
    `${Object.keys(now).length} против ${Object.keys(was).length}`,
  );
  const sameGaps = Object.keys(was).filter((k) => canon(was[k]) === canon(now[k]));
  ck(
    sameGaps.length === Object.keys(was).length,
    'и вмешивается ТЕМ ЖЕ САМЫМ — поле в поле',
    `совпало ${sameGaps.length} из ${Object.keys(was).length}`,
  );
  for (const key of Object.keys(now).sort()) {
    const mark = canon(was[key]) === canon(now[key]) ? 'известно' : 'НОВОЕ';
