...
export function build(): unknown {
  return buildMarkerLayout({
    ...
    // сравнивать ровно то, ради чего заводилось, — киральность. Количество 1, а не 6: по формуле
    // блоба perSetQty 6 × 6 изделий дали бы 36 экземпляров против 6 размещений — фикстура была
    // бы семантически невозможной (байтов однородного блоба количество всё равно не касается).
    composition: [{ sizeId: 3, quantity: 1 }],
  });
}
