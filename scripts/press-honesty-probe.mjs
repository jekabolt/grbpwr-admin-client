...
// ── 3. СТРОКА-СОСТОЯНИЕ ВИДНА, НО НЕ ВЫБИРАЕТСЯ, И ИЗ НЕЁ ЕСТЬ ВЫХОД ───────────────────────────
head('3. строка «не записан»: видна, не выбирается, и приём из неё записывается');
await mount({ operationType: T.PRESS, zone: T.ZONE, pressEquipment: T.IRON });
const rowsBare = await listRows();
const stateRow = rowsBare.find((r) => /not recorded/i.test(r.label));
ck(!!stateRow, 'строка-состояние есть в живом списке', stateRow?.label ?? 'строки нет');
ck(stateRow?.disabled === true, 'и она НЕ выбирается — отсутствие приёма не записывают');
ck(
