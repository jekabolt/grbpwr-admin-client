...
//     узел, а не тот же в другом регистре. Подрезка в модуле ровно одна и стоит РАНЬШЕ всех
//     сравнений — `unitRenameAct` нормализует набранное, потому что подрезанным ключ и уедет на
//     провод (`schema.ts`). Протечёт она ниже, в планировщик, — и «SHELL⎵» начнёт находить места
//     живого «SHELL»; не будет её вовсе — и «BODY⎵» пронесёт мимо занятости второго производителя
//     BODY, которого сервер не примет. Проба держит обе границы.
//
// ДВЕ НОГИ:
