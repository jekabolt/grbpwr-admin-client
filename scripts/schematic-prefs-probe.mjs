...
if (!chromium) {
  ...
  // ПОТОЛОК ПОЗИЦИЙ ПРОТИВ ТОЛЬКО ЧТО ПОСТАВЛЕННОГО КЛЮЧА. Чистка судит по СВИПУ, а свип отстаёт
  // на рендер: ключ, который ставит эта же пачка (переезд оверрайда при переименовании узла), в
  // живом наборе ещё не значится. Без пощады чистка стирала его тем же вызовом — и жест,
  // написанный ради сохранения ручной раскладки, ронял ноду в авто-раскладку. Замерено стендом на
  // 201 оверрайде. Живой набор у этого хука — ['SHELL', 'HOOD', 'FR', 'BK'], поэтому CARCASS для
  // чистки мёртв, и его судьба и есть вопрос.
  await run((k) => {
    const pos = { SHELL: { x: 1, y: 1 } };
    for (let i = 0; i < 200; i++) pos['DEAD-' + i] = { x: i, y: i };
    localStorage.setItem(k, JSON.stringify({ v: 1, pos }));
    window.__unmount();
    window.__mount(79);
  }, KEY(79));
  is('живой хук: потолок пробит до жеста', Object.keys(await run(() => window.__api.pos)).length, 201);
  await run(() =>
    window.__api.restore([
      { key: 'SHELL', at: null },
      { key: 'CARCASS', at: { x: 7, y: 8 } },
    ]),
  );
  await settle();
  rec = await stored(79);
  is('живой хук: чистка по потолку унесла мёртвые ключи', 'DEAD-0' in rec.pos, false);
  is('живой хук: и НЕ съела ключ, поставленный этой же пачкой', rec.pos.CARCASS, { x: 7, y: 8 });
  is('живой хук: снятый пачкой ключ пощадой не считается', 'SHELL' in rec.pos, false);
