...
export enum ROUTES {
  ...
  // Индекс проектов — отдельный экран, а не ряд чипов. Тридцать проектов это место, где ряд
  // перестаёт работать и начинается список: он сортируется, ищется и заводит новый проект в
  // одно нажатие. Тот же приём со статическим сегментом, что у «topics» и «shared»: он
  // выигрывает у `/files/:id` по рангу react-router, поэтому «projects» никогда не разберётся
  // как идентификатор файла.
  filesProjects = '/files/projects',
  file = '/files/:id',
