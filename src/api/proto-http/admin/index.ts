...
export type GetOrderReviewByUUIDRequest = {
  orderUuid: string | undefined;
};

export type GetOrderReviewByUUIDResponse = {
  review: common_OrderReviewFull | undefined;
};
...
export interface AdminService {
  ...
  // Get review for a specific order by UUID
  GetOrderReviewByUUID(request: GetOrderReviewByUUIDRequest): Promise<GetOrderReviewByUUIDResponse>;
}
...
export function createAdminServiceClient(
  ...
  return {
    ...
    GetOrderReviewByUUID(request) { // eslint-disable-line @typescript-eslint/no-unused-vars
      if (!request.orderUuid) {
        throw new Error("missing required field request.order_uuid");
      }
      const path = `api/admin/order/${request.orderUuid}/review`; // eslint-disable-line quotes
      const body = null;
      const queryParams: string[] = [];
      let uri = path;
      if (queryParams.length > 0) {
      ...
      return handler({
        ...
        service: "AdminService",
        method: "GetOrderReviewByUUID",
      }) as Promise<GetOrderReviewByUUIDResponse>;
    },
  };
}
