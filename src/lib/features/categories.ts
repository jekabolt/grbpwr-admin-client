import { common_Category, common_GenderEnum } from 'api/proto-http/admin';

export const CATEGORY_DESCRIPTIONS: Record<string, string> = {
  outerwear:
    'Utilitarian, dark forms. Outerwear crafted from leather, fleece, softshell and hardshell materials. This page features bombers, blazers, trenches, peacoats, parkas & duffle coats. Designed to resist rain, block wind, and protect against snow.',
  ...
  bottoms:
    'Defined lines, functional shapes. Bottoms crafted from denim, leather, and technical cotton blends. Includes trousers, cargos, joggers, shorts, and skirts — in cropped, pleated, wrap, or drop-crotch styles. Designed for utility, comfort and motion.',
  dresses:
    'Fluid forms, minimal structure. Dresses crafted from mesh, cotton, and flowing blends. Created to combine and evolve.',
  loungewear_men:
    'Rest & rhythm for men. Loungewear and sleepwear in cotton, mesh, lace, and waffle textures. Includes boxers, briefs, bralettes, and robes — classic, relaxed, belted, or wrapped. Adaptive, for comfort and motion.',
  loungewear_women:
    'Rest & rhythm for women. Loungewear and sleepwear in cotton, mesh, lace, and waffle textures. Includes boxers, briefs, bralettes, and robes — classic, relaxed, belted, or wrapped. Adaptive, for comfort and motion.',
  accessories:
    'Not ornaments, function in form. Accessories include jewelry. Gloves, hats, socks, belts, and scarves crafted in leather, silk, cashmere, and cotton. Made to layer and finish.',
  shoes:
    'Form follows function. Footwear includes boots, heels, flats, sneakers, sandals, slippers, and loafers — from ankle to tall, high-top to low, flat to heeled. Designed for stability.',
...
export function filterNavigationLinks(links: { title: string; href: string; id: string }[]) {
  ...
  const rightSideCategoryLinks = links
    ...
    .sort(
      (a, b) =>
        RIGHT_SIDE_CATEGORIES_ORDER[
          a.title.toLowerCase() as keyof typeof RIGHT_SIDE_CATEGORIES_ORDER
        ] -
        RIGHT_SIDE_CATEGORIES_ORDER[
          b.title.toLowerCase() as keyof typeof RIGHT_SIDE_CATEGORIES_ORDER
        ],
    );
...
export function resolveCategories(
  ...
  let topCategory = findCategoryByName(safeCategories, categoryName, undefined);
  ...
  let subCategory = findCategoryByName(safeCategories, subCategoryName, topCategory?.id);
  ...
  if (!subCategory && subCategoryName && topCategory) {
    subCategory = safeCategories.find((cat) => {
      ...
      return cat.name.toLowerCase() === subCategoryName.toLowerCase();
    });
  }
