...
// Периметр ЗАМКНУТОГО контура: сумма длин рёбер, включая замыкающее последнее→первое.
//
// Живёт рядом с площадью и по той же причине: это вторая мера ОДНОГО контура, и мерить их разными
// представлениями полигона нельзя. Ею оценивается краевое дублирование — клеевая полоса вдоль среза
// стоит «периметр × ширину полосы», тогда как площадь отвечает только за дублирование целиком.
//
// Контур обязан быть тем же, что пошёл в площадь: если площадь снята с раздутого припуском контура,
// периметр обязан быть его же — иначе полоса считается по линии шва, а кроится по линии кроя.
