...
export function applySeamAllowance(pieces: readonly PieceDTO[], allowanceCm: number): SeamResult {
  ...
  for (const p of pieces) {
    ...
    out.push({
      ...
      poly,
      inner: [
        // Линия шва — исходный контур, тем же сдвигом. Идёт ПЕРВОЙ, чтобы при обрезании
        // чертежа по бюджету точек уцелела именно она.
        { layer: p.layer ?? '', closed: true, pts: p.poly.map((q) => ({ x: q.x - dx, y: q.y - dy })) },
        ...(p.inner ?? []).map((c) => ({
          layer: c.layer,
          closed: c.closed,
