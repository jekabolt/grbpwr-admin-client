...
export function checkLayout(args: {
  ...
  const out: Violation[] = [];
  // Допуск не может быть ТОНЬШЕ, чем разрешение размещателя: тот считает в целых единицах по
  // SCALE (1 мкм) и округляет позицию к этой сетке, так что деталь, прижатая к краю полосы,
  // законно вылезает на долю кванта. С прежним 1e-6 см (в сто раз мельче кванта) проверка
  // ловила собственное округление: на эталонном файле деталь «FP_L_M» вылезала за полосу на
  // 0.000003 см — тридцать нанометров — и красилась красным как нарушение зазора. Ложная
  // тревога в единственном месте, которое сторожит НАСТОЯЩИЕ перехлёсты, обесценивает его.
  // Замаскировать реальный перехлёст этот допуск не может: они на порядки крупнее, а
  // отрицательный сентинел пересечения проверяется отдельно и без всякого eps.
  //
  // Число вписано, а не импортировано из clipper: шапка этого файла обещает зависимость только
  // от типов, чтобы редактор раскладки не тащил за собой clipper и dxf-parser.
  const eps = 1e-4; // = 1 / SCALE (geom/clipper.ts), один микрон
