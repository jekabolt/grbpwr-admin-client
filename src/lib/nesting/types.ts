...
//
// БЕЛЫЙ СПИСОК, а не «всё кроме one_way», и это не стилистика.
//
// Читать «всё кроме one_way» значило бы разрешить переворот на unknown, а сервер его там НЕ
// ПРИНИМАЕТ: ValidateMarkerFabricDirection отказывает с direction_unknown любой раскладке,
// которая несёт 180° или зеркало, пока направление не проставлено ни у одной строки скоупа —
// и, по признанию самого этого файла, не проставлено оно почти нигде. Разница между двумя
// прочтениями поэтому не «чуть хуже маркер», а ДВАДЦАТЬ СЕКУНД ПОИСКА В МУСОРНОЕ ВЕДРО: оператор
// выбирает детали, ждёт весь бюджет, жмёт «сохранить» и получает отказ — в единственный момент,
// когда результат уже нельзя ни поправить, ни пересчитать дешевле.
//
// unknown — это ОТСУТСТВИЕ ответа, и единственное действие, доступное клиенту без ответа, это
// построить раскладку, законную при ЛЮБОМ ответе. Она чуть длиннее, чем могла бы быть на
// ненаправленной ткани, и это осознанная цена: поле заставляют заполнить бейдж «направление?» на
// строке BOM и кампания Д1, а не отказ после долгого счёта.
//
// Держать предикат отдельно от allowedRotations всё же приходится: набор поворотов зависит ещё и
// от allowCrossGrain, а зеркало — нет.
export function allowsFlip(direction: FabricDirection): boolean {
  return direction === 'any' || direction === 'two_way';
}
...
export type NestPieceConfig = {
  ...
  // ГРУППА ДЕТАЛИ — непрозрачная метка вызывающего, по которой движок строит ВТОРОЙ засев
  // (см. groupedSeedOrder в nest/index.ts). На смешанном настиле сюда едет размер, и тогда засев
  // воспроизводит «уложи один размер, потом следующий» — допустимое решение смешанной задачи,
  // доступное даром. Замер Ф2.6 показал, что без него поиск на реальных выкройках проигрывает
  // этой дармовщине до 3.8 % длины: 516.0 см против склейки в 497.6 см.
  //
  // Движок в метку НЕ ЗАГЛЯДЫВАЕТ и размеров не знает — ему нужно только РАВЕНСТВО меток.
  // Отсутствует или одна на всех — второго засева нет, поведение ровно прежнее.
  groupKey?: string;
};
...
//     а не переопределять ось.
//
//     ОСЬ ПРОВЕРИТЬ ИЗНУТРИ НЕЧЕМ, и это надо знать, читая пробы. M_x = R(180)·M_y, а 180°
//     доступен ровно тогда, когда доступно зеркало, поэтому движок, последовательно
//     отражающий по ДРУГОЙ оси, выдаст геометрически те же маркеры — у зеркальных размещений
//     будет отличаться только записанный rot (на 180°). Зелёными останутся все пробы разом:
//     знак площади, «не совмещаются поворотом», совпадение DXF с SVG, перебор всех 128
//     комбинаций NFP, сравнение с честно удвоенным набором. Ось — ДОГОВОР, а не измерение, и
//     его авторитет лежит в комментарии к полю `flipped` серверного контракта (proto): там
//     записано то же выражение, та же ось и та же причина порядка. Расходиться с ним нельзя —
//     блоб и обе отрисовки читают одно, а поймать расхождение будет нечем.
//
...
//   'mirror'   — экземпляр заказан ЗЕРКАЛЬНЫМ, а ткань переворота не допускает (allowsFlip): либо
//                она направленная (one_way), либо направление у неё НЕ ЗАДАНО, и тогда сервер всё
//                равно не примет норму с переворотом. Единственный честный ответ: положить нечем.
...
export type NestTelemetry = {
  ...
  // Во что эти оболочки были ОЦЕНЕНЫ по времени — то самое число, которое сравнивалось с долей
  // бюджета и выбрало ступень упрощения. Без него телеметрия показывает оболочки и факт, но не
  // показывает решение: оценка в оболочках и оценка во времени расходятся в разы там, где
  // объединение уходит за обрыв clipper'а, и именно на этом расхождении предпросчёт однажды
  // съел весь бюджет. Пара (predictedPrepassMs, prepassMs) — то, чем эта оценка проверяется.
...
export type WorkerResponse =
  | {
      ...
      warnings: string[];
      // Сколько листов пачки разобрать НЕ удалось. Число едет наружу отдельно от warnings, потому
      // что это СТРУКТУРНЫЙ факт, а не текст: по нему вызывающая сторона решает, можно ли считать
      // отсутствие блока доказательством его исчезновения (модалка «детали кроя из DXF» по такому
      // доказательству УДАЛЯЕТ детали кроя вместе со строками рецепта и замеренными площадями).
      // Догадка по подстроке предупреждения этим доказательством быть не может.
      failedFiles: number;
      // ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ ФАКТА (находка 1 второго адверсарного ревью): блоки, тихо
      // пропущенные ВНУТРИ прочитавшихся листов — отсутствующее определение, вложенность глубже
      // предела, исчерпанный бюджет инстансов. Разбор при этом выглядит «успешным», а деталь такого
      // блока — исчезнувшей из чертежа, то есть кандидатом на удаление по умолчанию. Число, как и
      // failedFiles, а не подстрока предупреждения.
      skippedBlocks: number;
      // ИМЕНА ВСТРЕЧЕННЫХ БЛОКОВ (находка 1 третьего адверсарного ревью) — набор присутствия,
      // отдельный от `pieces`. Блок может быть в чертеже и не дать ни одного контура: порог
      // площади, незамкнутая линия, `sanitizeLoop` (dxf/pieces.ts). Для раскладки такой блок
