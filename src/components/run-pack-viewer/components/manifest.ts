...
export type RpManifest = {
  ...
  style_number?: string;
  style_name?: string;
  // Ревизия, ПО КОТОРОЙ ПОСЧИТАНО (а не та, на которую ссылается прогон): 0 = кроим по живой карте.
  release_id?: number;
  release_number?: number;
  // ПОЧЕМУ ревизии нет — у нуля выше две несовместимые причины, и цеху они говорят разное:
  //   live_card          — прогон не привязан к релизу. Норма для ранней партии.
  //   live_card_fallback — привязан, но снапшот релиза не прочитался. ТРЕВОГА: кроим не по тому,
  //                        что утверждали, и никто этого не выбирал.
  //   release_snapshot   — посчитано по замороженной спецификации.
  // Неизвестное значение (старый бэкенд) читаем как «не сказано» и молчим, а не пугаем.
  spec_source?: string;
