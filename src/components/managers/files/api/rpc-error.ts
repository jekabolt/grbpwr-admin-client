...
const RULES: Rule[] = [
  ...
  },
  {
    ...
    //
    // НАПРАВЛЕНИЕ НЕ НАЗЫВАЕТСЯ, И ЭТО ГЛАВНОЕ В ФОРМУЛИРОВКЕ. Один и тот же
    // `afterWriteReadError` стоит в ОБЕИХ мутациях — и в привязке, и в отвязке. Сказать здесь
    // «связь сохранена» значило бы на плече отвязки утверждать ровно обратное тому, что
    // произошло: строка связи удалена, а экран, которому не перечитали список, всё ещё
    // показывает проект — и человек уходит уверенным, что отвязка не прошла.
    //
    // Опорный кусок обрезан до запятой намеренно: дальше в фразе сервера стоит апостроф, а он
    // разъезжается со сканером таблицы (см. `ruleSpans` в пробе).
