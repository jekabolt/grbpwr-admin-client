...
export const filesService = {
  ...
  /**
   * СЛОВАРЬ РОЛЕЙ ОДНОГО ПРОЕКТА — без архива, по тому же правилу, что и темы выше.
   *
   ...
   *
   * ПРОЕКТ — ПЕРВЫЙ И ОБЯЗАТЕЛЬНЫЙ АРГУМЕНТ (0323), и необязательным он быть не может. У роли
   * появился владелец: «исходники» съёмки и «исходники» лукбука — две разные строки словаря,
   * даже когда слово одно. Умолчание здесь означало бы «спроси у кого-нибудь», а такого вопроса
   * больше нет; обязательность же заставила компилятор показать все девять мест, которые словарь
   * читают, — и каждое из них назвало свой проект вслух.
   *
   * `projectTopicId = 0` — НЕ «словарь по умолчанию», а ИНДЕКС РАЗРЕШЕНИЯ: все роли сразу, каждая
   * помечена своим проектом. Предлагать из него нечего (чужая роль отвергается сервером на любой
   * записи), и живёт он ровно в одном месте — нормализации старой ссылки `?frole=N` без
   * `project=` в `useFiles.ts`.
   */
  listRoles: (projectTopicId: number, includeArchived = false) =>
