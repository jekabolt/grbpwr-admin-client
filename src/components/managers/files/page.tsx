...
/**
 * Поставить фокус на узел, который ЕЩЁ НЕ СМОНТИРОВАН.
 *
 * Мишень рождается тем же нажатием, что и уносит нажатую кнопку, и к первому кадру её в дереве
 * нет. Ждём несколько кадров и сдаёмся: не нашлась — значит вид не сменился, и таскать фокус
 * некуда. Вечный цикл здесь был бы хуже потерянного фокуса.
 */
function focusWhenReady(id: string, tries = 10): void {
...
export default function FilesPage() {
  ...
  const patch = useCallback(
    (
      ...
      // РОЛЬ БЕЗ ПРОЕКТА ВЫЧИЩАЕТСЯ ОДНОЙ СТРОКОЙ — тем же приёмом, что роль человека выше.
      //
      // РАНЬШЕ ПРАВИЛО КАСАЛОСЬ ТОЛЬКО «БЕЗ РОЛИ» (её сервер не игнорирует, а ОТКАЗЫВАЕТ);
      // теперь оно накрывает и роль по имени. С 0323 у роли есть владелец, и роль, пережившая
      // снятие проекта, тут же разрешалась бы обратно в свой проект — то есть кнопка «искать во
      // всех темах» возвращала бы человека ровно туда, откуда он ушёл, и выглядела бы сломанной.
      //
      // ЭТО НЕ МЕШАЕТ СТАРЫМ ССЫЛКАМ: `?frole=N` без проекта ПРИХОДИТ снаружи, а не собирается
      // здесь, и разрешается своим эффектом (`useRoleIndex`). Правило гасит роль только тогда,
      // когда проект убрали ДЕЙСТВИЕМ.
      if (idFromUrl(p.get('project')) <= 0) p.delete('frole');
  ...
  /** Индекс не прочитался: фильтр из адреса НЕ ПРИМЕНЁН, и сетка шире, чем обещает ссылка. */
  const roleResolveFailed = needsRoleResolve && roleIndexQuery.isError;
   ...
   */
  /**
   * МОЖНО ЛИ ЗДЕСЬ ПРЕДЛАГАТЬ «ПОЛОЖИТЬ ФАЙЛ».
   *
   * Нельзя в трёх случаях, и все три кончаются отказом сервера, а не «просто ничем»:
   * словарь ролей не приехал (`isSuccess`, а не «не в пути» — то же правило, что у ряда ролей
   * ниже: пока не знаем, утверждать нечего), роли из адреса в словаре НЕТ (старая ссылка,
   * удалённая роль — `SetFileRoles` ответит отказом), и роль архивная (назначать архивную
   * нельзя по правилу сервера). Смотреть раздел при этом можно — исчезает только приглашение.
   */
  const narrowedRoleUnusable =
    !fileRole.withoutRole &&
    fileRole.roleId > 0 &&
    (!rolesQuery.isSuccess ||
      !roles.find((r) => Number(r.id) === fileRole.roleId) ||
  ...
  useEffect(() => {
    ...
    // узнали: обрыв связи выглядел бы как «такой роли нет».
    //
    // ЧТО ПРОИСХОДИТ НА ОТКАЗЕ — НАЗВАНО ТОЧНО, потому что первая версия этого комментария
    // называла неправду («выдача уходит с одинокой ролью, как до 0323»). Не уходит:
    // `normalizeGrouping` гасит роль без проекта, значит запрос уезжает БЕЗ НЕЁ и показывает
    // ВСЮ библиотеку под адресом, который утверждает фильтр. Это деградация, а не поломка, но
    // молчать о ней нельзя — поэтому рядом с сеткой встаёт строка (см. `roleResolveFailed`).
    // Адрес при этом не трогается: человек перезагрузит страницу, индекс придёт, и фильтр
    // применится тем же самым разрешением.
  ...
  // съёмку, а обещала всю библиотеку. РОЛЬ УХОДИТ ВМЕСТЕ С ПРОЕКТОМ (0323): она живёт на связи
  // с ним, и «исходники во всех съёмках» перестало быть вопросом — у каждой съёмки свои слова.
  // Число рядом с кнопкой считается ТЕМ ЖЕ условием (`normalizeGrouping` гасит роль вместе с
  // проектом одним правилом на весь раздел), поэтому оно не может разойтись с выдачей.
  //
  ...
  const tile = (f: LibraryFile) => (
    <FileTile
       ...
       *
       * ⚠ «ОДНО ПРАВИЛО» ВЕРНО НА ТРИ ЧЕТВЕРТИ, и это надо знать заранее. Функция
       * `hasViewPath` решает, ЕСТЬ ЛИ путь; эти две ветки пере-излагают те же два слагаемых,
       * чтобы по нему пойти; а подвал карточки файла несёт свою пару органов («open» /
       * «open the note») с той же развилкой в третий раз. Любой НОВЫЙ путь просмотра —
       * например, вьюер выкроек — обязан менять функцию И обе эти ветки И подвал карточки
       * ВМЕСТЕ: разойдясь, они дадут плитку, которая обещает «view» и открывает карточку.
       *
  ...
  const visibleSections: ProjectSectionView[] = sectionViews
    ...
    .map(({ spec, q }) => ({
      ...
      onShowAll: () => {
        ...
        //
        // ОДНОГО КАДРА НЕ ХВАТАЕТ, И ЭТО ЗАМЕРЕНО. Раньше мишень (чип роли) на экране УЖЕ
        // была, и хватало одного `requestAnimationFrame` — надо было лишь пережить коммит.
        // Заголовок суженной секции рождается ТЕМ ЖЕ нажатием, и к первому кадру его в дереве
        // ещё нет: проба поймала фокус на `body`. Поэтому мишень ждут несколько кадров и
        // сдаются, а не крутятся вечно: не нашлась — значит секция не открылась, и таскать
        // фокус некуда.
        focusWhenReady(NARROWED_HEAD_ID);
      },
    }));
  ...
  return (
    <div className='flex flex-col gap-gutter'>
      ...
      {/* ЦЕНА ТАБУЛЯЦИИ НА ХОЛСТЕ — ОДИН ОРГАН, А НЕ ПЕРЕДЕЛКА ПЛИТКИ.
          ...
          СТОИТ ВПЛОТНУЮ ПЕРЕД СЕТКОЙ, А НЕ ВЫШЕ. Сначала она стояла сразу за тулбаром — и в
          режиме проекта перескакивала заодно КРОШКУ, ПРАВКУ ПРОЕКТА, ОПИСАНИЕ И СТРОКУ ЗАДАЧ,
          называя при этом одни плитки. Орган, уносящий больше, чем обещает подпись, хуже
          отсутствующего: его нажимают один раз и перестают доверять. */}
      {files.length > 0 && (
        <Button
          size='xs'
          variant='secondary'
      ...
      {/* СЕТКА ШИРЕ, ЧЕМ ОБЕЩАЕТ АДРЕС, И ОБ ЭТОМ СКАЗАНО. Старая ссылка `?frole=N` без
          проекта разрешается индексом ролей; не прочитался индекс — роль гаснет вместе с
          отсутствующим проектом, и выдача становится всей библиотекой. Промолчать значило бы
          дать прочесть несужённую выдачу как суженную — то есть поверить, что в роли лежит
          ВСЁ. Строка, а не плашка: это деградация чтения, а не отказ действия. */}
      {roleResolveFailed && (
        <Text size='micro' variant='label'>
          couldn't work out which project this role link belongs to, so nothing is filtered by it
          — everything is shown. reload the page to try again.
        </Text>
      )}
      ...
      {/* СЛОВАРЬ ПРОЕКТА ДОСТИЖИМ И У ПУСТОГО ПРОЕКТА — ряд стоит на СТРАНИЦЕ, а не внутри
          секций. Пока он жил в `ProjectSections`, он исчезал вместе с ними, а исчезают они
          ровно там, где вместо секций рисуется пустое состояние: свежесозданный проект не давал
          ни увидеть засеянные сервером роли, ни завести, ни переименовать, ни слить — пока в
          него не упадёт первый файл. Экран тем при этом обещает обратное.

          `isSuccess`, А НЕ «НЕ В ПУТИ»: пока словарь не пришёл или не пришёл вовсе, строка
          сказала бы «у проекта нет своих слов» — утверждение о факте, которого мы не знаем. */}
      {sectionMode && rolesQuery.isSuccess && activeProject && (
        <ProjectRolesRow
          projectName={activeProject.name ?? `#${projectId}`}
      ...
      {/* ТЕ ЖЕ УСЛОВИЯ, ЧТО У ПЛИТКИ. Адрес мог смениться под открытой модалкой (кнопка
          «назад»), и без этого условия она пережила бы переход в раздел архивной роли —
          с готовым выбором и гарантированным отказом на отправке. */}
      {addingFiles && projectId > 0 && writable && !narrowedRoleUnusable && (
