 ...
 * случаях из десяти открывают почитать. Правка включается по ⌘E, а выключается ⌘E же или кнопкой
 * «finish editing» — Esc из правки НЕ выводит (см. роутер клавиш ниже).
 *
 ...
 * 3. ЧТЕНИЕ ПОКАЗЫВАЕТ БУФЕР, А НЕ СОХРАНЁННЫЙ ТЕКСТ. Выход из правки не должен выглядеть как
...
export function NotePage() {
  ...
  useEffect(() => {
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      // ⌘A СТОИТ ВЫШЕ ЭТОГО ВЫХОДА СОЗНАТЕЛЬНО. Выход глушит ⌘S и ⌘E, пока открыто
      // подтверждение, — и это верно: записывать и выходить из правки в этот момент нечего.
      // Но «выделить всё» при открытом подтверждении остаётся законным жестом, и отданное
      // браузеру оно снова выделяло бы всю страницу — ровно ту жалобу, ради которой всё это.
      ...
      // ── ESC ИЗ ПРАВКИ БОЛЬШЕ НЕ ВЫВОДИТ ────────────────────────────────────────────────
      //
      // Претензия владельца дословно: «если у нас открыта модалка, допустим, добавления файла, и
      // мы нажимаем esc, то нас выкидывает из редактирования файла, а должно закрывать модалку;
      // в принципе esc не должен закрывать редактирование».
      //
      // ПОЧИНКА НЕ В ТОМ, ЧТОБЫ ПЕРЕЧИСЛИТЬ МОДАЛКИ. Слушатель висит на `window`, а модалки живут
      // в портале body — то есть их Esc всплывает СЮДА в любом случае, и гейт `confirmOverwrite ||
      // confirmRestore` перечислял ровно две из них поимённо. Каждая следующая (пикер файла,
      // медиатека, просмотрщик снимка) выбрасывала из правки, и список надо было бы дописывать
      // вечно — а пропущенная строка в нём стоит человеку набранного экрана.
      //
      // ВЗАМЕН У ESC ЗДЕСЬ ОСТАЁТСЯ РОВНО ОДНО ДЕЛО — закрыть блок помощника, и то лишь когда
      ...
      if (e.key === 'Escape' && editing) {
        ...
        editorRef.current?.consumeEscape();
      }
    };
