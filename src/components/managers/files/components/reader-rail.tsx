...
function ReaderThumb({ doc, pageNumber }: { doc: PDFDocumentProxy; pageNumber: number }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // страницах держит разбор каждой прокрученной мимо. В try — по той же причине, что и
      // отмена рендера ниже: документ могли разрушить обновлением просроченной ссылки.
      if (!cancelled) {
        try {
          page.cleanup();
        } catch {
