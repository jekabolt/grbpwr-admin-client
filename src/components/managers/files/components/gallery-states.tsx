...
export function EmptyGroupingState({
  ...
  const title = (() => {
    ...
    // «БЕЗ РОЛИ» БЕЗ ПРОЕКТА — СТРАХОВКА: его гасит `fileRoleFromUrl` при разборе адреса и
    // второй раз `normalizeGrouping` при сборке запроса. Строка стоит на случай следующего
    // вызывающего, который соберёт такую пару руками, — альтернатива ей «ни в одном проекте нет
    // роли «»» с пустыми кавычками.
    //
    // А ВОТ РОЛЬ БЕЗ ПРОЕКТА ДОСТИЖИМА, и звать её мёртвой веткой было бы ошибкой. Обычно она
    // нормализуется в свой проект, но если индекс ролей НЕ ПРОЧИТАЛСЯ (сеть, 500, права), адрес
    // остаётся с одинокой `frole`, а запрос уходит без неё. На пустой библиотеке экран дойдёт
    // сюда — и заголовок скажет про фильтр, которого не применяли. Смягчено строкой у сетки
    // («nothing is filtered by it»); совсем врать этот заголовок не должен, поэтому он говорит
    // про роль ОСТОРОЖНО.
    if (withoutRole) return 'pick a project — “without a role” is asked inside one';
    return `the library is empty under the role link “${role}”`;
  })();
