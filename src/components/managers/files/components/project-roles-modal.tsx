...
export function ProjectRolesModal({
  ...
  /**
   * ⚠ ПОЛНАЯ СТРОКА ИЗ КЭША, А НЕ ЧАСТИЧНАЯ ПРАВКА — и это ГОНКА, унаследованная от старого
   * общего блока, а не заведённая здесь.
   *
   * `UpsertFileRole` замещает набор целиком, поэтому архив/разархив шлёт имя и порядок ТЕМИ,
   * какими они лежали в кэше на момент отрисовки. Коллега, переименовавший роль в соседней
   * вкладке между нашим чтением и нашим нажатием, получит своё имя молча откаченным. То же у
   * `saveRole` с полем `archived`.
   *
   * Не чинится здесь по одной причине: лечение — либо частичное сообщение в контракте, либо
   * версия строки, и то и другое правит бэкенд. Записано, чтобы это не сочли новым дефектом
   * страницы проекта.
   */
