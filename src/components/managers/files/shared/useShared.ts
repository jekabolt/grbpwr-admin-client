 ...
 * `people` — возвращает файл в общие выдачи.
 *
 * ПУСТОЙ `adminIds` БЕЗОПАСЕН, И ЭТО СВОЙСТВО СЕРВЕРА, а не удача. `SetLibraryFileAccess`
 * трогает поимённый список ровно одним вызовом `replaceAccessPeople`, и вызывает его ТОЛЬКО
 * когда новый уровень — `people`. На `team` и `link` присланный список не читается вовсе:
 * строки `library_file_access_person` остаются лежать, и возврат team → people не заставит
 * набирать людей заново — ровно это и обещает диалог подтверждения на экране. Блок доступа в
 * карточке шлёт сюда то же самое (`adminIdsFor` в `file-access-section.tsx`): два места,
 * посылающие на один RPC разные тела в одном и том же переходе, — это клиент, спорящий сам с
 * собой, и однажды один из них станет правдой, а другой останется в чьей-то голове.
 */
export function useCloseSharedAccess() {
  ...
  return useMutation({
    ...
    // второй запрос на любой отказ — включая 403, где он ничего не меняет, и обрыв связи,
    // где первый запрос мог уже примениться. Смена уровня убивает выданную наружу ссылку;
    // второй разрушительный запрос за одно нажатие — цена, которую никто не просил платить.
    retry: 0,
