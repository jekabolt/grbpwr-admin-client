...
export function getProductPagedParans({
    ...
    return {
        ...
        filterConditions: {
            ...
            topCategoryIds: topCategories ? topCategories.split(',').map(id => parseInt(id)) : undefined, //done
            subCategoryIds: subCategories ? subCategories.split(',').map(id => parseInt(id)) : undefined, //done 
