import { useState } from 'react';
import { common_OrderFull } from 'api/proto-http/admin';
import { formatDateShort } from 'components/managers/orders-catalog/components/utility';
import { Avatar } from 'ui/components/avatar';
import { Button } from 'ui/components/button';
import Text from 'ui/components/text';
import Textarea from 'ui/components/text-area';
import { useAddOrderComment, useOrderComments } from '../hooks/useOrderComments';

/**
 * ordComment v2 — the single free-text note is now an author thread. Comments list
 * oldest→newest (each: author Avatar + short timestamp + body); a composer below posts a
 * new one. The author is stamped server-side from the JWT, so the operator never picks
 * it. Read-only accounts see the thread but not the composer.
 */
export function Comment({
  orderDetails,
  canEdit = true,
}: {
  orderDetails?: common_OrderFull;
  canEdit?: boolean;
}) {
  const orderUuid = orderDetails?.order?.uuid;
  const { data: comments = [], isLoading } = useOrderComments(orderUuid);
  const add = useAddOrderComment(orderUuid);
  const [body, setBody] = useState('');

  async function submit() {
    const text = body.trim();
    if (!text || !orderUuid) return;
    try {
      await add.mutateAsync(text);
      setBody('');
    } catch {
      /* snackbar shown by the mutation */
    }
  }

  return (
    <div className='flex flex-col gap-3'>
      <div className='flex flex-col gap-2.5'>
        {isLoading ? (
          <Text size='micro' variant='label' component='span'>
            loading…
          </Text>
        ) : comments.length === 0 ? (
          <Text size='micro' variant='label' component='span'>
            no comments yet
          </Text>
        ) : (
          comments.map((c) => (
            <div key={c.id} className='flex gap-2 border-b border-borderColor pb-2 last:border-b-0'>
              <Avatar name={c.author} title={c.author} />
              <div className='flex min-w-0 flex-1 flex-col gap-0.5'>
                <div className='flex items-baseline justify-between gap-2'>
                  <Text size='micro' component='span' className='font-bold uppercase tracking-label'>
                    {c.author || 'unknown'}
                  </Text>
                  <Text size='nano' variant='label' component='span' className='shrink-0'>
                    {formatDateShort(c.createdAt, true)}
                  </Text>
                </div>
                <Text size='micro' component='span' className='whitespace-pre-wrap break-words'>
                  {c.body}
                </Text>
              </div>
            </div>
          ))
        )}
      </div>

      {canEdit && (
        <div className='flex flex-col gap-2'>
          <Textarea
            name='newOrderComment'
            variant='secondary'
            placeholder='leave a comment…'
            className='mb-0 min-h-16 border border-borderColor placeholder:uppercase placeholder:text-textInactiveColor'
            value={body}
            onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setBody(e.target.value)}
          />
          <Button
            type='button'
            variant='secondary'
            size='sm'
            className='self-end uppercase'
            loading={add.isPending}
            disabled={!body.trim() || !orderUuid}
            onClick={submit}
          >
            comment
          </Button>
        </div>
      )}
    </div>
  );
}
