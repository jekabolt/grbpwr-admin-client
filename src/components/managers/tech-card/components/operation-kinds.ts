...
//
// ЧТО ИЗ ЭТОГО НЕ СЛЕДУЕТ: будто без каталога их нет вовсе. Снимок-фолбэк несёт их отдельной
// дельтой (`CATALOG_ONLY_WORKS` в `operation-work.ts`) — вместе со снятием `gather_ease` и честным
// именем свалки, потому что вывести их отсюда не из чего. Пункт и работа — две разные
// идентичности, и отсутствие первой не отменяет второй.
...
/**
 * ЯКОРЬ, КОТОРЫЙ ПИКЕР САМ И ПОСТАВИЛ, — единственное, что выбор пункта имеет право снять.
 *
 * Пара «имя поля → пусто». Сегодня в ней одна строка, и это не заготовка на вырост: `seam_class`
 * — ЕДИНСТВЕННОЕ поле, которое пункт (`A2`) пишет как СВОЮ ЛИЧНОСТЬ, а не как ось шага. Глагол и
 * машинка сюда не входят никогда: шаг `MACHINE` обязан нести машинку, и снять её значит родить
 * заведомо несохраняемый шаг.
 *
 * `label_attach_stitch` ЗДЕСЬ НЕТ СОЗНАТЕЛЬНО, хотя резолв перехватывает и им (B5). Его не пишет
 * ни один пункт — это ответ технолога на вопрос «каким швом пришита этикетка», то есть ДАННЫЕ, а
 * не отпечаток пикера; шаг с заполненным швом этикетки ЕСТЬ «пришить этикетку», и пикер, стирающий
 * его ради другого имени, стирал бы сказанное человеком. Граница ровно эта: пикер снимает то, что
 * пикер написал.
 */
const KIND_ANCHOR_FIELDS: ReadonlyArray<readonly [OperationFormStringField, string]> = [
  ['seamClass', 'TECH_CARD_SEAM_CLASS_UNKNOWN'],
];

/**
 * ЧТО ВЫБОР ПУНКТА ОБЯЗАН СНЯТЬ, ЧТОБЫ ВЫБОР ВООБЩЕ СОСТОЯЛСЯ.
 *
 * ЗАМЕРЕННЫЙ ДЕФЕКТ. На шаге `{MACHINE, LOCKSTITCH, seam_class = OS_TOPSTITCH}` выбор «Join —
 * lockstitch» писал машинку (ту же самую) и НЕ трогал класс шва — по правилу «смена вида ничего не
 * стирает», — резолв снова отвечал «Topstitch», и пикер откатывался к нему у человека на глазах.
 * Не брались ровно `A2 → {A1, A4, A6, A9, B5}`. Цена не в неудобстве: `kindHeadingVerb` продолжал
 * отдавать слово «topstitch», и им назывался шаг рельса, заголовок редактора, карта примерки,
 * подписанный релиз и печатный лист — обычный коверлочный шов уезжал в цех как отстрочка.
 *
 * ПОЧЕМУ ЧИСТКА, А НЕ «ПОКАЗАТЬ ВЫБРАННОЕ В ПИКЕРЕ». Вид НИГДЕ НЕ ХРАНИТСЯ — он вычисляется из
 * записи. Оставить запись как есть и нарисовать в пикере другое имя значило бы завести теневое
 * значение к паре осей: экран говорил бы «Coverstitch», а заголовок, релиз и лист — по-прежнему
 * «topstitch». Это не починка лжи на листе, это вторая ложь на экране.
 *
 * ПРАВИЛО «СМЕНА ВИДА НИЧЕГО НЕ СТИРАЕТ» НЕ НАРУШЕНО, А СУЖЕНО ДО СВОЕЙ ГРАНИЦЫ: снимается ровно
 * тот якорь, который пикер САМ пишет как личность другого пункта (`KIND_ANCHOR_FIELDS`), и только
 * если снятие ДЕЙСТВИТЕЛЬНО делает выбранный пункт ответом. Не сделало — не снимается ничего:
 * резолв разошёлся по причине, которой эта функция не знает, и выдумывать в таком месте нельзя.
 *
 * ПРАВИЛА РЕЗОЛВА ЗДЕСЬ НЕ ПОВТОРЕНЫ НИ ОДНОЙ СТРОКОЙ — функция их СПРАШИВАЕТ (`kindOf`), пробуя
 * снятие и сверяя ответ. Вторая копия таблицы «что чем перехватывается» разошлась бы с первой ровно
 * так же молча, как разошлись бы два списка дискриминаторов.
 *
 * `pendingResolve` ЗАСЧИТЫВАЕТСЯ ЗА ОТВЕТ: у `B5` («Attach label») различающий факт — шов этикетки,
 * которого при выборе ещё нет, и резолв честно отвечает родителем `A1`. Без этого «Attach label»
 * остался бы недостижимым с отстрочки — то есть чинилось бы четыре перехода из пяти.
 */
export function kindClears(
  k: OperationKind,
  step: OperationKindStep,
): Partial<Record<OperationFormStringField, string>> {
  const out: Partial<Record<OperationFormStringField, string>> = {};
  const answers = (id?: string) => !!id && (id === k.id || id === k.pendingResolve);
