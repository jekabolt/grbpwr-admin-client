...
// меняется ни на число, пока над ней ничего не выросло.
//
// Т9г довела эту цену до конца. Т9а оставила `stackH` третьего прохода считаться по ГОЛОВАМ
// плиток — «пересчёт сдвинул бы боксы, а их обещано не двигать», — и обещание оказалось куплено
// наездом: стопка выезжала ниже своего слота на 12 за строку, а до следующей стопки в той же
// вертикали оставалось GAP_Y = 18. Уже ДВЕ обработки на одной детали (зафьюзить и отстрочить —
// норма, а не экзотика) клали плитку на соседнюю на 6px, и обещание «полотно не шевельнётся»
// защищало картинку, которую нельзя показывать. Теперь стопка меряется настоящими высотами своих
// плиток. Обещание при этом осталось буквальным там, где что-то значит: без обработок `tileH`
// равна `TILE`, и обе формулы совпадают до числа. Двигаются ровно те боксы, над стопкой которых
// что-то выросло.
//
...
export type BoxLayout = {
   ...
   * зарезервированным). Расхождение намеренное: выброси пустой слот из резерва — и сдвинулись бы
   * все боксы, а Ф7 обещала не двигать ни одного. Т9г изменила, ЧЕМ меряется слот (настоящей
   * высотой плитки вместо головы), но не то, какие слоты в стопке есть.
   */
  pieceInputs: string[];
};
...
/**
 * ДЕТАЛЬ, НА ПЛИТКЕ КОТОРОЙ РИСУЕТСЯ СТРОКА ЭТОГО ШАГА; пусто — ни на одной.
 *
 * ПРАВИЛО ЖИВЁТ ЗДЕСЬ В ОДНОМ ЭКЗЕМПЛЯРЕ, и это не украшение. Его спрашивают трое: раскладка
 * (какие строки растят плитку и какие уходят из хвоста), подвал хвоста (что он считает) и щит
 * диалога создания (появится ли обещанная строка на обещанной плитке). Переписанное вторым
 * читателем своими словами, оно разошлось бы молча — а расхождение стоит ровно того, ради чего
 * все три вопроса и задаются: подпись обещает одно, экран показывает другое.
 *
 * Условие ровно два: шаг ничего не собирает (`!outputUnitKey`) и все его входы — одна и та же
 * ДЕТАЛЬ. Обработка на узле отсеивается родом первого входа, обработка на двух предметах — вторым
 * условием: у неё нет одного предмета, и приписать её одному значило бы соврать. Шаг без входов
 * не принадлежит никакой плитке.
 *
 * СРАВНЕНИЕ ПО КЛЮЧУ, А НЕ ПО ПАРЕ «ключ + род»: род входа есть чистая функция ключа
 * (`classifyAssemblyInputs` — `pieceKeys.has(key) ? 'piece' : 'unit'`), поэтому два входа с одним
 * ключом одного рода ВСЕГДА, а лишняя проверка рода разошлась бы с движком на карточке, где узел
 * назван именем детали (правило 6, `key-is-piece`).
 */
export function processedPieceOf(step: AssemblyStep): string {
  if (step.outputUnitKey) return '';
  const first = step.inputs[0];
  if (!first || first.kind !== 'piece') return '';
  for (const input of step.inputs) if (input.key !== first.key) return '';
  return first.key;
}

/**
 * СТРОКИ, КОТОРЫЕ РИСУЕТ ХВОСТОВОЙ БОКС: шаги хвостового псевдоблока МИНУС обработки, уехавшие на
 * плитки своих деталей. В хвосте они были только потому, что им больше негде было быть.
 *
 * Отдельной функцией, потому что список нужен ДВОИМ и в разных местах дерева: раскладке — чтобы
 * отмерить высоту коробки, и подвалу — чтобы посчитать своё Σ по тому же множеству, которое над
 * ним нарисовано. Пока это считалось только внутри раскладки, подвал спрашивал число у атрибуции
 * и печатал сумму ДВУХ шагов рядом с надписью «1 step».
 */
export function drawnTailSteps(looseSteps: number[], steps: AssemblyStep[]): number[] {
  return looseSteps.filter((i) => {
    const s = steps[i];
    return !s || !processedPieceOf(s);
  });
}
...
export function assemblyLayout(
  ...
  //
  // САМО ПРАВИЛО ЗДЕСЬ НЕ ЖИВЁТ, оно вынесено в `processedPieceOf` наверху файла: тот же вопрос
  // задают подвал хвоста и щит диалога создания, а правило, переписанное вторым читателем, — это
  // ровно тот дефект, который в этом раунде чинили дважды.
  ...
  consumersOf.forEach((indexes, key) => {
    const own = indexes.filter((i, n) => {
      ...
      return processedPieceOf(steps[i]) === key;
    });
  ...
  const tailSteps = drawnTailSteps(loose?.steps ?? [], steps);
  ...
  for (let c = 0; c <= maxCol; c++) {
    ...
    for (const b of cols[c]) {
      ...
      // Стопка меряется НАСТОЯЩИМИ высотами своих плиток, а не головами. Расстановка ниже сдвигает
      // каждую следующую плитку на рост предыдущих (`grown`), и без того же роста в резерве стопка
      // вылезала за него ровно на 12 за строку — при GAP_Y = 18 до следующей стопки в той же
      // вертикали это наезд уже на ДВУХ обработках одной детали.
      //
      // Резерв считается по ВСЕМУ `pieces`, включая слот, оставшийся пустым (деталь свободна и
      // ушла в колонку у левого края, но место под неё держится — см. `pieceInputs`): такой слот
      // и до Т9г держал место под голую плитку, теперь держит его вместе с ростом. Это резерв С
      // ЗАПАСОМ: низ стопки равен `stackH`, когда заняты все слоты, и выше него, когда заняты не
      // все, — за резерв он не выходит НИКОГДА, и это единственное направление ошибки, которое
      // ничего не ломает.
      const stackH = pieces.length
        ? pieces.reduce((t, k) => t + tileH(k), 0) + (pieces.length - 1) * TILE_GAP
        : 0;
  ...
  for (const box of boxes) {
    // Рост плиток с обработками копится ВНУТРИ стопки: шаг по индексу отмеряет ГОЛУЮ плитку, и
    // без сдвига следующая деталь легла бы на строки предыдущей. Слот, оставшийся пустым
    // (свободная деталь ушла в колонку), роста не даёт — резерв под него в `stackH` посчитан с
    // ростом, и стопка просто кончается выше своего резерва.
    let grown = 0;
  ...
  // Полотно обязано вместить каждую плитку: уехавшее за край прокрутки не достать — `overflow:
  // auto` вниз за границу контента не листает. До Т9г этот проход работал ПОСТОЯННО: стопка
  // резервировалась по головам и регулярно свисала ниже своего бокса. Теперь `stackH` меряет
  // настоящие высоты, низ полной стопки ровно совпадает с низом её резерва, и проход стал
  // СТРАХОВКОЙ — он сработает, если расстановка плиток и резерв под них однажды разойдутся.
