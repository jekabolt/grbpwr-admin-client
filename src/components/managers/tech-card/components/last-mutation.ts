...
/**
 * СМЕРТЬ ФОРМОВЫХ ЗАПИСЕЙ, ЖИЗНЬ РАСКЛАДОЧНЫХ. Это и есть все одиннадцать точек сброса разом: массив
 * поехал (перестановка, удаление, генератор), в доке начали печатать, карточку выпустили, закрыли
 * фулскрин, сохранили карточку (ресет формы раздаёт строкам новые id). Раскладка ни одним из этих событий не затронута — её записи обязаны пережить все.
 *
 ...
 *
 * И ПОЭТОМУ ЖЕ ОН СНИМАЕТ РАСКЛАДОЧНУЮ ПОЛОВИНУ ПЕРЕИМЕНОВАНИЯ, А НЕ ТОЛЬКО РОД `move`. Деление
 * «формовая запись или раскладочная» на переименовании не работает: оно единственное трогает оба
 * хранилища НАМЕРЕННО — оверрайды ключуются кодом узла, то есть тем самым, что жест и переписывает,
 * и не перенеси он оверрайд, нода прыгнула бы в авто-раскладку. Отфильтровав такую запись как
 * «формовую», сброс оставлял её целиком вместе с `posBack`, и ближайший ⌘Z писал в хранилище
 * оверрайд, только что снесённый подтверждённым жестом: подвинул ▣ SHELL рукой, переименовал в
 * ▣ CARCASS, нажал «reset layout», отменил переименование — и нода снова стоит там, где её никто
 * уже не ставил. Это ровно то, что запрещает условие 1 Т5: одна запись не имеет права трогать оба
 * рода. Половина снимается, формовая часть остаётся живой и отменяемой — жест был один, и отмена
 * ссылок после сброса раскладки законна.
 *
 * ЗЕРКАЛЬНОЙ ДЫРЫ У `dropForm` НЕТ: там переименование умирает ЦЕЛИКОМ, вместе со своей
 * раскладочной половиной, и воскрешать снесённое просто нечему.
 */
export function dropMove<TRow>(h: History<TRow>): History<TRow> {
  let stripped = false;
  /** Раскладочная половина смешанной записи. Пустые пачки `restore` игнорирует (ранний возврат). */
  const dropPos = (e: HistoryEntry<TRow>): HistoryEntry<TRow> => {
    if (e.kind !== 'rename' || (e.posBack.length === 0 && e.posForward.length === 0)) return e;
    stripped = true;
    return { ...e, posBack: [], posForward: [] };
  };
  const undo = h.undo.filter(isFormEntry).map(dropPos);
  const redo = h.redo.filter(isFormEntry).map(dropPos);
  if (!stripped && undo.length === h.undo.length && redo.length === h.redo.length) return h;
  return { undo, redo };
}
