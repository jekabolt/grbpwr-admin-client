...
function BomTile({
  ...
  return (
    ...
    >
      ...
      {/* ⚠ `items-stretch` ЗДЕСЬ ОБЯЗАТЕЛЕН, И ЭТО НЕ ДУБЛЬ УМОЛЧАНИЯ. У `<button>` браузерный стиль
          ставит `align-items: flex-start`, у `<div>` действует стретч — поэтому подпись `truncate`
          внутри кнопки мерится ПО СОДЕРЖИМОМУ и обрезать ей нечего. Замерено на этой плитке:
          подпись 372px внутри плитки в 169px уже на 60 символах имени, 492px на 80 — то есть на
          обычных данных она ложится поверх соседки, — а когда такая плитка попадает в последнюю
          колонку сетки, СТРАНИЦА едет вбок: +724px при 1280 и +558px при 1440 на длинном имени
          материала. Это ровно претензия D-4 владельца («почти два экрана в ширину»).
          Та же поломка была в примитиве `ui/components/tiles.tsx` и починена там же; здесь плитка
          написана РУКАМИ, поэтому починку примитива она не унаследовала — цена своей копии органа.
          Причина названа мутацией, а не рассуждением: выставление `alignItems:'stretch'` этим
          кнопкам прямо в браузере убирает 724px в ноль. */}
      <button
        type='button'
        onClick={onOpen}
        className='flex w-full flex-col items-stretch gap-1 p-1.5 text-left'
