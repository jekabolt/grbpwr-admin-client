...
export function mapFormToTechCardInsert(
  ...
  return {
    ...
    // `moodNote` ПИШЕТСЯ, а `callouts[].clientRef` — нет, и разница не в аккуратности.
    //
    // Записку мудборда человек печатает и ждёт, что она сохранится; поле уже есть и на проводе
    // (`common_TechCardInsert.mood_note`), и в схеме беты (`tech_card.mood_note`). Не слать её
    // значит показать поле, которое молча теряет набранное, — а это хуже, чем не показать поля.
    // От старого бинаря её защищает не молчание здесь, а ГЕЙТ ВОЗМОЖНОСТЕЙ на выходе
    // (`design/payload-gate.ts`, зовётся в `doSubmit`): он снимает поле, когда сервер про полосу
    // не знает, и тогда «отсутствие» означает «сохрани хранимое» — верное поведение.
    //
    // `clientRef` по-прежнему не пишется, и это ОТДЕЛЬНОЕ решение с отдельной причиной:
    // `MintCalloutNumbers` на сервере минтит номер листа ЛЮБОЙ выноске с непустым `client_ref`,
    // без предиката по медиа, — то есть мудбордная заметка съела бы номер листа. Это порча данных,
    // которую не чинит откат клиента. Поле поедет, когда предикат будет на контуре.
    // МОЛЧАНИЕ, А НЕ ПУСТАЯ СТРОКА, когда форма про поле ничего не знает. У сервера здесь семантика
    // ПРИСУТСТВИЯ: `MoodNoteOmitted: pb.MoodNote == nil` (`internal/dto/techcard.go`), то есть
    // отсутствие ключа означает «сохрани хранимое», а явная пустая строка — «сотри». Слать значение
    // безусловно значило бы стирать чужую записку при каждом сохранении карточки, прочитанной
    // клиентом, который поле не заполнил. Человек, стерший записку руками, получает `''` и стирает —
    // это разные состояния, и разница выражается ключом, а не значением.
