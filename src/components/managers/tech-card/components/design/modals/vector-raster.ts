 ...
 * непрозрачности и кладётся на документ ОДИН раз. Ластик едет по той же дороге и с круга 15 кладёт
 * БУМАГУ (`PAPER_INK`), а не снимает материал, — см. довод у `commitStage`; арифметика
 * непрозрачности от этого не меняется ни одной веткой в коде дабов.
 *
...
/**
 * ЦЕНТРОСТРЕМИТЕЛЬНЫЙ CATMULL-ROM: ОДИН СЕГМЕНТ p1→p2 ПО ЧЕТЫРЁМ ТОЧКАМ (круг 15, J-35).
 *
 * ⚠ ПОЧЕМУ ВООБЩЕ СПЛАЙН. Владелец: «когда мы рисуем брашем в эдит моде он какой-то угловатый
 * выходит рисунок не smooth». Дословно так: путь кисти был ЛОМАНОЙ по сэмплам `pointermove`, и
 * дабы раскладывались по ПРЯМЫМ отрезкам между соседними. Chromium отдаёт указатель кадрами
 * (~60 Гц), при быстрой руке соседние сэмплы отстоят на 10-30 px, и на повороте видно угол:
 * провис хорды на дуге R = 150 юнитов при шаге 30° — 5.1 юнита.
 *
 * ⚠ ПОЧЕМУ ЦЕНТРОСТРЕМИТЕЛЬНЫЙ (α = 0.5), А НЕ РАВНОМЕРНЫЙ. Равномерный Catmull-Rom на НЕРАВНЫХ
 * шагах — а у руки они всегда неравны — даёт петли и выбросы за пределы своих же точек: кисть
 * «выстреливала» бы вбок там, где рука притормозила. Центростремительная параметризация это
 * исключает по построению, и именно поэтому её здесь стоит назвать вслух: равномерная короче на
 * три строки и хуже ровно в том месте, ради которого сплайн и заводится.
 *
 * Точки приходят и уходят ДОЛЯМИ кадра, а считаются в ПИКСЕЛЯХ РАСТРА: узловая параметризация
 * меряет расстояния, и в долях они анизотропны — по y в `h/w` раз мельче, чем по x.
 */
export function smoothSegment(
  ...
  const to = ([x, y]: readonly [number, number]) => [x * box.w, y * box.h] as [number, number];
  const a = to(p0);
  const b = to(p1);
  const c = to(p2);
  const d = to(p3);
  const knot = (t: number, u: [number, number], v: [number, number]) =>
    t + Math.max(1e-4, Math.sqrt(Math.hypot(v[0] - u[0], v[1] - u[1])));
  const t0 = 0;
  const t1 = knot(t0, a, b);
  const t2 = knot(t1, b, c);
  const t3 = knot(t2, c, d);
  const span = Math.hypot(c[0] - b[0], c[1] - b[1]);
  /* Шагов ровно столько, чтобы хорда САМОГО сплайна была меньше пикселя растра: дальше дробить
     нечего — `dabPoints` всё равно разложит отпечатки по своему шагу. */
  const steps = Math.max(2, Math.min(48, Math.ceil(span / 3)));
  ...
  for (let i = 1; i <= steps; i++) {
    ...
    const A1 = mix(a, b, t0, t1);
    const A2 = mix(b, c, t1, t2);
    const A3 = mix(c, d, t2, t3);
    const B1 = mix(A1, A2, t0, t2);
    const B2 = mix(A2, A3, t1, t3);
    const C = mix(B1, B2, t1, t2);
    out.push([C[0] / box.w, C[1] / box.h]);
  }
  return out;
}
 ...
 *
 * Владелец: «ERASE в эдит моде должен стирать просто белым цветом а не удалять фон». До этого
 * круга ластик был `destination-out`: он честно снимал материал до ПРОЗРАЧНОСТИ, и под ним для
 * того и стояла шахматка `data-raster-checker`, чтобы дырку было видно. Это верно для слоя,
 * который лежит на чём-то; здесь он лежит на ФОТОГРАФИИ, и дырка показывает не «ничего», а
 * чужую землю — на флэте бумагу, на рендере то, что окажется под ним завтра.
 *
 * Теперь ластик — это кисть цветом бумаги: тот же `source-over`, та же непрозрачность, та же
 * жёсткость. Прецедент в этом же файле старше на круг — `PAPER_INK` у «delete inside» (G-14).
 *
 * ⚠ РЕЖИМ ОСТАЁТСЯ РАЗЛИЧИМЫМ, ХОТЯ КОМПОЗИТ У ОБОИХ ОДИН. Он несёт РЕЗКУ ЛИНИЙ (Y-9: ластик
 * режет штрихи, кисть нет) и цвет ниба (`growRasterGesture`), и слить его с `paint` значило бы
 * потерять оба различия ради одной сэкономленной ветки.
 *
...
export function renderView(
  ...
  /* ЖИВОЙ ЖЕСТ РИСУЕТСЯ ТЕМ ЖЕ КОМПОЗИТОМ, ЧТО И ЛЯЖЕТ — включая ластик, который с круга 15
     кладёт бумагу, а не снимает материал (см. `commitStage`). Разойдись они, превью ластика
     показывало бы дырку там, где ляжет белое. */
  ctx.globalCompositeOperation = 'source-over';
  ctx.drawImage(layer.stage, 0, 0);
  ctx.restore();
}
