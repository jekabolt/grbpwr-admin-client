...
/**
 * СТЕРЕТЬ ПИКСЕЛИ ВНУТРИ ОБЛАСТИ — до прозрачности, СКВОЗЬ ПОДЛОЖКУ.
 *
 * Это половина `softenInside` — та самая, которой у удаления не было вовсе: «delete inside» резал
 * ШТРИХИ и никогда не трогал краску, хотя стоял в одном ряду с «soften inside», которая пиксели
 * трогает. Один ряд глаголов «inside», и один из них молча про другой материал — ровно та
 * асимметрия, на которую владелец и пожаловался: «при выделении не работает удаление на самой
 * картинке».
 *
 * МАСКА ПРИХОДИТ ГОТОВОЙ И РАСТУШЁВАННОЙ. Растушёвка области играет здесь ту же роль, что у
 * смягчения: заданная — край удаления сходит на нет, не заданная — режет ровно по дорожке. Второй
 * арифметики края тут нет и быть не должно, иначе одно и то же число значило бы у двух соседних
 * кнопок разное.
 *
 * `destination-out` СКВОЗНОЙ ПО УСТРОЙСТВУ: документ и есть полное состояние пикселей, заведённое
 * копией подложки, поэтому вычитание альфы прогрызает и её — прямой ответ владельца на прямой
 * вопрос про ластик. Отдельной маски базы не существует, и заводить её не надо.
 */
export function clearInside(layer: RasterLayer, mask: HTMLCanvasElement): void {
  const ctx = ctxOf(layer.doc);
  ctx.save();
