 ...
 */
/**
 * ⚠ КРУГ 8 (Q-7), ПРЯМОЕ РЕШЕНИЕ ВЛАДЕЛЬЦА: «убрать ограничения по размерам», диапазон 0.25 … 200.
 * Это НЕ отмена Y-1 («все браши слишком болд»): та жалоба была про УМОЛЧАНИЕ, и умолчание осталось
 * тонким (2). Y-1 сузил заодно и концы — тогда это было единственным способом сделать тонкий край
 * достижимым мышью на РАВНОМЕРНОМ регуляторе. Регулятор теперь логарифмический, и рабочая полоса
 * 0.25…6 занимает на нём 47 % хода при полном диапазоне до 200 — то есть тонкое стало доступнее,
 * чем было при потолке 20, а толстое перестало быть запрещённым.
 */
export const MIN_GAUGE = 0.25;
...
/**
 * Размер к записи: ДВАДЦАТЫЕ доли пикселя платы. Дальше — цифры, которых не видно ни на одном зуме.
 *
 * Шаг был десятой, и при новом полу 0.25 он врал бы человеку: набранное 0.25 записалось бы как 0.3
 * — число, которого никто не выбирал. Двадцатая — НАДМНОЖЕСТВО десятой: всякое уже сохранённое
 * значение (кратное 0.1) проходит через неё нетронутым, поэтому ни один лежащий документ не
 * меняется ни на байт.
 */
const SIZE_Q = 20;
export const roundGauge = (n: number) =>
  Math.round(Math.min(SIZE_CEIL, Math.max(SIZE_FLOOR, n)) * SIZE_Q) / SIZE_Q;
 ...
 */
/**
 * ⚠ ОКРУГЛЕНИЯ ДО ЦЕЛОГО ЗДЕСЬ БОЛЬШЕ НЕТ. Оно стояло, пока пол был 1 и дробей просто не
 * существовало; с полом 0.25 оно превращало набранное 0.25 в 0, а тот — обратно в пол. То есть
 * самый тонкий конец нового диапазона был недостижим НИ МЫШЬЮ, НИ КЛАВИАТУРОЙ. Квантование —
 * забота `roundGauge`/`roundStep`, оно ниже по течению и знает про формат.
 */
 ...
 */
export const CONSTRUCTION_DASH: [number, number] = [3.33, 2.5];
 ...
 */
export const LOCK = { pitch: 3.83, duty: 0.65 };

/**
 * САМАЯ КОРОТКАЯ ЛИНИЯ, НА КОТОРОЙ ЭТОТ ШОВ ВООБЩЕ НАРИСУЕТСЯ — в юнитах платы, при данной длине
 * стежка.
 *
 * ⚠ ЗАЧЕМ ЭТО ВЫВЕДЕНО НАРУЖУ. Каждый генератор фигуры МОЛЧА возвращает пустую строку на линии
 * короче одной внятной фигуры, и вызывающий подставляет прямую (`if (!d) d = plainD()`). Пока
 * потолок длины стежка был 60, порог потайного шва равнялся 660 юнитам — две трети чертежа, редкий
 * край. С потолком 200 (Q-7) он стал 2200, то есть НА ЛЮБОЙ линии потайной шов молча рисуется
 * прямой. Молчаливая подмена, ставшая нормой, — это уже не аккуратность, а сломанный орган выбора:
 * человек переключает вид шва и не видит разницы.
 *
 * Числа взяты из порогов самих генераторов (`wavePath` 1.5·wl, `stitchPath` 1.5·pitch,
 * `stitchedRails` 2·spacing, `blindPath` 1.2·period) и обязаны меняться вместе с ними.
 */
export function stitchMinLength(brush: StitchKey, step: number): number {
  const S = step;
  switch (brush) {
    case 'zigzag':
      return ZIG.wl * S * 1.5;
