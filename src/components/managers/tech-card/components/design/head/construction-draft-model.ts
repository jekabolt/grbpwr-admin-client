...
export type FormSnapshot = {
   ...
   */
  detailSlots?: Array<{ id?: number; name?: string }>;
};
...
export function diffProposal(
  ...
  // сервер узнаёт «Sleeve / Cuff», «sleeve_cuff» и «sleeveCuff» как один ключ. Имя слота человек
  // потом переименовывает как хочет; узнаём мы его тем, чем узнаём всё остальное на этой полосе.
  const benchNames = new Set(
    (form.detailSlots ?? []).map((s) => foldToken(s.name)).filter(Boolean),
  );
