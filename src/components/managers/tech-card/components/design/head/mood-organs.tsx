 ...
 * SLOTS — одними и теми же органами: пилюля `from the moodboard`, пилюля `moodboard moved on`,
 * пилюля происхождения поля (`drafted` / `drafted, edited` / `by hand`), полоса `LOCKED …` с дверью,
 * раскрытие `show ▸ / hide ▾`. У продукта их не было, а три копии по трём файлам разошлись бы на
 * первой же правке — поэтому они лежат здесь, ОДИН раз, поверх примитивов `ui/components`.
 *
...
export function FromMoodboardPill(): JSX.Element {
  return <Pill tone='ink'>from the moodboard</Pill>;
}
