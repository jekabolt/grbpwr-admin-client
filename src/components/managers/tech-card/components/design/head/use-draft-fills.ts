 ...
 *
 * ⚠ ВЛАДЕЛЕЦ — КАРТОЧКА, И ЭТО ПРОВЕРЯЕТСЯ ПРИ ЧТЕНИИ (образец — `../flat-slots-send.ts`). Ключ уже
 * несёт id, но хранилище правит кто угодно (соседняя вкладка, ручная чистка, старая сборка), поэтому
 * в значении лежит `owner`, и запись с чужим владельцем читается как ПУСТАЯ: пометки карточки 38 на
 * карточке 41 были бы хуже, чем отсутствие пометок. Разбор недоверчивый — берётся только то, что
 * похоже на запись журнала (`isStoredFill`); всё остальное молча отбрасывается.
 *
