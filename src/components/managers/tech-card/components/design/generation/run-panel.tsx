...
export function RunPanel({
   ...
   */
  /* ⚠ ПЛЕЙГРАУНД ЧИТАЕТ КАРТОЧКУ НЕ БОЛЬШЕ, ЧЕМ ПЕРЕКРАС, И ЭТО СЕРВЕРНОЕ ПРАВИЛО, А НЕ ДОГАДКА:
     `designKindReadsTheCard` возвращает false для `freeform` и `cutout`, поэтому плит верстака в
     их снимке нет вовсе, а если бы они там оказались (снимок старого прогона, чужая ветка), панель
     обязана применить то же сужение, что применил воркер, — иначе она пересчитает картинки, до
     модели не доехавшие. Тот же довод, что у рекола и паттерна двумя строками выше. */
  const sentRows =
    run.kind === 'threed'
      ? plateRows
      : run.kind === 'recolor' ||
