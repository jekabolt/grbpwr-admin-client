 ...
 *
 * Владелец, круг 20, дословно: «когда я смотрю GENERATION HISTORY первая страница пустая потом я
 * нажимаю на следующую страницу и возвращаюсь обратно страницы появляютя проблемма в фильтрах и в
 * архиве».
 *
 * ПОЧЕМУ ТАК БЫВАЕТ. Лента страничится на СЕРВЕРЕ (12 прогонов страницей), а сужают её ДВА
 * клиентских решения, о которых сервер не знает: фильтр рода (`visible`) и вечное исключение
 * архива. Сервер честно прислал полную страницу, окно честно нарезало её по три — и обе честности
 * вместе дают пустой экран под живым пейджером. Дочитывали до этого только двое: «show all» и
 * раскрытая полка архива; случай «окно короче страницы, а у сервера есть ещё» не читал НИКТО, и
 * человеку приходилось нажимать `older ›` и возвращаться `‹ newer`, чтобы строки «появились».
 *
 * ⚠ ОТДЕЛЬНЫЙ КОМПОНЕНТ, А НЕ ЭФФЕКТ В ОРГАНЕ, И ЭТО НЕСУЩЕЕ. Он монтируется ВНУТРИ `Section`,
 * поэтому у свёрнутого блока его нет вовсе — а свёрнут блок на четырёх вкладках из пяти. Эффект,
 * стоявший на верхнем уровне органа, вычитывал за закрытой дверью всю историю карточки; разбор — на
 * месте, откуда он снят.
 *
 * ⚠ ПОЧЕМУ ЭТО КОНЕЧНО — ЧЕТЫРЕ СТОРОЖА, И КАЖДЫЙ ЗАКРЫВАЕТ СВОЙ СПОСОБ ЗАЦИКЛИТЬСЯ.
 *   1. `!loading` — запрос в полёте не порождает второго. Пока страница едет, условие ложно на
 *      каждом рендере, поэтому цикл идёт ШАГАМИ ПО ОТВЕТАМ, а не по перерисовкам.
 *   2. `hasMore` — гаснет сам, когда сервер отдал последний курсор.
 *   3. Само условие сравнивает ЗАГРУЖЕННОЕ с ЗАПРОШЕННЫМ (`want`), а не с бесконечностью: набрав
 *      свои три строки, окно останавливается, и следующая страница читается только когда человек
 *      шагнул на неё сам.
 *   4. `budget` — ПОТОЛОК ЧИСЛА ЗАПРОСОВ, и он здесь потому, что первых трёх НЕ ХВАТАЛО. `have`
 *      считается ПОСЛЕ клиентского сужения: страница, приехавшая без единой подходящей строки,
 *      оставляет его тем же, `hasMore` истиной — и сторожа 1–3 пропускают следующий круг. На
 *      карточке, у которой прогонов нужного рода нет вовсе, это шаг за шагом вычитывало ВСЮ
 *      историю, по запросу на серверную страницу, за одно раскрытие блока. Разбор потолка и
 *      довод «ровно одна страница» — у `autofillBudget` в теле органа.
 * Зависимости — ЧИСЛА И ФЛАГИ, поэтому эффект просыпается на смене факта, а не на каждом рендере;
 * `fetchMore` и `onSpend` в них не входят: они пересоздаются на каждом рендере родителя, и
 * зависимость от них запускала бы эффект на каждый рендер вместо фактов, которые что-то решают.
 *
 * ⚠ ЧЕГО ЭТОТ ЦИКЛ НЕ ДЕЛАЕТ: он не расширяет окно, не меняет размер серверной страницы и не
 * впускает архив в окно — у архива своя полка, и это нарочно.
 */
function HistoryWindowAutofill({
  ...
  /** Сколько строк окно у себя уже спросило: `PAGE * (страница + 1)`. */
  want: number;
  /** Сколько их набралось после клиентского сужения. */
  have: number;
  hasMore: boolean;
  loading: boolean;
  /** Сколько серверных страниц ещё разрешено взять на ЭТОМ положении окна (`autofillBudget`). */
  budget: number;
  ...
  useEffect(() => {
    if (have < want && hasMore && !loading && budget > 0) {
      // ПОРЯДОК ЗДЕСЬ НЕСУЩИЙ: списание идёт ПЕРВЫМ. `fetchMore` синхронно поднимает `loading`
      // не всегда (первый зов лишь взводит `wanted`), и списание после него оставляло бы кадр,
      // в котором бюджет ещё цел, а запрос уже ушёл, — то есть вторую страницу задаром.
      onSpend();
      fetchMore();
    }
...
export function GenerationHistory({
   ...
   *
   * ⚠ САМА ЗАПИСЬ ПЕРЕЕХАЛА В ЭФФЕКТ (ниже, рядом с дочитывателями), И ЭТО ЧАСТЬ ПОЧИНКИ B-1.
   * Здесь стояло `if (page > reachable) setPage(reachable)` — правка состояния ПРЯМО В ТЕЛЕ
   * РЕНДЕРА, и она отличается от двух законных таких правок выше: те сравнивают проп с рефом и
   * срабатывают ровно на смене вкладки или карточки, а эта сравнивает состояние с величиной,
   * которая моргает сама по себе. `reachable` учитывает `more.loading`, поэтому продолжение,
   * приехавшее без единой подходящей строки, роняло `reachable` на единицу ПОСРЕДИ рендера — и
   * окно прыгало под курсором в тот самый кадр, когда человек читал. Рисуется по-прежнему
   * зажатое `current`, то есть кадр верен всегда; выравнивается `page` уже после коммита.
   */
  ...
  /**
   * ═══ ТРЕТИЙ СЛУЧАЙ: ОКНО КОРОЧЕ СВОЕЙ СТРАНИЦЫ — ОНО САМО ПРОСИТ НЕДОСТАЮЩЕЕ (B-1, D-3) ═══
   *
   * Владелец, круг 20, дословно: «когда я смотрю GENERATION HISTORY первая страница пустая потом
   * я нажимаю на следующую страницу и возвращаюсь обратно страницы появляютя проблемма в
   * фильтрах и в архиве».
   *
   * ЭТОГО ЦИКЛА НЕ БЫЛО ВОВСЕ, И ИМЕННО ЕГО ОТСУТСТВИЕ — ОБА КОРНЯ РАЗОМ. Лента страничится на
   * СЕРВЕРЕ (12 прогонов страницей), а сужают её ДВА клиентских решения, о которых сервер не
   * знает: фильтр рода (`visible`) и вечное исключение архива (`unfiltered`). Сервер честно
   * прислал полную страницу, окно честно нарезало её по три — и обе честности вместе дают пустой
   * экран под живым пейджером:
   *   · вкладка открывается на СВОЁМ роде (`defaultRep`), и если среди первых 12 прогонов
   *     карточки нет ни одного рендера, вкладка FABRIC RENDER показывает ноль строк;
   *   · заархивированный прогон занимает слот серверной страницы и не даёт окну ни строки, так
   *     что на карточке, где свежие прогоны убраны в архив, первая страница пуста или коротка.
   * Дочитывали до сих пор только двое: «show all» (`showAll`) и раскрытая полка архива. Случай
   * «окно короче страницы, а у сервера есть ещё» не читал НИКТО — и человеку приходилось нажимать
   * `older ›` (который И дочитывает, И шагает) и возвращаться `‹ newer`, чтобы строки «появились».
   * Именно этот жест владелец и описал.
   *
   * ⚠ ПОЧЕМУ ЭТО КОНЕЧНО И ЧЕГО СТОИТ — РАЗБОР ЖИВЁТ ОДИН, У САМОГО ОРГАНА
   * (`HistoryWindowAutofill` выше, четыре сторожа) и у его бюджета (`autofillBudget` ниже).
   * Здесь стоял ВТОРОЙ ЕГО ЭКЗЕМПЛЯР, слово в слово, — и он успел протухнуть первым же
   * изменением: обещал «дочитает историю до конца, ровно как „show all“», а потолок в одну
   * серверную страницу на положение окна этого больше не делает. Две копии одного правила
   * расходятся молча; остаётся адрес, а не пересказ.
   *
   * ⚠ ЧЕГО ЭТОТ ЦИКЛ НЕ ДЕЛАЕТ: он не расширяет окно, не меняет размер серверной страницы и не
   * впускает архив в окно — у архива своя полка, и это нарочно. Он лишь доводит ЗАГРУЖЕННОЕ до
   * того, что окно у него уже спросило.
   */
  /*
   * ⚠ ЭТОГО ЦИКЛА ЗДЕСЬ БОЛЬШЕ НЕТ — ОН ПЕРЕЕХАЛ ВНУТРЬ БЛОКА (`HistoryWindowAutofill` ниже), И
   * ЭТО ПОЧИНКА, А НЕ ПЕРЕСТАНОВКА.
   *
   * Эффект стоял ЗДЕСЬ, на верхнем уровне органа, — то есть работал и тогда, когда блок СВЁРНУТ, а
   * `defaultOpen={false}` стоит у ЧЕТЫРЁХ из пяти его монтажей (`studio-tab.tsx`). Свёрнутый блок
   * ничего не показывает вовсе: `Section` при `collapsible && !open` детей НЕ МОНТИРУЕТ. Значит на
   * вкладке, чей `defaultRep` не встретился в первой серверной странице (12 прогонов), `visible`
   * оставался нулём, `hasMore` — истиной, и цепочка вычитывала ВСЮ историю карточки по 12 строк за
   * запрос, пока сервер не кончится, — за закрытой дверью, никем не читаемая. Подпись оправдывала
   * цену словами «ровно как „show all“», но `show all` — это КНОПКА, которую нажал человек, и
   * согласие на трафик даёт нажатие.
   *
   * ГЕЙТ СДЕЛАН МОНТАЖОМ, А НЕ ФЛАГОМ, и это единственный честный способ его написать: состояние
   * «раскрыт» принадлежит `Section` (человек может нажать стрелку сам, и `defaultOpen` этого уже не
   * знает), наружу примитив его не отдаёт, а спрашивать общий примитив ради одного блока значило бы
   * менять его для всех вызывающих. Ребёнок, стоящий ВНУТРИ `Section`, монтируется ровно тогда,
   * когда блок раскрыт, и размонтируется, когда его свернули, — то есть отвечает на вопрос
   * «смотрят ли на это сейчас» по построению, а не по копии флага.
   *
   * ЧТО ПРИ ЭТОМ НЕ ИЗМЕНИЛОСЬ: опрос живого прогона (`useRunPolling`) по-прежнему идёт у
   * свёрнутого блока — он про ДЕНЬГИ, а не про показ (см. довод у пропа `defaultOpen`), и трогать
   * его нельзя.
   */

  /**
   * ЗАЖИМ ОКНА ЗАПИСЫВАЕТСЯ НАЗАД — ПОСЛЕ КОММИТА, А НЕ В ТЕЛЕ РЕНДЕРА. Разбор, зачем запись
   * вообще нужна и почему одна страница перелёта законна, стоит у `reachable` выше; здесь только
   * место. Рисуется зажатое `current`, поэтому кадр верен и до того, как эффект выровняет `page`.
   */
  useEffect(() => {
   ...
   *
   * ЧТО БЫЛО НЕОГРАНИЧЕНО. Условие дочитывателя сравнивает `visible.length` (число ПОСЛЕ
   * клиентского сужения) с `want`. Страница, приехавшая без единой подходящей строки, оставляет
   * `have` тем же, а `hasMore` истиной — и эффект просыпается на каждом переходе `loading`,
   * то есть шагает по истории до самого её начала. Цикл конечен и объясняет себя словами, но на
   * длинной архивной истории одно раскрытие блока стоит запроса на страницу. Согласие было дано
   * ОДИН РАЗ, а трафика потрачено сколько угодно.
   *
   * ПОЧЕМУ ГРАНИЦА ИМЕННО ОДНА СТРАНИЦА, А НЕ «ДВЕ» И НЕ «ПЯТЬ». Довод самого дочитывателя —
   * «ОКНО КОРОЧЕ СЕРВЕРНОЙ СТРАНИЦЫ»: окно просит три строки, сервер отдаёт двенадцать, и весь
   * разрыв, ради которого орган заведён, закрывается ОДНОЙ страницей. Всё, что дальше, — уже не
   * закрытие разрыва, а догадка о ПЛОТНОСТИ нужного рода в истории, которой у клиента нет
   * никаких оснований. Число 1 поэтому не выбрано из вкуса: оно и есть тот самый разрыв,
   * названный в доводе, а не круглая величина рядом с ним.
   *
   * ЧЕЛОВЕКУ ПРИ ЭТОМ НЕ ЗАКРЫТО НИЧЕГО. Дальше историю читают ДВА нажатия — `older ›` (оно и
   * само зовёт `fetchMore` на последней локальной странице) и «show all», у которого своя цепочка
   * без потолка. Нажатие и есть согласие на трафик; ровно этим доводом был снят и прежний
   * дочитыватель за закрытой дверью.
   *
   * СЧЁТЧИК СБРАСЫВАЕТСЯ СМЕНОЙ ПОЛОЖЕНИЯ ОКНА (`rep` + `current`), А НЕ ТАЙМЕРОМ. Оба слагаемых
   * меняются только от того, что ПРОИЗОШЛО: сегмент переключил человек, страницу — он же или
   * приехавшие строки (`current` растёт лишь вслед за `pageCount`). Значит новая страница
   * запрашивается только там, где окно и правда встало на новое место, и «шаг вперёд» никогда не
   * стоит больше одного запроса.
   */
  const [autofillSpent, setAutofillSpent] = useState(0);
  // ⚠ КАРТОЧКА — ЧАСТЬ ПОЛОЖЕНИЯ ОКНА, И БЕЗ НЕЁ БЮДЖЕТ ПЕРЕТЕКАЕТ К СОСЕДУ. `StudioTab` не
  // размонтируется при переходе на другую карточку, а блок сброса выше не трогает `autofillSpent`:
  // при переходе страница уже 0, сегмент уже дефолтный, полки закрыты — ни одного `setState`, и
  // строка слота осталась прежней. Значит страница, дочитанная на карточке A, считалась
  // потраченной на карточке B, и её окно молчало «нет среди загруженных», требуя ровно того жеста
  // (`older ›`, потом `‹ newer`), ради устранения которого предел и заводился.
  ...
  if (autofillSlotRef.current !== autofillSlot) {
    ...
    // Правка состояния в теле рендера — тот же штатный приём, что у двух соседей выше: сравнение
    // с рефом, а не с моргающей величиной. `autofillSlot` не моргает — он меняется от события.
  ...
  /**
   * ПУСТОЕ ОКНО НАЗЫВАЕТ СВОЮ ПРИЧИНУ — И ПОД `all` ТОЖЕ (B-1). Фраза стояла под условием
   * `rep !== 'all'`, то есть ровно в том случае, который владелец и описал — «первая страница
   * пустая», — экран не говорил НИЧЕГО: голый пейджер над пустым местом. Гейт снят, причин
   * ровно три, и каждая названа своим словом: окно ещё дочитывает свою страницу (цикл выше),
   * сегмент сузил ленту до рода, которого среди загруженного нет, или всё загруженное убрано в
   * архив — у него своя полка, и в окно он не впускается нарочно.
   *
   * ⚠ ФОРМУЛИРОВКА СУЖЕННОГО СЛУЧАЯ ЗАМОРОЖЕНА ПРОБОЙ (`qa-g1`, 6c) — «no 3D generations among
   * the loaded runs», без хвоста. Хвост «show all reads the rest of the history» ушёл вместе со
   * своей причиной: окно больше не ждёт, что человек нажмёт «show all», оно дочитывает само — и
   * пока читает, так и говорит.
   */
  /**
   * ⚠ «ЧИТАЮ» ГОВОРИТСЯ, ТОЛЬКО ПОКА ДЕЙСТВИТЕЛЬНО ЧИТАЕТСЯ. Здесь стояло `more.hasMore ||
   * more.loading`, и это было верно ровно до потолка дочитывателя: тогда «у сервера есть ещё»
   * и правда означало «сейчас дочитаю». С бюджетом в одну страницу окно замолкает раньше
   * сервера, и прежнее условие оставило бы на экране вечное «reading earlier runs…» под ничем.
   * Читателей ровно три, и все три названы: «show all» (без потолка), полка архива (до своего
   * обещанного числа) и дочитыватель окна (пока цел бюджет). Молчащему окну отвечает фраза
   * СУЖЕННОГО случая — «among the loaded runs», — и она честна именно потому, что читать
   * перестали: названо ровно то, что загружено.
   */
  const stillReading =
    more.loading ||
    (more.hasMore &&
      (showAll ||
        autofillBudget > 0 ||
        (archShown && archivedLoaded.length < archivedRuns)));
  const windowNote =
    stillReading
      ? 'reading earlier runs…'
      : rep !== 'all'
        ? `no ${repLabel(rep)} generations among the loaded runs`
        : // ⚠ УТВЕРЖДЕНИЕ О ПОЛНОТЕ ПРОВЕРЯЕТСЯ ПОЛНОТОЙ, А НЕ НАЛИЧИЕМ. Здесь стояло
          // `archivedRuns > 0` — «есть архивные» выдавалось за «архивные ВСЕ». До предела
          // дочитывания эта ветка была недостижима с непрочитанными страницами (`stillReading`
          // держал её), а с пределом стала достижимой: карточка, у которой все 12 свежих прогонов
          // архивные, а живые лежат глубже, говорила бы человеку, что живых нет вовсе — и он ушёл
          // бы на полку, увидел 12 строк и поверил.
          totalRuns > 0 && totalRuns === archivedRuns
          ? 'every run on this card is archived — open the archived shelf above'
          : 'no runs to show';
  ...
  return (
    ...
    <Section
      ...
      /* ⚠ СВЁРНУТАЯ ЛЕНТА НЕ НЕСЁТ НИЧЕГО, КРОМЕ ИМЕНИ И СТРЕЛКИ.
         ...
         примитиве (`ui/components/section.tsx`), разбор — там.

         ⚠ ЗДЕСЬ СТОЯЛА ССЫЛКА НА ТРИ КОМПЕНСАЦИИ, И ДВЕ ИЗ ТРЁХ БЫЛИ ВЫДУМКОЙ. «Строки живого
         прогона студии» (`LiveLine`) в репозитории нет вовсе, а `starting…` держится на полёте
         МУТАЦИИ и гаснет на брони, а не на ответе; развёрнутый `action` ниже — правда, но обе
         истории монтируются `defaultOpen={false}`, то есть в состоянии покоя его не видно. Признак
         прогона в полёте теперь стоит НА СВОЁМ ЭКРАНЕ и в форме ответа: пунктирная ячейка в голове
         полосы выходов (`render/outputs.tsx`, `PendingCell`) на FABRIC RENDER и на 3D, `PendingTile`
         на паттернах, строка прогона на ON MODEL. Свёрнутая лента про прогон молчит, и это правило,
         а не недосмотр. */
      action={
        <div className='flex flex-wrap items-baseline gap-2'>
          {liveRun && (
    ...
    >
      {/* ДОЧИТЫВАНИЕ ОКНА — ТОЛЬКО ПОКА БЛОК РАСКРЫТ. Орган невидим и ничего не рисует; он стоит
          здесь, а не наверху, именно ради монтажа: `Section` детей свёрнутого блока не монтирует,
          и это и есть гейт (разбор — у снятого эффекта выше). `showAll` держит СВОЮ цепочку, и
          вторая рядом с ней просила бы у сервера то же самое дважды. */}
      {!showAll && (
      ...
      {/* ПУСТОЕ ОКНО — СЛОВАМИ, НА МЕСТЕ СТРОК. Пустое место под живым переключателем читается как
          поломка; фраза называет, ЧЕГО нет и почему. Слова собраны в `windowNote` выше — там же и
          разбор, почему условие больше не спрашивает про `all`. Блок при этом не сворачивается: он
          существует потому, что у карточки есть прогоны, а не потому, что окно их нашло. */}
      {visible.length === 0 && (
