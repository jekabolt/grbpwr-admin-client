            ...
            {sides.map((s) => {
              ...
              return (
                <div key={s.view} className='flex min-w-0 flex-col gap-1' data-colour-side={s.view}>
                  <PictureTile
                    /* ⚠ ПОКРАШЕННАЯ КАРТА ПОКАЗЫВАЕТСЯ ТОЛЬКО ПОКА ЖИВА ЭТА ВКЛАДКА, и это честно
                       названная дыра контракта: `DesignColourMap` несёт один `media_id`, а чтения
                       медиа по идентификатору у админского контракта нет. После перезагрузки на
                       плитке стоит ФЛЭТ, а покрашенное называют ярлык и образцы под ним. */
                    url={stored || flat}
