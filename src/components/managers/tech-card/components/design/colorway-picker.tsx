 ...
 * ⚠ КРУГ r3 ПЕРЕСТАВИЛ ОРГАН С РЕЛЬСА НА ЭКРАН: он стоит `for:` в ряду GENERATE — и фабрик-
 * рендера, и 3D, — а слота `action` у рельса больше нет вовсе. Довод — у самого
 ...
 * named — `for:` beside the money, in the GENERATE row, on BOTH of them.
 *
 ...
 * («for:») на ОБОИХ платных экранах, а с рельса снят целиком (`chain-rail.tsx` слота
 * `action` больше не имеет).
 *
 * ОДНА ДВЕРЬ НА ОДИН ВОПРОС. У каждого экрана орган ровно один: render — `for:`; 3D — тот же
 * `for:` в своём ряду GENERATE (владелец: «в 3D сделай выбор колорвея как в фабрик рендере»);
...
export function ColorwaySelect({
  ...
  disabled,
  /**
   * Подпись слева от органа. `for` у ряда GENERATE — на обоих платных экранах одно и то же слово
   * на один и тот же вопрос, «для кого эта покупка». Умолчание оставлено прежним ради
   * вызывающего, который спрашивает просто «чей».
   */
  label = 'colourway',
  /**
