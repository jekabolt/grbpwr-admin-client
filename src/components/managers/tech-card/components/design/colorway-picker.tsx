...
/**
 * ═══ ⚠ ВЕСЬ ЭТОТ ФАЙЛ СТАЛ ПРОДУКТОВО МЁРТВЫМ КРУГОМ 16 (E-1 + E-16), И ЭТО НАЗВАНО ВСЛУХ ══════
 *
 * Владелец: «в MAKE A PATTERN оставь только имя убери колорвей» и «в GENERATION — FABRIC RENDER мы
 * полностью убираем колорвеи только имена остаются». После этих двух пунктов у КАЖДОГО экспорта
 * этого модуля ноль продуктовых вызывающих:
 *   · `ColorwayPicker` — уже стоял без вызывающих с круга 15 (см. вторую записку ниже);
 *   · `ColorwaySelect` — стоял в заголовочной линейке FABRIC RENDER, снят E-16;
 *   · `useColorwayChoice` — состояние студии, снято в `studio-tab.tsx` целиком;
 *   · `colorwayLabel` / `colorwaySubtitle` — их читали палитра, библиотека паттернов и сам пикер.
 * Читатели, оставшиеся сегодня, — только стенды проб (`cw-stand.tsx`, `ceil-stand.tsx`,
 * `frs-stand.tsx`, `j15-stand.tsx`).
 *
 * ФАЙЛ НЕ УДАЛЁН ЭТОЙ ВОЛНОЙ, И ЭТО РЕШЕНИЕ, А НЕ ЗАБЫВЧИВОСТЬ. Снос модуля целиком — правка,
 * которую видно только на ОБЪЕДИНЁННОМ диффе круга: в этом же дереве работают три соседние волны,
 * и файл, снесённый под чужой незакрытой волной, обнаруживается как сломанная сборка у соседа, а
 * не как чистая уборка. Точно тем же приёмом и по тому же доводу здесь однажды была названа
 * смерть `ColorwayPicker` — и следующим кругом это оказалось верным.
 *
 * ⚠ ОСЬ ПРИ ЭТОМ ЖИВА, И ФАЙЛ — НЕ ЕЁ ЕДИНСТВЕННЫЙ СЛЕД. `params.colorway_id`, колонка
 * `design_bench_slot.colorway_id`, `render_bench_colorway_ids` в полосе и
 * `DesignRunKindTakesColorway` на сервере не тронуты ни одной строкой. Клиент перестал ПРЕДЛАГАТЬ
 * выбор и адресует безколорвейный верстак (`0`) — законное, вечное значение. Вернуть орган — это
 * вернуть вызов, а не написать механизм заново.
 *
