...
/* ─────────────────────────────────────────────────────────────────────────────────────────────
 * ⚠ F-13 СНЯЛ СТРОКУ «AI · run 26 · from mixed input» С ОДНОГО ЭКРАНА, А НЕ ИЗ ЭТОГО ФАЙЛА.
 *
 * Владелец, дословно: «убери текст "AI · run 26 · from mixed input"» — про полосу «RENDERS OF
 * THIS CARD», где происхождение у всех строк одно и то же слово и повторялось столько раз,
 * сколько плиток на экране. Снято оно ТАМ (`render/outputs.tsx`, проп `lines`).
 *
 * ПОТРЕБИТЕЛИ ПЕРЕСЧИТАНЫ, А НЕ ПРЕДПОЛОЖЕНЫ. `provenanceLabel` читают подвал плиты верстака,
 * ARTIFACTS, `render/model.ts:stripProvenance` и векторное окно; `mixedInputNote` — тот же подвал,
 * `stripProvenance` и лента генераций. `stripProvenance`, в свою очередь, кормит ОБЕ полосы входа
 * и `what-model-gets`, где отвечает на другой вопрос: «откуда взялось то, что СЕЙЧАС ПОЙДЁТ В
 * ПРОГОН». Молчать об этом там нельзя, поэтому ни одна функция здесь не мертва и ни одна не
 * удалена. Тот, кому снова скажут «убери этот текст», обязан пересчитать список заново.
 * ───────────────────────────────────────────────────────────────────────────────────────────── */
