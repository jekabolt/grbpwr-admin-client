...
/**
 * ═══ ПРЕДЪЯВЛЯТЬ ЛИ УГОЛ `split` — ОДНО МЕСТО НА ВСЕ ЭКРАНЫ (F-8, F-18) ═══════════════════════
 *
 * Владелец, дословно: «на уже заспличеных картинках на ховер сплит писать не нужно так же как и
 * на не мультивью картинках» и «везде где картинка не мультивью флет или рендер там не должно на
 * ховер показываться сплит».
 *
 * ⚠ ЭТО БЫЛО ПРОЗОЙ В `picture-tile.tsx` И ЧЕТЫРЬМЯ ПЕРЕПИСЫВАНИЯМИ НА МЕСТАХ ВЫЗОВА — и ровно
 * поэтому одно из четырёх (плитка референса) не сверялось ни с одним из двух условий вовсе:
 * `!readOnly && url` предъявляло угол любому снимку, принесённому в референсы руками. Правило
 * без единственного носителя расходится не «иногда», а в первом же месте, где его переписали.
 *
 * ТЕПЕРЬ ПРАВИЛО ЖИВЁТ ЗДЕСЬ, И ТОЛЬКО ЗДЕСЬ. Экран отвечает на ДВА факта, которых у примитива
 * нет и быть не может (он знает лишь адрес файла): «склеены ли в этом файле несколько видов» и
 * «резали ли этот лист уже». Оба на входе, вывод один.
 *
 * · `alreadyCut` — «из этого кадра уже что-то вырезано». Каждый экран знает это по-своему —
 *   полоса выходов по колоде, вход 3D по членам семьи, референсы по `readComposite().splitInto`, —
 *   и это законно: способ УЗНАТЬ у них разный, а ВЫВОД обязан быть один.
 *
 * · Картинка, про которую система ЗНАЕТ, что она одновидовая, угла не получает НИКОГДА — и не
 *   гашёного, а не объявленного вовсе: угол это тихий орган, погашенный он читается как
 *   «сломалось», а не как «здесь нечего резать».
 *
 * Единственное исключение живёт в ленте генераций (`generation/generation-history.tsx`) и названо
 * там вслух: это та самая дверь, где ЧЕЛОВЕК ОБЪЯВЛЯЕТ принесённый руками лист многовидовым.
 * Закрыть её этим предикатом значило бы сделать такой лист нерезаемым нигде.
 */
export function pictureOffersSplit(
  picture: common_DesignPicture | null | undefined,
  alreadyCut: boolean,
): boolean {
  return pictureIsComposite(picture) && !alreadyCut;
}
 ...
 *
 * ⚠ СНЯТО ЦЕЛИКОМ (r3, пул). Здесь жили `RenderPlacement` и `renderPlacements()` — заготовка двери
 * «fill the empty sides», которая ставила бы разрезанные кадры по их сторонам одним нажатием. Её
 * ЕДИНСТВЕННЫМ потребителем был `render-input-strip.tsx`, а тот не монтировался ниоткуда с круга
 * r2 (ленты с верха FABRIC RENDER сняты, разметка уехала к самим картинкам в `outputs.tsx`).
 * Проверено грепом по всему `src/`: ноль вызовов, ноль импортов помимо реэкспорта из `./index`.
 * Файл полосы снесён вместе с ними; жест, ради которого всё это писалось, живёт и работает —
 * `mark ▸` на плитке и `apply splitted` в RENDERS OF THIS CARD.
 */
...
export type Gate =
  ...
  | {
       ...
       *
       * ⚠ `colourway` — ЧЛЕН, КОТОРЫЙ ЗНАЧИТ «ДВЕРИ ЗДЕСЬ НЕ РИСОВАТЬ», И ЭТО НЕ ПРОБЕЛ. У отказа
       * по архивному имени выход ОДИН и он уже на экране — селект колорвея в ряду представлений;
       * дверь «generate a render ▸» под ним вела бы на соседний экран, где стоят ТЕ ЖЕ ворота с
       * тем же отказом. Отсутствие поля этого не выражает: `!lock.next` в `threed-input-strip.tsx`
       * как раз и рисует ту самую дверь в никуда. Ни одна ветка полосы этот член не разбирает —
       * ровно поэтому он и заведён.
       *
       * Поле необязательное: у ворот, которым нечего предложить, его нет, и полоса тогда рисует
       * то, что рисовала.
       */
      next?: 'flat' | 'render' | 'front-slot' | 'refill' | 'colourway';
    };

/**
 * ═══ АРХИВНОЕ ИМЯ ЗАКРЫВАЕТ ДВЕРЬ К НОВОЙ РАБОТЕ — ОДНА ФОРМА ОТКАЗА НА ТРИ СТУДИИ ═════════════
 *
 * ⚠ ДО ЭТОЙ ПРАВКИ ДВЕРЬ БЫЛА ЗАКРЫТА ТОЛЬКО СЛОВАМИ, И ЭТО БЫЛО НАПИСАНО ВСЛУХ. Волна, вернувшая
 * архивные колорвеи в селект (иначе их рендеры не видны НИОТКУДА — довод целиком в
 * `colorway-picker.tsx`), тронуть ворота не могла: файл принадлежал соседней волне. Там осталась
 * записка «ВОРОТА ГЕНЕРАЦИИ АРХИВ НЕ ЧИТАЮТ … прогон под снятым именем физически возможен» —
 * честная ровно до того дня, когда её перестанут читать. Теперь читают: подсказка селекта
 * ОБЪЯВЛЯЕТ правило, а эта функция его ИСПОЛНЯЕТ, и записка переписана в ту же минуту.
 *
 * ⚠ ЭТО ЕДИНСТВЕННОЕ УСЛОВИЕ ВО ВСЕЙ ПОЛОСЕ, КОТОРОГО У СЕРВЕРА НЕТ, И ЭТО НАЗЫВАЕТСЯ ЗДЕСЬ, А НЕ
 * ЗАМАЛЧИВАЕТСЯ. Соседний файл (`onmodel/model.ts`) формулирует общее правило: «ворота никогда не
 * шире серверных», потому что придуманное здесь условие запрещало бы законный прогон, а обойти его
 * можно перезагрузкой вкладки — то есть это была бы помеха, а не защита. Оба довода здесь НЕ
 * РАБОТАЮТ, и по разным причинам:
 *   · перезагрузка ничего не снимает. Статус приезжает с КАРТОЧКОЙ (`AdminColorwayRef.status`,
 *     `useTechCard`), а не из черновика на монтаже; после F5 отказ ровно тот же;
 *   · «законность» прогона под архивным именем — вопрос не провода, а учёта. Сервер примет и
 *     заморозит в истории атрибуцию цвету, который больше не разрабатывают; починить это потом
 *     нечем — `colorway_id` прогона неизменяем. Отказ на клиенте стоит нажатия, отказа не
 *     сделавшего — денег и строки в истории навсегда;
 *   · выход у человека ОДИН жест и он на экране: назвать живой колорвей в том же селекте (или
 *     снять архив на вкладке COLORWAYS). Ворота, у которых выход есть, — дверь, а не стена.
 *
 * ⚠ СТОИТ ПЕРВЫМ У КАЖДОЙ ДВЕРИ, ВЫШЕ ДАЖЕ ЗЕРКАЛА СЕРВЕРНОГО ОТКАЗА, И ПОРЯДОК ТУТ — ДОВОД.
 * Остальные отказы посылают ЧТО-ТО СДЕЛАТЬ: начертить флэт, положить кадр во ФРОНТ, переложить
 * сторону. Под архивным именем каждое из этих указаний исполнимо и БЕСПОЛЕЗНО — сделав всё,
 * человек упрётся в эту же строку. Отказ, отправляющий работать в тупик, дороже отказа, который
 * первым же словом называет настоящую причину.
 *
 * ⚠ И ОН ЗАПРЕЩАЕТ РОВНО НОВЫЙ ПРОГОН — БОЛЬШЕ НИЧЕГО. Живёт он в ВОРОТАХ, а не в `disabled`
 * экрана, именно поэтому: верстак, выходы, разметка сторон и разрез мультивью под архивным
 * колорвеем работают слово в слово как под живым. Погасить экран значило бы спрятать ту самую
 * работу, ради достижимости которой архивный колорвей и вернули в селект, — то есть починить
 * дефект его же второй половиной.
 *
 * ФОРМА ОТКАЗА ОДНА, А СУЩЕСТВИТЕЛЬНОЕ РАЗНОЕ (`work`): «fabric render» / «3D build» /
 * «on-model picture». Три написания одной фразы разъехались бы в первый же день; одно написание,
 * не называющее предмет, говорило бы на трёх экранах об одном и том же «прогоне» — а человек
 * стоит на конкретном.
 */
export function archivedColorwayGate(
  /** Резольвнутый ответ `useColorwayChoice`, а не статус: предикат архива — ОДИН, и он там. */
  archived: boolean,
  /** Имя выбранного. Пусто бывает только под `no colourway`, а он архивным не бывает вовсе. */
  colorwayLabel: string,
  /** Что именно отказано купить, существительным: «fabric render», «3D build», … */
  work: string,
): Gate {
  if (!archived) return { ok: true };
  const named = colorwayLabel.trim() || 'this colourway';
  return {
    ok: false,
    reason:
      `${named} is archived — no new ${work} is made under a colourway that is no longer worked ` +
      `on. Everything already made under ${named} stays here to read, mark and split; pick a live ` +
...
export function renderGate(
  band: GetDesignBandResponse,
  /**
   * ⚠ АРХИВНОЕ ИМЯ — ПЕРВЫЙ ВОПРОС ЭТИХ ВОРОТ, А НЕ ПОСЛЕДНИЙ (довод у `archivedColorwayGate`).
   * Умолчание `false` держит прежнее поведение у вызывающего, который колорвея не назвал: полоса
   * входа читает эти же ворота ради списка обязательных сторон, и ей про архив знать нечем.
   */
  colorwayArchived: boolean = false,
  /** Его имя — отказ им НАЗЫВАЕТ цвет, как это делают соседние отказы 3D. */
  colorwayLabel: string = '',
): Gate {
  const live = archivedColorwayGate(colorwayArchived, colorwayLabel, 'fabric render');
...
export function threedGate(
  ...
  colorwayLabel: string = '',
  /**
   * ⚠ АРХИВЕН ЛИ ЭТОТ КОЛОРВЕЙ. Четвёртым, а не первым, НАРОЧНО: три вызова с прежней позиционной
   * подписью (`band, id, label`) остаются верными, и вызывающий, которому статуса взять неоткуда,
   * получает ровно прежние ворота. Умолчание `false` — «не сказано», а не «архивен».
   */
  colorwayArchived: boolean = false,
): Gate {
  /**
   * ⚠ ЭТОТ ОТКАЗ СТОИТ ВЫШЕ ЗЕРКАЛА СЕРВЕРНОГО, И ПРАВИЛО «СНАЧАЛА СЕРВЕРНЫЙ» ЗДЕСЬ УСТУПАЕТ
   * ОСОЗНАННО. Оно написано затем, чтобы первая фраза экрана про 3D совпадала с той, которой
   * ответил бы сервер, — и верно, пока оба отказа про ОДНО: про материал. Архив — про ИМЯ, и
   * серверного близнеца у него нет вовсе (довод у `archivedColorwayGate`). Показать под архивным
   * колорвеем «draw the flats, render them, then come back» значило бы отправить человека делать
   * работу, после которой он упрётся в ту же стену; порядок ниже от этой вставки не меняется —
   * `fabricRenderGate` по-прежнему первый среди отказов ПРО МАТЕРИАЛ.
   */
  const live = archivedColorwayGate(colorwayArchived, colorwayLabel, '3D build');
  if (!live.ok) return live;
