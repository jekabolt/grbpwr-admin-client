...
export function StudioTab({
   ...
   *
   * ЗДЕСЬ СТОЯЛА СОБСТВЕННАЯ КОПИЯ `navTo` (`useSearchParams` + та же запись `?tab=` с чисткой
   * протухших `sample`/`fits` и `replace`), и завёл её не выбор, а ЗАПРЕТ: волна, писавшая блоки
   * CONSTRUCTION, не имела права трогать `index.tsx`. Копия была верной построчно — и именно
   * поэтому опасной: ЧИТАТЕЛЬ у неё ОДИН (`index.tsx` берёт активную вкладку из `?tab=`), а
   * писателей стало два. Первая же правка правила («не ронять ещё и `?bom=`», «пушить, а не
   * replace») попала бы в один из двух, и вкладка вела бы себя по-разному в зависимости от того,
   * какая кнопка её открыла, — расхождение, которого не видно ни в типах, ни в сборке.
   *
   * Теперь писатель ровно один и живёт у владельца адреса. Проп ОБЯЗАТЕЛЕН, а не «не задан —
   * двери нет»: две двери отсюда («связать материал ›» в спецификации и размерный ряд в общих
   * сведениях) — это работа, а не украшение, и композитор, смонтированный без адресата, обязан
   * не собраться, а не тихо потерять их.
   */
