 ...
 *
 * ⚠ `parts` НА ЭТОМ ПУТИ НЕ СОБИРАЕТСЯ (J-21) — довод у снесённого `partsOfAsset` выше. У поля
 * снова есть автор, но ДРУГОЙ и другого рода: ЦВЕТОВОЙ ПЛАН (фича A). Там человек красит флэт
 * заливкой и кистью, и получившаяся карта уезжает с прогоном; размещение при этом несёт `map_hex`
 * — САМА КАРТИНКА, — а `parts` остаётся необязательными человечьими словами про ту же деталь.
 * Это ровно то, чего не было у снесённой примерки: метку видно, снять её можно, и она едет
 * картинкой, а не выводом из невидимых строк.
 */
export function fabricUses(
  ...
  for (const id of assetIds) {
    ...
    out.push(fabricUseOf(band, id));
  }
  return out;
}

/**
 * ═══ ОДНА ТКАНЬ ПРОГОНА, С НЕОБЯЗАТЕЛЬНОЙ ПРАВКОЙ ПОВЕРХ ══════════════════════════════════════
 *
 * Тот же сборщик, что и был, разложенный на одну строку, — потому что появился ВТОРОЙ автор
 * тканей: цветовой план (фича A) собирает список не из чипов сетки, а из покрашенных цветов, и
 * каждой строке доназывает цвет, слова и МЕТКУ. Второе написание «как выглядит ткань на проводе»
 * разошлось бы с этим при первом же новом поле — ровно так однажды потерялся `kind`.
 *
 * ⚠ `assetId = 0` — ЗАКОННАЯ СТРОКА, А НЕ ОШИБКА. Покрашенный цвет, которому назвали простой цвет
 * или слова и не назвали ткани, — полный ответ на вопрос «из чего эта деталь»; полки за ним нет, и
 * копии с полки браться неоткуда. Всё, что тогда едет, приходит правкой.
 */
export function fabricUseOf(
  band: GetDesignBandResponse,
  assetId: number,
  over?: Partial<common_DesignFabricUse>,
): common_DesignFabricUse {
  const a = assetId > 0 ? assetById(band).get(assetId) : undefined;
  return {
    // ПУСТАЯ СТРОКА — ЭТО «ЭТА ТКАНЬ НЕ ПРИВЯЗАНА К ПОКРАШЕННОМУ ЦВЕТУ».
    //
    // ⚠ ЗАПОЛНИТЬ ЕЁ МОЖЕТ РОВНО ОДИН ПИСАТЕЛЬ — `colour-plan/model.ts`, и только тем hex, который
    // ЕСТЬ на палитре карты, УЕЗЖАЮЩЕЙ С ЭТИМ ЖЕ ПРОГОНОМ. Сетка текстур ниже по-прежнему не имеет
    // права её трогать: она не знает ни одной карты. ⚠ И НЕ СТАВИТЬ СЮДА `colourHex` — цвет ткани
    // и цвет МЕТКИ на карте разные величины, а сервер, увидев непустой `map_hex`, объявит модели,
    // что деталь размечена, и напечатает в платный промпт ссылку на картинку, которой в прогоне
    // нет (ревью `5dbb3b5`, находка 1 — ровно эта ложь).
    mapHex: '',
    assetId,
    name: a ? assetLabel(a) : '',
    mediaId: a?.mediaId ?? 0,
    ...
    words: (a?.note ?? '').trim(),
    // ⚠ ПУСТАЯ СТРОКА — УТВЕРЖДЕНИЕ, А НЕ ПРОПУСК (J-21). Экран меток на флэтах снесён, и оттуда
    // части изделия больше не выводятся. ЧТО ИЗМЕНИЛОСЬ С ФИЧЕЙ A: у поля снова есть автор — но
    // другой и другого рода. Это ЧЕЛОВЕЧЬИ СЛОВА про деталь («cuffs and collar»), которые пишут
...
/**
 * ⚠ `fabricOfColorway` СНЕСЕНА ЗДЕСЬ, И ЭТО НЕ УБОРКА НЕИСПОЛЬЗУЕМОГО, А ЗАКРЫТИЕ ЛОЖНОЙ ДВЕРИ.
 *
 * Она отвечала «какую ткань НАЗНАЧИЛИ этому колорвею», читая `design_asset.colorway_id`. Круг 16
 * снял её ЕДИНСТВЕННОГО писателя (`SetDesignAssetColorway` и чипы «worn by»), и с тех пор функция
 * читала колонку, которую ни один экран не заполняет: любой вызов возвращал `undefined` — не
 * ошибкой, а тихим «ткани нет». Последний вызывающий (ряд чипов `ColorwayPicker`) ушёл волной 2
 * круга r3, и осталась готовая с виду дверь, за которой пустая комната.
 *
 * И ВОССТАНОВИТЬ ЕЁ БЫЛО БЫ НЕЛЬЗЯ, ДАЖЕ ЗАВЕДЯ ПИСАТЕЛЯ ЗАНОВО. Ссылка выражает ОДНУ ткань на
 * колорвей — это серверный инвариант (`SetDesignAssetColorway` в одной транзакции снимает колорвей
 * со всех прочих ассетов карточки), а рендеру нужно N тканей: сетка TEXTURE & COLOUR берёт их
 * набором. Структура, которая держит N, уже есть, и это РЕЦЕПТ ПРОШЛОГО ПРОГОНА — им и засевается
 * подача (`render/drafts.ts`, `ofColorway`).
 */
