...
export function CalloutRail({
   ...
   *
   * НЕОБЯЗАТЕЛЕН С ВОЛНЫ 25.09 (D-12): мудборд свой пустой текст снял целиком — пустая раскрытая
   * панель там это одна шапка со счётчиком `0`, а доска сама говорит, как ставится указание. Не
   * передан — на месте списка не рисуется НИЧЕГО (а не пустой абзац, который держал бы отступ).
   */
  emptyLabel?: string;
}) {
  if (rows.length === 0) {
