...
export type ParseState =
  ...
  | {
      ...
      // ЧТО ЧЕРТЁЖ ВООБЩЕ СОДЕРЖИТ — имена верхнеуровневых вставок, встреченных разбором, в
      // написании файла (находка 1 третьего адверсарного ревью). Это НЕ то же, что имена блоков в
      // `pieces`: между блоком и деталью лежит геометрия (сшивка цепочек, порог площади, отбор
      // внешнего контура, sanitizeLoop), и она законно возвращает ноль контуров по блоку, который в
      // файле есть. Раскладке нужен `pieces` — ей нечего класть без контура. А вопрос «детали
      // больше нет в чертеже» обязан задаваться ЭТОМУ набору: иначе неудавшаяся геометрия читается
      // как исчезновение и деталь предлагается удалить вместе со строками рецепта и замерами.
      blockNames: string[];
