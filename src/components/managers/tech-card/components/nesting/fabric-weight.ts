...
// вместе с кромкой, один раз. Процент раскроя слота покрывает межлекальные выпады, концевые
// потери настила, стыки и обход пороков — кромку НЕТ: заложить её туда значит посчитать дважды.
export function weightBasisNote(b: FabricWeightBasis): string {
  return `килограммы посчитаны через длину: метры × полная ширина рулона ${b.fullWidthCm} см × плотность ${b.gsm} г/м² ÷ 100000. Ширина — полная, кромка входит: её покупают, и она весит — и входит она сюда РОВНО ОДИН РАЗ: netto-длина получена делением площади на раскройную ширину (рулон − 2×кромка), кромка учтена этим делением, и процент раскроя слота её НЕ добавляет (он покрывает межлекальные выпады, концы настила, стыки и пороки). Плотность — всегда артикула; ширина — строки BOM, когда та её переопределяет, иначе артикула (при пине колорвея на другой артикул обе — с него). Ошибка любой из них входит в вес линейно и не видна ни в одном числе`;
}
