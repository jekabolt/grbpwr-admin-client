...
export function PieceDxfPreview({
  ...
  const found = useMemo(() => {
    ...
    for (const r of refs) {
      ...
      // Сколько раз деталь лежит в ОДНОМ листе, а не сумма по листам: два листа одной ткани и
      // одного размера — это обычно две РЕВИЗИИ одного чертежа, и сложенные они прочитались бы
      // как «деталь идёт по две на изделие». Тот же максимум-по-файлам, что считает диалог
      // сопоставления, когда выводит pieces_per_garment.
            ...
            <Text size='nano' variant='label' component='span' className='px-1 text-center'>
              {!activePieceKey
                ...
                : refs.length === 0
                  ...
                  : // Связь есть, но её ткань ни к чему живому не ведёт (строку BOM удалили или
                    // переклассифицировали). Сказать «блока нет в файлах» тут значило бы обвинить
                    // чертёж в том, чего он не делал.
                    refs.every((r) => !r.scopeKey)
                    ? `у связи с «${refs[0].block}» потеряна ткань — перепривяжите лист на панели выкроек`
