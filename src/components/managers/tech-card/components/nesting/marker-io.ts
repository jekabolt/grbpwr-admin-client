...
//
// ПРЕФИКС `PR<runId>_` У РАСКРОЙНОГО ЭКСПОРТА (Ф4.7, §10). Имя собрано из СЕЗОНА, СТИЛЯ, размера,
// слота и имени маркера — то есть из карточки; про прогон в нём нет ни слова. Два прогона одной
// модели дают две РАЗНЫЕ раскладки с одинаковыми частями имени, и в папке «Загрузки» второй файл
// лёг бы на первый (или получил бы «(1)» от браузера, что ещё хуже: который из них чей — на диске
// не написано). Префикс, а не хвостовая часть: имена сортируются по алфавиту, и прогонные файлы
// обязаны собираться вместе, а не рассыпаться между карточными.
//
// runId 0 / не передан ⇒ префикса нет ВОВСЕ. Карточный экспорт обязан остаться байт в байт тем же
// (§10: «Файл из карточки шапки прогона не несёт» — это регрессионный критерий, а не пожелание),
// и это же делает параметр безопасным для всех сегодняшних вызовов.
...
// ── ЧЕРНОВИК (0299) ─────────────────────────────────────────────────────────────────────
//
// Раскладка, у которой легла ЧАСТЬ деталей: поиск упёрся в бюджет раньше, чем уложил всё. Она
// хранится, чтобы оплаченный прогон не пропадал вместе с вкладкой, и ровно поэтому её длина —
// НЕ ДЛИНА ИЗДЕЛИЯ: недостающие детали ещё займут место, которого в этом числе нет.
//
// Признак ВЫВОДИТ СЕРВЕР из пары placed_count/total_count (колонка не копируется с запроса), так
// что читать здесь нечего, кроме поля. Локальные ветки ниже — не вторая политика: сервер называет
// причину сам (scalar_apply_refusal), и его слово идёт первым; наши срабатывают там, где он смолчал
// — на устаревшем бэкенде и на оптимистичном объекте, ещё не съездившем на сервер.
export function isDraftMarker(s: common_TechCardMarkerSummary | undefined): boolean {
  return s?.isDraft === true;
}

// Отказ выдать ЛЮБУЮ норму черновиком, словами и с лекарством. Отдельная функция, а не строка
// внутри scalarNormRefusal: тот же текст обязан появляться и там, где решается ПЕР-РАЗМЕРНОЕ
// применение, — у черновика нет ни скалярной нормы, ни размерной, и причина у обеих одна.
...
// оно печатается, уже пять (список раскладок, полоса костинга, строка рецепта, селектор диалога,
...
export function scalarNormRefusal(s: common_TechCardMarkerSummary): string {
  ...
  // ЧЕРНОВИК ОТКАЗЫВАЕТ РАНЬШЕ СОСТАВА. Состав у него читается прекрасно (это то, что раскладку
  // ПРОСИЛИ разложить), и без этой ветки однородный черновик проходил бы сюда без единого
  // возражения, а `consumptionCm` ниже поделил бы его короткую длину на число изделий — то есть
  // выдал бы заниженную норму с безупречным провенансом.
...
// ── ПЕР-РАЗМЕРНЫЙ РАСХОД (Ф2.4) ─────────────────────────────────────────────────────────
//
// Отказ выше не отменён и не отменяется: у смешанной раскладки НЕТ одного расхода на изделие, и
// consumptionCm по-прежнему возвращает на ней null. Ф2.4 добавила ДРУГОЙ ответ — не скаляр, а по
// одному числу на КАЖДЫЙ размер состава:
//
//     расход(s) = a_s · L / Σ_j (q_j · a_j),   a = площадь одного изделия размера, L = длина настила
//
// то есть измеренная длина, поделённая между размерами ПРОПОРЦИОНАЛЬНО занимаемой площади, а не
// поровну. Считает это сервер (entity.MarkerPerSizeConsumption) и кладёт результат на КАЖДУЮ
// строку состава сводки, вместе с площадью a_s, из которой он выведен.
//
// ЭТИХ ДВУХ ПОЛЕЙ НЕТ В ГЕНЕРИРОВАННОМ ТИПЕ. Контракт (grbpwr-proto c227d1e) их несёт и бэкенд их
// шлёт, а `src/api/proto-http` в этой ветке отстал: `make proto` в коммите пина зеркала не
// доехал. Генерированное трогать нельзя, поэтому чтение идёт через СТРУКТУРНОЕ расширение —
// ровно тот же приём, которым marker-rebuild.ts читает условия съёмки (Ф3ConditionFields). Когда
// клиент перегенерируют, расширение станет тавтологией и просто исчезнет; до тех пор оно
// единственное, что отличает «поля нет на проводе» от «поле не объявлено в бандле».
type Ф24CompositionFields = {
  consumptionPerUnitCm?: googletype_Decimal;
  areaPerGarmentCm2?: googletype_Decimal;
};

// Одна строка состава С ПРОИЗВОДНОЙ ПОЛОВИНОЙ. null у обоих чисел — «не сказано», и это ответ:
// раскладка, снятая между Ф2.1 и Ф2.4, площадей по размерам не записывала вовсе.
export type MarkerSizeNorm = {
  sizeId: number;
  quantity: number;
  /** см ткани на ОДНО изделие ЭТОГО размера. null = раскладка такого ответа не даёт. */
  consumptionCm: number | null;
  /** a_s, см² — площадь одного изделия этого размера, БАЗИС числа выше. */
  areaCm2: number | null;
};

// Шаг округления, с которым сервер публикует пер-размерный расход (markerCompositionToPb: Round(2)).
// Точная сходимость Σ q·расход = L ломается ровно на нём и ровно на столько: каждое слагаемое
// сдвинуто не больше чем на пол-шага, поэтому вся сумма — не больше чем на 0.005 × изделий.
const PER_SIZE_ROUND_CM = 0.005;

// Состав С ПЕР-РАЗМЕРНЫМИ ЧИСЛАМИ. Порядок и фильтрация — те же, что у compositionOf (одна
// канонизация на файл), производная половина дочитывается с проводной строки того же размера.
export function sizeNormsOf(s: common_TechCardMarkerSummary): MarkerSizeNorm[] {
  const comp = compositionOf(s);
  if (comp.length === 0) return [];
  const wire = new Map<number, Ф24CompositionFields>();
  for (const c of (s.composition ?? []) as Array<{ sizeId?: number } & Ф24CompositionFields>) {
    const id = Number(c.sizeId ?? 0);
    if (id > 0) wire.set(id, c);
  }
  return comp.map((c) => {
    const w = wire.get(c.sizeId);
    const cons = decNum(w?.consumptionPerUnitCm);
    const area = decNum(w?.areaPerGarmentCm2);
    return {
      sizeId: c.sizeId,
      quantity: c.quantity,
      consumptionCm: cons > 0 ? cons : null,
      areaCm2: area > 0 ? area : null,
    };
  });
}

// ВСЕ СТРОКИ ИЛИ НИ ОДНОЙ — и это не осторожность, а то же правило, что у сервера
// (MarkerPerSizeConsumptionComplete). Размеры применяются в ОДИН рецепт разом, и дырка в середине
// положила бы измеренную норму на три размера и ноль на четвёртый.
//
// СХОДИМОСТЬ ПРОВЕРЯЕТСЯ ЗДЕСЬ, а не «где-нибудь в UI». Тождество Σ_s q_s · расход(s) = L
// выполняется ПО ПОСТРОЕНИЮ (та же a_s стоит и в числителе, и внутри знаменателя), поэтому
// нарушить его может только одно: строки сводки описывают РАЗНЫЕ раскладки — состав от одной,
// длина от другой. Такое число применять нельзя, и распознать это больше негде.
export function perSizeComplete(s: common_TechCardMarkerSummary): boolean {
  // ЧЕРНОВИК НЕ НОРМИРУЕТ НИ ОДНОГО СВОЕГО РАЗМЕРА, и проверяется это ПЕРВЫМ — до всякой
  // арифметики. Скалярную дверь сервер закрыл сам (consumption_per_unit_cm не приходит, причина
  // лежит в scalar_apply_refusal), а пер-размерная закрывалась у него лишь тем, что вместе с
  // расходом он не публикует и площади. Клиенту этого мало: длину настила (used_length_cm) он
  // получает и у черновика, а площади умеет считать САМ по сегодняшним выкройкам — то есть все
  // слагаемые пер-размерной нормы у него на руках, и короткая длина черновика превратилась бы в
  // измеренную норму с провенансом 'marker'. Сервер поймать это не может: применение по размерам
  // приходит к нему готовыми числами, а norm_marker_id при нескольких источниках отправляется
  // нулём намеренно (см. marker-apply), так что сверять ему не с чем.
  if (isDraftMarker(s)) return false;
  const rows = sizeNormsOf(s);
  if (rows.length === 0) return false;
  // Однородная раскладка пер-размерных площадей НЕ ТРЕБУЕТ: один размер — значит весь настил
  // принадлежит ему, и ответ L/q это ровно то, что скалярный путь и выдавал всегда. Именно это
  // оставляет применимыми по размерам все раскладки, снятые до Ф2.4, и легаси-пары (size_id, sets):
  // площади — цена СМЕШИВАНИЯ размеров, а не цена наличия нормы.
  if (rows.length === 1) return consumptionCm(s) != null;
  const used = decNum(s.usedLengthCm);
  if (!(used > 0)) return false;
  let units = 0;
  let sum = 0;
  for (const r of rows) {
    if (r.consumptionCm == null || r.quantity < 1) return false;
    units += r.quantity;
    sum += r.quantity * r.consumptionCm;
  }
  return Math.abs(sum - used) <= PER_SIZE_ROUND_CM * units + 1e-6 * Math.max(1, used);
}

// Расход на ОДНО изделие ЗАДАННОГО размера. null = раскладка про этот размер ничего не говорит.
//
...
export function consumptionForSize(s: common_TechCardMarkerSummary, sizeId: number): number | null {
  ...
  if (!row) return null;
  if (rows.length === 1) return consumptionCm(s);
  if (!perSizeComplete(s)) return null;
  return row.consumptionCm;
}
...
export function markerAreaSplitCm2(m: common_TechCardMarkerSummary): {
  ...
  // КПД ВЫШЕ 100% — ОТКАЗ, А НЕ ЗАЖИМ, и это разбор целиком, а не одно слагаемое. Детали не лежат
  // плотнее собственной площади: КПД > 1 означает, что записанные КПД, ширина и длина описывают
  // РАЗНЫЕ вещи (пересняли на другой ширине, площадь посчитана по другому набору). Зажать одни
  // выпады в ноль мало — площадь деталей осталась бы больше самого полотна, строки дали бы 110%
  // при итоге «100%», и таблица, вся ценность которой в сходимости, соврала бы дважды. Молчание
  // здесь честнее: вызывающий покажет прежнюю процентную подпись, где та же беда уже зажата.
  if (!(eff > 0) || eff > 1 || !(w > 0) || !(l > 0)) return null;
  const selvedge = decNum(m.selvedgeCm);
  const piece = eff * w * l;
  return {
    pieceCm2: piece,
    // Неотрицательно ПО ПОСТРОЕНИЮ (eff ≤ 1 проверен выше) — отдельного зажима не нужно.
    interPieceCm2: w * l - piece,
...
// сих пор; живёт, пока жива карточка. Ненулевой — РАСКРОЙНАЯ: снята под один прогон, принадлежит
// ему и умирает вместе с ним по FK CASCADE (0282).
//
// КАРТОЧНЫЕ ПЕРЕЧИСЛЕНИЯ ПОКАЗЫВАЮТ ТОЛЬКО ПЕРВЫХ, и это не косметика списка. Однодневка,
// просочившаяся в полосу расхода на костинге или в предложение «применить расход в рецепт», даёт
// число, которое ПЕРЕЖИВЁТ свой прогон — но только внутри рецепта, куда его скопировали: связи
// рецепта с маркером нет (consumption_source='marker' — строковая метка, не FK), поэтому смерть
// прогона провенанс не отзовёт и даже не пометит. Прогонная раскладка снята на количествах и
// условиях ОДНОГО заказа; нормой она не может быть даже физически (CHECK chk_tcm_run_not_norm).
//
// ФИЛЬТР ОБЪЯВЛЕН ЗДЕСЬ ОДИН РАЗ. Сервер фильтрует СВОЙ список (`markers.go`: `run_id IS NULL`), но
// клиентские перечисления кормятся ОДНИМ полем `techCard.markers` и отдельного List-RPC не имеют —
// значит правило нужно и здесь, и ровно в одном экземпляре: четыре копии условия `=== 0`
// разъезжаются, и разъедется первой та, про которую забудут.
//
// UNDEFINED ЧИТАЕТСЯ КАК «КАРТОЧНЫЙ» — ЯВНО, ОТДЕЛЬНОЙ ВЕТКОЙ, И ВОТ ПОЧЕМУ. Поле может не приехать
// по двум причинам, и обе означают одно и то же: (а) proto3 не шлёт нулевые скаляры, то есть у
// КАЖДОГО карточного маркера его на проводе нет вовсе; (б) старый бэкенд, который про поле ещё не
// знает, не шлёт его ни у кого. Генерированные типы объявляют каждое поле `| undefined`, так что
// отличить эти случаи от «неизвестно» нечем — да и не нужно: «неизвестно» здесь не бывает, потому
// что раскройный маркер по построению несёт НЕНУЛЕВОЕ значение, а ненулевое proto3 шлёт всегда.
// Обратное умолчание («не сказано ⇒ спрячем») было бы катастрофой ровно в одном сценарии, зато в
// самом вероятном: устаревший бандл против свежего сервера — и весь список раскладок карточки
// пустеет молча, вместе с костингом и рецептом.
export function cardMarkers(
  markers: common_TechCardMarkerSummary[] | undefined,
): common_TechCardMarkerSummary[] {
  return (markers ?? []).filter((m) => {
    // поле не пришло вовсе — карточный (см. простыню выше), а не «неизвестно»
    if (m.productionRunId == null) return true;
    return Number(m.productionRunId) === 0;
  });
}
...
export function markersForLine(
  ...
  if (!lineKey) return [];
  return cardMarkers(markers).filter((m) => (m.bomLineKey ?? '') === lineKey);
}
...
// Прогонные однодневки отсеиваются ПЕРВЫМИ и на ОБЕИХ ветках, включая «колорвей не выбран»: без
// колорвея фильтр по принадлежности выключается, а по владельцу — нет. Владелец и колорвей —
// независимые вопросы («чей это маркер» и «какое полотно он мерил»), и ослабление одного не
// освобождает от другого.
export function markersOfColorway(
  markers: common_TechCardMarkerSummary[] | undefined,
  colorwayId: number,
): common_TechCardMarkerSummary[] {
  const card = cardMarkers(markers);
  if (!colorwayId) return card;
  return card.filter((m) => {
...
//
// СМЕШАННАЯ РАСКЛАДКА ТЕПЕРЬ СЮДА ПОПАДАЕТ — но только та, что несёт ПЕР-РАЗМЕРНЫЕ ЧИСЛА.
//
// Здесь стояло `if (comp.length !== 1) continue` с объяснением: смешанная не является нормой ни
// одного из своих размеров, её длина общая на весь настил, а поделить её по размерам нечем «до
// Ф2.4». Ф2.4 наступила, и причина истекла — но ровно для тех раскладок, у которых деление
// действительно есть. Смешанная раскладка с полным пер-размерным составом нормирует КАЖДЫЙ свой
// размер отдельно: расход(s) = a_s·L/Σ(q·a) — это НЕ среднее, это измеренная длина, поделённая по
// занимаемой площади, и по построению Σ q_s·расход(s) = L.
//
// Смешанная БЕЗ этих чисел (снятая между Ф2.1 и Ф2.4 — площади по размерам тогда не записывались)
// по-прежнему НЕ попадает, и это тот же отказ, что и был: подставить ей среднее значило бы вернуть
// молча ровно тот перекос, ради устранения которого заводился состав. Причину называет
// scalarNormRefusal — сервер с Ф2.4 договаривает в нём и лекарство («пересохраните раскладку»).
//
// СКАЛЯРНЫЙ ОТКАЗ ЭТИМ НЕ ТРОНУТ. consumptionCm на смешанной по-прежнему null: «применить по
// размерам» и «записать одно число как ТОТ САМЫЙ расход» — разные действия, и второго у смешанной
// раскладки нет по-прежнему.
export function latestPerSize(
  ...
  for (const m of markers) {
    ...
    // Один предикат на однородную и смешанную: он же и есть «раскладка отвечает по размерам».
    if (!perSizeComplete(m)) continue;
    for (const c of comp) {
      const prev = bySize.get(c.sizeId);
      // ТОТ ЖЕ компаратор (норма → свой колорвей → свежесть), и второго правила тут не заводится:
      // назначенная смешанная норма обязана бить непомеченную однородную по той же причине, по
      // которой она бьёт её везде — назначение это высказывание человека «мерить по этой».
      if (!prev || better(m, prev) < 0) bySize.set(c.sizeId, m);
    }
  }
  return bySize;
}
