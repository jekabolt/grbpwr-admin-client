...
/**
 * ВНЕШНИЙ CSS-бокс плитки по умолчанию — СОБСТВЕННЫЙ размер этой плитки (`size-14` ниже, то есть
 * 56×56), а не TILE=48 из раскладки: дефолт обязан описывать ту плитку, которую рисует сам этот
 * компонент. Числом 48 он врал ровно тем вызывателям, которые `pxBox` не передают (лоток деталей):
 * `u` считался под бокс рисования 40×30 при реальном 48×38, и шаг решётки выходил на четверть
 * грубее, чем у плиток полотна. Словарь различает роли ЧАСТОТОЙ (contrast 3.5 против main 6), так
 * что «одна ткань, две плотности» на одном экране ломает его выучиваемость.
 *
 * Кто рисует плитку ДРУГОГО размера — обязан передать свой `pxBox` явно (так делает полотно схемы).
 ...
 */
export const TILE_BOX = { w: 56, h: 56 };
