...
/** НАПРАВЛЕНИЕ СТРЕЛКОЙ И СЛОВОМ — «↑ lengthwise». Стрелка ведёт, слово подтверждает.
 *
 *  ЗАЧЕМ ПАРОЙ, А НЕ ОДНОЙ СТРЕЛКОЙ: глиф читается мгновенно, но словарь у него закрытый, и текст
 *  из чужой записи стрелки не получает вовсе (`grainlineArrow` молчит там, где не уверена). Слово
 *  рядом делает такую строку по-прежнему читаемой, а не пустой.
 *
 *  ЗАЧЕМ СОСТАВИТЕЛЬ, А НЕ СКЛЕЙКА НА МЕСТЕ. Раскройный лист печатал ОДНО СЛОВО, без стрелки, —
 *  а `lengthwise` и `crosswise`, стоящие в столбик, различаются одной буквой в середине; глиф ради
 *  этого и заведён. Собранная же на месте пара «стрелка + слово» существовала в одном экземпляре и
 *  при неопознанном тексте давала ведущий пробел, потому что пустая стрелка всё равно склеивалась
 *  через него. Оба вопроса решаются один раз здесь. */
export function grainlineText(grainline?: string): string {
  const g = (grainline ?? '').trim();
  if (!g) return '';
  const arrow = grainlineArrow(g);
  return arrow ? `${arrow} ${g}` : g;
}
