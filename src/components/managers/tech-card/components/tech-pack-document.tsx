  ...
  const stepEquipment = (o: common_TechCardOperation) => {
    ...
    const head = machineStep
      ...
      : [
          ...
          // В САМОМ ТИПЕ, — и до этой правки клетка у такого шага оставалась ПУСТОЙ, а номер по
          // ISO 4915 держался в имени. Теперь имя зовёт композитор (как везде), а номер стоит
          // здесь: ровно та же пара «глагол в заголовке, подпись типа рядом», что панель релизов
          // печатает для таких шагов с самого начала. Фолбэк ПОСЛЕДНИЙ и только для легаси: у
          // современного ВТО-шага без под-глагола подпись типа пересказала бы его худшими
          // словами — перечислением в скобках, из которого оператору предлагалось бы выбрать.
          .join(' · ') ||
          (o.operationType && LEGACY_OPERATION_TYPES.has(o.operationType)
