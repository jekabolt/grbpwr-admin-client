                ...
                {printGroups.map((g) => (
                  <Fragment key={g.key ? `u:${g.key}` : 'tail'}>
                    ...
                    {g.operations.map(({ op: o, index: i }) => {
                      ...
                      //
                      // ЛИСТ ЧИТАЕТ ПРИПУСК СВОЕЙ ЛЕСТНИЦЕЙ, А НЕ ОБЩИМ `seamAllowanceText`, и это
                      // ОТМЕЧЕННЫЙ ДОЛГ, а не решение в пользу копии. Расходятся они двумя вещами.
                      // (1) У клетки есть КОЛОНКА, то есть существительное уже написано в шапке
                      // таблицы; общий составитель ставит слово «allowance» внутрь строки, потому
                      // что зовут его бегущие строки через ·, где называть факт нечем. (2) У листа
                      // НЕТ ЦЕХОВОЙ СТУПЕНИ: настройки цеха он не читает вовсе, и карточка без
                      // своего стандарта печатается тут «card standard not set» ровно тогда, когда
                      // редактор и карта примерки уже показывают цеховое число. Это НАСТОЯЩЕЕ
                      // расхождение бумаги с экраном, и закрыть его дешевле всего нельзя: чтение
                      // цеха — сетевой запрос, а документ печатается по гейту готовности
                      // (`onDataStatus`), так что ступень пришлось бы заводить ВМЕСТЕ с её строкой
                      // в этом гейте — иначе PDF иногда уезжал бы в цех с «not set» просто потому,
                      // что ответ не успел. Отдельной правкой, со своей цитатой в пробе.
