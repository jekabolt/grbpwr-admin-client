...
const wireStepFacts = (o: common_TechCardOperation): StepFacts => ({
  // РАБОТА ЕДЕТ В ФАКТЫ ИЗ ТОГО ЖЕ ДОКУМЕНТА, ЧТО И ВСЁ ОСТАЛЬНОЕ, — и это ответ на вопрос про
  // РЕЛИЗНЫЕ СНАПШОТЫ. `o` здесь — строка шага ЛИБО живой карточки, ЛИБО замороженного протоблоба
  // (`print-page` подставляет снапшот на место карты одной строкой `snapshot ?? techCard`), и
  // читается она одинаково. Снапшот, подписанный ДО оси работ, поля `work` не несёт — значит у
  // шага его нет, — и лестница `cutLengthNoun` берёт первую ступень, сегодняшнюю деривацию: бумага
  // говорит ровно то, что говорила в день подписи. Снапшот, подписанный ПОСЛЕ, работу несёт, и
  // тогда бумага обязана звать шаг ею же — иначе каждая будущая ревизия печаталась бы не тем
  // словом, каким её подписывали, то есть дефект R8 вернулся бы через архив.
  //
  // ОТЛИЧАТЬ «РАБОТЫ НЕТ У ШАГА» ОТ «РАБОТЫ НЕТ У ИСТОЧНИКА» ЗДЕСЬ НЕЧЕМ, И НЕ НУЖНО: отсутствие
  // поля в подписанном блобе — это и есть утверждение «у этого шага работа не была названа». Оно
  // не пробел в данных, оно и есть данные. Признак `printScope.revision.source` («live» / «release»)
  // рядом существует и НАМЕРЕННО не спрашивается: он сказал бы, замороженный ли это документ, но
  // не то, предшествует ли он оси, — а глушить работу по нему значило бы отобрать имя у всех
  // будущих релизов ради прошлых. Запрещено здесь ровно одно: ДОСОЧИНЯТЬ работу источнику, который
  // её не хранит (вывести токен из машинки, подсмотреть в живой карточке, угадать по каталогу).
  // Такая «услужливость» заставила бы подписанную бумагу назваться именем, которого в подписанном
  // документе нет.
  work: o.work,
...
const operationTypeText = (
  ...
  // ТИП НЕ НАЗВАН — «не сказано», а не выдуманное слово. Композитор на такой строке дошёл бы до
  // фолбэка и напечатал «step»: имя, которого никто не выбирал, и хуже прочерка ровно тем, что
  // выглядит ответом.
  if (!v || v === 'TECH_CARD_OPERATION_TYPE_UNKNOWN') return '—';
  // ВСЁ ОСТАЛЬНОЕ — ОДНИМ ХВОСТОМ ЧЕРЕЗ КОМПОЗИТОР: девять легаси-членов, ручная работа и «прочее».
  //
  // У РУЧНОЙ РАБОТЫ И «ПРОЧЕГО» ЗДЕСЬ БЫЛА СВОЯ ВЕТКА, И ОНА СТАЛА ЛИШНЕЙ РОВНО В ТОТ МОМЕНТ, когда
  // хвост перестал печатать подпись типа и начал звать композитор с заметкой. Две записи одного
  // поведения — это ещё одна копия, только внутри одной функции; убрана. (Замечено гейтом:
  // мутация, отключавшая ту ветку, позеленела — значит ветка уже ничего не решала.)
  //
  // ЗАМЕТКА — ПОСЛЕДНЯЯ СТУПЕНЬ ИМЕНИ, и до неё композитор доходит только там, где глагола нет:
  // у ручной работы («hand») и у «прочего» («step») он сам себя признаёт невнятным. У легаси-членов
  // глагол есть, и заметка им не понадобится — передаётся она всё равно, потому что решает это
  // композитор, а не эта функция.
  //
  // И ДЕВЯТЬ ЛЕГАСИ-ЧЛЕНОВ ТОЖЕ ЗОВУТ КОМПОЗИТОР. Здесь они оставались на полной подписи
  // («join — lockstitch 301») ради номера по ISO 4915, которого в глаголе нет, — и цена была та,
  // что бумага звала шаг иначе, чем панель релизов рядом, а это ЕДИНСТВЕННЫЕ два места, где такой
  // шаг вообще виден.
  //
  // НОМЕР ПРИ ЭТОМ НЕ ПОТЕРЯН, А ПЕРЕЕХАЛ В СВОЮ КОЛОНКУ. Панель релизов давно устроена так:
  // заголовок говорит ГЛАГОЛ, а строка рядом — «на чём это шло», и у записи до 0306 отвечает на
  // второе подпись типа. У листа те же две клетки, и «machine / mode» теперь знает тот же фолбэк
  // (см. `head` ниже). То есть номер стежка стоит там же, где он стоит у современного шага, —
  // в колонке, которую оператор ради него и читает, — а имя шага совпало с экраном.
  return operationHeading({
    operationType: v,
    work: undefined,
    workCatalog: undefined,
...
export function TechPackDocument({
  ...
  // ЦЕХОВОЙ СТАНДАРТ ПРИПУСКА — ПОСЛЕДНЯЯ СТУПЕНЬ ЛЕСТНИЦЫ, и до этой правки лист её не читал вовсе:
  // карточка без своего стандарта печаталась «card standard not set» ровно тогда, когда редактор и
  // карта примерки уже показывали цеховое число. Раскройщик читает ЛИСТ, а не экран, и «стандарт не
  // задан» на бумаге при заданном цеховом стандарте — это указание резать не так.
  //
  // ЗАПРОС ОБЪЯВЛЕН В ГЕЙТЕ ГОТОВНОСТИ (`onDataStatus` ниже) — ВМЕСТЕ со ступенью, одной правкой.
  // Ступень без своей строки в гейте превратила бы стабильный дефект в ПЛАВАЮЩИЙ: лист печатался бы
  // верно ровно тогда, когда ответ singleton-запроса успел, и уезжал бы в цех со старой ложью,
  // когда не успел. Плавающий хуже стабильного — стабильный виден всегда, плавающий выглядит
  // случайностью и переживает несколько поколений исполнителей.
  const {
    data: workshop,
  ...
  useEffect(() => {
    onDataStatus?.([
      ...
      // ЦЕХОВЫЕ СТАНДАРТЫ — РОВНО ТАКАЯ ЖЕ ЗАВИСИМОСТЬ, КАК СОСЕДИ ВЫШЕ, и заведена она вместе со
      // ступенью припуска, которая её читает. Пока ответ не приехал, лист НЕ ГОТОВ — а не печатает
      // «припуск не назван» и уезжает в цех. Отказ сервера печать не запирает (правило 1 гейта) и
  ...
  const stepEquipment = (o: common_TechCardOperation) => {
    ...
    // КАТАЛОГ — ТОТ ЖЕ, ЧТО НАЗЫВАЕТ ШАГ В ЗАГОЛОВКЕ ЭТОГО ЖЕ ЛИСТА. Второго обращения к сети нет
    // (ключ на приложение один), и второго словаря тоже: колонка «machine / mode» обязана звать
    // разрез тем же словом, каким его зовёт шапка строки над ней.
    const toolFacts = stepToolFactParts(facts, workCatalog).map((p) => ({
      ...p,
      overridden: false,
    }));
    const settings = [
    ...
    const head = machineStep
      ...
      : [
          ...
          .filter(Boolean)
          // ЗАПИСЬ ДО 0306 ГОВОРИТ «НА ЧЁМ» СВОИМ ТИПОМ. Машинки у неё нет отдельным полем — она
          // В САМОМ ТИПЕ, — и до переезда клетка у такого шага оставалась ПУСТОЙ, а номер по
          // ISO 4915 держался в имени шага. Теперь имя зовёт композитор, как везде, а факт о
          // машине стоит здесь.
          //
          // ХВОСТ ПОДПИСИ, А НЕ ПОДПИСЬ: `legacyMachineFact` отрезает глагол, с которого подпись
          // начинается. Печатая её целиком, клетка пересказывала бы имя шага из соседней колонки —
          // у семи членов повтор прятался за приклеенным к нему фактом, а у петли и пришивания
          // пуговицы подпись РАВНА глаголу, и бумага печатала одно слово дважды подряд. С хвостом
          // легаси-клетка стала той же формы, что современная («zigzag 304» / «lockstitch 301»), —
          // ровно то, ради чего переезд и делался, — а два члена без факта честно пусты.
          //
          // Фолбэк ПОСЛЕДНИЙ и только для легаси: у современного ВТО-шага без под-глагола подпись
          // типа пересказала бы его худшими словами — перечислением в скобках, из которого
          // оператору предлагалось бы выбрать.
          .join(' · ') || legacyMachineFact(o.operationType);
    return {
      head,
      settings,
      density,
                ...
                {printGroups.map((g) => (
                  <Fragment key={g.key ? `u:${g.key}` : 'tail'}>
                    ...
                    {g.operations.map(({ op: o, index: i }) => {
                      ...
                      // ПРИПУСК ПЕЧАТАЕТСЯ ВЕЗДЕ, ГДЕ ОН ЕСТЬ ХОТЬ НА ОДНОЙ СТУПЕНИ. Раньше пустая
                      // клетка означала «наследует стандарт карты», но выглядела ровно как «не
                      // задано», и число приходилось помнить из строки над таблицей — стоя у
                      // машины, по листу, вынутому из стопки. Ноль при этом ЛЕГАЛЕН («кроить по
                      // линии») и обязан печататься нулём — отсутствие поля отличается от значения
                      // 0 сравнением с пустой строкой, а не по truthy; правило это живёт теперь в
                      // самой лестнице, одним экземпляром на бумагу и оба экрана.
                      //
                      // ЛЕСТНИЦА — ОБЩАЯ, ТА ЖЕ, ЧТО У РЕДАКТОРА И КАРТЫ ПРИМЕРКИ. Своя копия
                      // стояла здесь и расходилась с экранами двумя способами разом. СЛОВОМ: лист
                      // говорил «(card standard)», экраны — «(card)», то есть один источник
                      // назывался на бумаге не так, как на экране, у которого его сверяют. И —
                      // хуже — ФАКТОМ: у листа НЕ БЫЛО ЦЕХОВОЙ СТУПЕНИ вовсе, и карточка без
                      // своего стандарта печаталась «card standard not set» ровно тогда, когда обе
                      // поверхности уже показывали цеховое число. Раскройщик читает лист, а не
                      // экран: «стандарт не задан» на бумаге при заданном цеховом стандарте — это
                      // указание резать не так.
                      //
                      // ФРАЗА ЦЕЛИКОМ, СО СЛОВОМ «allowance», А НЕ ОДНО ЧИСЛО: колонка называется
                      // «seam» и держит ТРИ чтения (класс шва, припуск, факты шва), так что
                      // существительное шапкой не написано ни у одного из них. Прежнее голое «10
                      // mm» второй строкой под классом шва означало припуск лишь по месту, а
                      // теперь оно ещё и обязано читаться тем же, чем на экране, — иначе сверка
                      // бумаги с картой примерки снова стала бы переводом с языка на язык.
                      const allowance = seamAllowanceReading({
                        own: dec(o.seamAllowanceMm),
                        card: cardAllowance,
                        workshop: shopAllowance,
                        operationType: o.operationType,
                        seamClass: o.seamClass,
                      });
                      // «НИКТО НЕ НАЗВАЛ» ГОВОРИТСЯ ВСЛУХ — но только там, где лестница вообще
                      // применима, и это РАЗНИЦА БУМАГИ С ЭКРАНОМ, а не остаток своей лестницы.
                      // Экрану молчать можно: пустое поле формы видимо не заполнено. Клетка листа
                      // рисуется всегда, и её пустота неотличима от «не напечаталось». А на шаге
                      // без шва (упаковка, чистка) молчит и лист: «allowance 12 mm (card)» там
                      // верно и бессмысленно разом. Вердикт о гейте приезжает ИЗ ЛЕСТНИЦЫ тем же
                      // вызовом — спросить его отдельно значило бы завести копию гейта здесь.
                      const allowanceCell =
                        allowance.text || (allowance.inherits ? 'allowance not stated' : '');
                      ...
                      return (
                        <Fragment key={i}>
                          <tr className='break-inside-avoid'>
                            ...
                            {/* `data-sheet-mode` — РУКОЯТКА ПРОБЫ, как `data-sheet-step` строкой
                                выше. Фраза длины прорези печатается ИМЕННО ЗДЕСЬ, и без имени у
                                клетки проба читала бы её из строки таблицы целиком, где рядом
                                стоят зона, детали и материалы: «слово про петлю» нашлось бы в
                                соседней колонке и позеленело бы впустую. */}
                            <td className={TD} data-sheet-mode={i}>
