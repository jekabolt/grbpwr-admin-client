...
export type PanelPrefs = {
   ...
   * Док свёрнут.
   *
   * ПИСАТЕЛЯ И ЧИТАТЕЛЯ У ПОЛЯ НЕТ, и это итог, а не недоделка: свёрнутость дока оказалась
   * СЕССИОННОЙ. Вход в фулскрин закрывает док всегда (правило Ф3), а дальше им распоряжается
   * `dockStep` — «док открыт ⇔ открыт шаг», состояние жизни экрана, а не предпочтение человека.
   * Поле осталось в разборе намеренно: формат в localStorage переживает версии клиента, и если
   * свёрнутость дока однажды станет предпочтением, читать её придётся из хранилищ, которые уже
   * существуют. Соседний `shelfCollapsed` — не двойник: у полки свёрнутость как раз предпочтение,
   * её пишет `[` и она переживает перезагрузку.
   */
