...
//
// Модуль ЧИСТЫЙ: ни React, ни DOM, ни формы — только данные. Он и есть место, где читатели —
// плитки инлайна (схема, лоток, состав шага), полотно фулскрина, полка — договариваются об одном
// ответе; печатный комплект встанет сюда же, когда начнёт рисовать силуэты (Ф7 закрыта пробой:
// печатный лист силуэтов сегодня не рисует вовсе). Вторая копия любого из этих правил разъедется
// молча: дублированная лицевая полочка получит крест клеевой вместо штриха основной ткани, и
// увидят это только на фабрике.
 ...
 * НА ВЫРОСТ под печатный комплект: когда Ф7 начнёт рисовать силуэты на листе, лист обязан звать
 * ровно эту функцию, а не переизобретать приоритет (сегодня внешних вызывателей у неё нет,
 * снаружи карты её видит только проба). Пустой набор слоёв — это `unbound`, а не «основная»:
