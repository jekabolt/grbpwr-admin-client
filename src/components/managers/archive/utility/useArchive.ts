...
export function useArchiveDetails(id: number | undefined, archiveData?: { heading?: string; tag?: string }) {
    return useQuery({
...
export function useCreateArchive() {
    ...
    return useMutation({
        mutationFn: (archiveData: common_ArchiveInsert) => adminService.AddArchive({ archiveInsert: archiveData }),
        onSuccess: () => {
