  ...
  return (
    <Component
      ...
      className={cn(
        ...
        // ⚠ `items-stretch` НЕ УКРАШЕНИЕ И НЕ ДУБЛЬ УМОЛЧАНИЯ. У `<button>` браузерный стиль
        //    ставит `align-items: flex-start`, а у `<div>` действует стретч — то есть ОДНА И ТА ЖЕ
        //    плитка вела себя по-разному в зависимости от того, кликабельна ли она. У некликабельной
        //    подпись растягивалась на ширину плитки и `truncate` работал; у кликабельной подпись
        //    мерилась ПО СОДЕРЖИМОМУ, `truncate` не имел от чего отсчитывать ширину и молчал, и
        //    длинное имя раздувало плитку изнутри: замерено 576px подписи внутри плитки в 207px,
        //    страница PATTERNS уезжала вбок на 134px при 1280 (претензия владельца D-4).
        //    Чиним в ПРИМИТИВЕ, а не приписывая `w-full` каждой подписи по месту: точек, где текст
        //    попадает внутрь плитки, три (`name`, `sub`, `children`), и заплатка на две из трёх
        //    оставила бы дефект живым ровно для того, что положат сюда завтра.
        'flex h-full w-full min-w-0 flex-col items-stretch bg-bgColor p-1.5 text-left',
