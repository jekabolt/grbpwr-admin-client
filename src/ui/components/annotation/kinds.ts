...
export type KindDef = {
  ...
  /**
   * Показывать чипом в панели. Мимо палитры идут ровно два вида, и оба — виды ХРАНЕНИЯ, а не
   * жеста: `pin` (то же, что записка, только текст читается легендой) и `multi` (та же записка,
   * которой добавили лучей). Палитра называет ЖЕСТЫ, а не строки таблицы.
   */
  inPalette: boolean;
...
/** Виды в порядке показа в панели. Шесть жестов: записка, мерка, скоба, дуга, зона, след. */
