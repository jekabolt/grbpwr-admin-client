...
export type AnnotationSurfaceProps = {
   ...
   *
   * СТАРШИНСТВО ЗАВИСИТ ОТ ТОГО, ЧТО ДЕЛАЕТ РУКА, И ЭТО ТРИ СЛУЧАЯ, А НЕ ОДИН (см. `isolatedKey`
   * в разделе отрисовки — там правило и живёт, здесь только его пересказ):
   *   · ЖЕСТ ИДЁТ (`drawing`) — не изолирует НИЧТО: гасить соседей во время рисования значит
   *     мешать целиться;
   *   · ВИД ВЗВЕДЁН, НО ЖЕСТ НЕ НАЧАТ (`placing`) — считается ТОЛЬКО ЭТОТ ПРОП, а наведение на
   *     кадр не считается вовсе. Лист артефактов держит вид взведённым с открытия (D-18), и рука,
   *     ведущая записку к месту, гасила бы по дороге все соседние указания;
   *   · ВИД НЕ ВЗВЕДЁН — ВНУТРЕННЕЕ НАВЕДЕНИЕ СТАРШЕ ВНЕШНЕГО: мышь на кадре отвечает точнее, чем
   *     мышь над списком, и список не имеет права перебивать её подсветку.
   *
   * ⚠ ЗДЕСЬ СТОЯЛО «ВНУТРЕННЕЕ НАВЕДЕНИЕ СТАРШЕ ВНЕШНЕГО» БЕЗ ОГОВОРОК — правда до круга 20 и
   * ложь после него: средний случай переворачивает старшинство ровно наоборот.
   */
...
/** Порог, разводящий клик и перетаскивание.
 *  ⚠ ЕДИНИЦА РАЗНАЯ У ДВУХ ЧИТАТЕЛЕЙ, И ЭТО НЕ ОПЕЧАТКА, А ФАКТ: путь панорамирования сравнивает
 *  сырые ЭКРАННЫЕ пиксели (`e.clientX - p.startX`), а путь перетаскивания фигуры и ручек —
 *  `dx * size.w`, то есть пиксели НЕУВЕЛИЧЕННОГО кадра. Значит для фигуры мёртвая зона на самом
 *  деле `6 × scale` экранных пикселей: до 36 при `MAX_SCALE`. Раньше здесь стояло просто
 *  «ЭКРАННЫЕ пиксели» — верно ровно для одного из двух читателей. */
...
export function AnnotationSurface({
   ...
   *
   * `placing` говорит «клик по фону ПОСТАВИТ точку». `drawing` говорит «жест УЖЕ ИДЁТ, и кадр
   * принадлежит ему». До круга 20 второго состояния не было вовсе, и на всех поверхностях это
   * сходило с рук ровно потому, что вид взводили НА ОДИН ЖЕСТ: между жестами `tool` был `null`,
   * значит `placing` был ложью, значит фигуры жили. Лист артефактов взвёл вид С ОТКРЫТИЯ (D-18) —
   * и тем же движением, ничего больше не поменяв, погасил на себе указания НАВСЕГДА: хит-пути не
   * рисовались, пин не тащился, плашка была `pointer-events-none`, ручек не появлялось. Владелец
   * сказал это дословно: «сломались колауты в артефактах я не могу их двигать и ничего не могу с
   * ними делать».
   *
   * ПОЧИНКА — НЕ РАЗОРУЖИТЬ ЛИСТ, А ПЕРЕСТАТЬ ПУТАТЬ ГОТОВНОСТЬ С НАМЕРЕНИЕМ. D-18 просит, чтобы
   * вид был взведён сразу; B-8 просит, чтобы поставленное оставалось живым. Противоречия между
   * ними нет — было одно выражение на два вопроса.
   *
   * ⚠ ЭТОГО РАЗДЕЛЕНИЯ ОДНОГО МАЛО, И ПЕРВАЯ РЕДАКЦИЯ ЭТОГО ДОВОДА ЗАНИЗИЛА ЦЕНУ. Здесь стояло
   * «первая точка не встанет ровно на штрих соседней, дальше цена исчезает». Замерено на стенде,
   * и все три части оказались шире: (1) мёртв был не штрих, а ПОЛОСА `HIT_WIDTH` вокруг него —
   * 4px мимо линии тоже мимо, 40px уже живо; (2) у заштрихованной зоны мёртвой была ВСЯ ПЛОЩАДЬ,
   * а не контур; (3) «дальше цена исчезает» неверно для вида, который лист держит по умолчанию:
   * у записки `points: [1, 1]`, её жест кончается ПЕРВОЙ ЖЕ точкой, и `drawing` не бывает истинным
   * НИКОГДА — то есть на этом листе не было ни одной записки, для которой цена бы исчезла.
   * Вдобавок умирала панорама по заштрихованной зоне. Источник у всего этого ОДИН и он замерен
   * мутацией: глушение `pointerdown` на хит-пути (кадр без нажатия не заводит жест, а постановка
   * висит на `pointerup` кадра); снятие глушения `click` там же не меняет НИЧЕГО — у кадра
   * обработчика клика нет вовсе.
   *
   * ПОЭТОМУ `drawing` РЕШАЕТ ТОЛЬКО ТО, ЧТО КАСАЕТСЯ ИДУЩЕГО ЖЕСТА (ручки, плашки, превью), а
   * вопрос «выбрать или поставить» разведён ТЕМ, ЧТО ЗАДЕТО: пин и плашка — ручки выноски, штрих
   * и площадь — картинка. Довод и цена — у `style` хит-пути ниже.
   */
  const drawing = placing && points.length > 0;
  ...
  /**
   * ИЗОЛЯЦИЯ ПО НАВЕДЕНИЮ — ЖЕСТ ЧТЕНИЯ, И ВЗВЕДЁННАЯ РУКА ЕГО НЕ ДЕЛАЕТ (B-8a, круг 21).
   *
   * Наведение на фигуру гасит ВСЕ ОСТАЛЬНЫЕ (`dim` → `invisible`), и до круга 20 на листе
   * артефактов этого не случалось никогда: под взведённым видом хит-пути не рисовались, плашки и
   * пины были `pointer-events-none`, наводиться было не на что. Вернув им жизнь, мы вернули и
   * изоляцию — на лист, который держит вид взведённым С ОТКРЫТИЯ (D-18). То есть рука, ведущая
   * записку к месту, по дороге гасила бы все соседние указания и зажигала обратно: замерено на
   * стенде — курсор на зоне делал пин и обе плашки невидимыми.
   *
   * ПОЭТОМУ ИЗОЛИРУЕТ ТОЛЬКО ЧТЕНИЕ: наведение НА КАДР считается, пока вид не взведён, а
            ...
            {size.w > 0 && !hideCallouts && (
              ...
              >
                ...
                {/* ХИТ-ПУТИ — невидимые толстые копии штрихов: попасть мышью в волосяную линию
                    ...
                    кликов, иначе вторую точку под чужой фигурой не поставить.
                    ⚠ УСЛОВИЕ БЫЛО `!placing`, И ЭТО ДЕФЕКТ B-8a. Взведённый вид — это ещё не жест,
                    и на листе, который взводит вид С ОТКРЫТИЯ (D-18), «взведён» значило «навсегда»:
                    слой не рисовался никогда, и ни одну фигуру нельзя было ни выбрать, ни потащить.
                    Довод выше при этом цел — он про НАЧАТУЮ постановку, а не про готовность к ней. */}
                {editable &&
                  !drawing &&
                  ...
                  callouts.map((c) => {
                    ...
                    return (
                      <path
                        ...
                        /**
                         * ═══ РАЗДЕЛЯЕТ ТО, ЧТО ЗАДЕТО, А НЕ ТО, НАЧАТ ЛИ ЖЕСТ (B-8a, круг 21) ═══
                         *
                         * ШТРИХ И ЗАШТРИХОВАННАЯ ПЛОЩАДЬ — ЭТО САМА КАРТИНКА, а не орган
                         * управления. Под взведённым видом клик по ним обязан ПОСТАВИТЬ точку
                         * ровно так же, как клик по пустому месту: лист держит вид взведённым с
                         * открытия (D-18), поэтому «пока взведено — сюда нельзя» означало бы
                         * «сюда нельзя никогда». Замерено на стенде: мёртв был не штрих, а ПОЛОСА
                         * шириной `HIT_WIDTH` вокруг него (4px мимо линии — тоже мимо), а у
                         * заштрихованной зоны — ВСЯ ЕЁ ПЛОЩАДЬ; и умирала заодно панорама по ней.
                         *
                         * ВЗЯТЬСЯ ЗА УЖЕ СТОЯЩУЮ ВЫНОСКУ ЕСТЬ ЧЕМ И БЕЗ ШТРИХА: пин и плашка —
                         * это её ручки, они нажатие глушат сами (см. `PinMarker`/`Plate`), и
                         * именно туда тянется рука. У безымянной зоны и следа плашки нет вовсе
                         * (`plateWhenEmpty: false`) — их выбирают строкой в списке CALLOUTS рядом,
                         * и это тот же жест, которым их и подписывают.
                         *
                         * ВЫБРАННАЯ ФИГУРА — ИСКЛЮЧЕНИЕ: её штрих остаётся живым, потому что
                         * ИМЕННО ИМ ЕЁ И ТАЩАТ (ветка `startShapeDrag` ниже).
                         *
                         * ⚠ ЦЕНА БЫЛА НАЗВАНА НЕВЕРНО, И ИМЕННО ЭТО ЧИНИТСЯ ЗДЕСЬ. Стояло «пока
                         * фигура выбрана, первая точка новой не встанет на её штрих — а у
                         * выбранной ЗОНЫ и на её площадь; снимается одним Esc ИЛИ КЛИКОМ ПО
                         * ФОНУ». Половина названного выхода не существует: КЛИКА ПО ФОНУ НА ЭТОМ
                         * ЛИСТЕ НЕ БЫВАЕТ. Лист держит вид взведённым С ОТКРЫТИЯ (D-18), а
                         * взведённый вид забирает отпускание кадра себе под постановку — ветка
                         * `placing` в `onFramePointerUp` стоит ВЫШЕ ветки «выбор снят», и до
                         * второй управление не доходит никогда. Выбранная ЗОНА при этом ловит по
                         * всей площади и глушит нажатие (`startShapeDrag`), то есть добрая
                         * половина снимка становилась областью, где не происходит НИЧЕГО: ни
                         * точки, ни снятия, — и ни одна строка на экране не называла Esc.
                         *
                         * ПОЭТОМУ НАЖАТИЕ БЕЗ СДВИГА ПО САМОЙ ВЫБРАННОЙ ФИГУРЕ ЗНАЧИТ «ОТПУСТИ»
                         * (ветка `onClick` ниже, и только под взведённым видом — без него клик по
                         * фону работает сам и переключать выбор незачем).
                         *
                         * ЦЕНА. Первый клик по штриху выбранной фигуры (у зоны — в любую точку её
                         * площади) снимает выбор и больше ничего; снятая фигура тем же кадром
                         * перестаёт хит-теститься (`placing && selected !== c.key → 'none'`), и
                         * второй клик в ту же точку ставит точку как по пустому месту. Мигания
                         * «выбрал — снял — выбрал» одним жестом не выходит: под взведённым видом
                         * выбор приходит СТРОКОЙ CALLOUTS, пином или плашкой, а не штрихом. Esc
                         * делает то же самое и не целясь.
                         *
                         * ⚠ ЗДЕСЬ СТОЯЛО «РОВНО ОДИН ЛИШНИЙ КЛИК», И ЭТО БЫЛО ВТОРОЙ ПОДРЯД
                         * ЗАНИЖЕННОЙ ЦЕНОЙ ОДНОГО И ТОГО ЖЕ ПРАВИЛА. Ветка снятия достаётся ЛЮБОМУ
                         * нажатию, не добравшему порог, — в том числе НЕУДАВШЕЙСЯ ПОПЫТКЕ
                         * ПЕРЕТАЩИТЬ. Подвинуть выноску на 4px теперь не «ничего не произошло», а
                         * «выбор снят»; а поскольку порог у этого пути меряется в пикселях
                         * неувеличенного кадра (см. `CLICK_MOVE_THRESHOLD`), в зуме ×6 мёртвая
                         * зона доходит до 36 экранных пикселей. Механизм от этого верен, цена —
                         * шире одного клика, и написать иначе значит повторить ошибку в третий раз.
                         */
                        style={{
                          pointerEvents:
                        ...
                        onPointerDown={(e) => {
                          ...
                          // после того, как кадр уже принял то же нажатие за постановку. Только
                          // глушило оно и постановку тоже (кадр без `pointerdown` не заводит
                          // жест — см. `onFramePointerDown`), а с ней и панораму. Один жест
                          // разводится теперь ВЫШЕ и раньше: под взведённым видом невыбранный
                          // штрих не хит-тестится вовсе, значит ни выбора, ни клика с него не
                          // придёт, и разводить нечего.
                        }}
                        onClick={(e) => {
                          e.stopPropagation();
                          ...
                          // ПОД ВЗВЕДЁННЫМ ВИДОМ КЛИК ПО ВЫБРАННОЙ ФИГУРЕ ОТПУСКАЕТ ЕЁ — это и
                          // есть тот выход, который комментарий выше обещал «кликом по фону», а
                          ...
                          // И НЕ ПЕРЕКЛЮЧАЕТ, ПОКА ВИД НЕ ВЗВЕДЁН: повторный клик по выбранной
                          // фигуре раньше снимал выбор всегда, то есть хлопал редактором на каждое
                          // второе попадание. Там, где вид не взведён, выбор по-прежнему снимают
                          // клик по фону и Esc — два жеста, которые ничего больше не значат.
                          select(c.key);
                        }}
                      />
                    );
...
function PinMarker({
  ...
  onPress: () => void;
  /**
   * Отсутствует = кружок только читается: на выпущенной карточке и ПОКА ИДЁТ ЖЕСТ постановки.
   * Именно пока идёт, а не пока взведён вид (B-8a): взведённый вид — это готовность, и лист,
   * который держит его с открытия (D-18), иначе не отдавал бы пин руке никогда.
   */
