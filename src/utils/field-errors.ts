...
//
// A FIELD THAT IS NOT ON SCREEN FIRST ASKS TO BE SHOWN, and there are two ways of not being on
// screen, answered by the same event:
//   · the anchor is in the DOM but HIDDEN (`display:none` under a collapsed container):
//     `scrollIntoView` is a no-op on it and the pulse paints nothing, so a bare `true` here used
//     to be one toast and no field. The event is dispatched ON the anchor and BUBBLES: whichever
//     ancestor folds it listens and opens;
//   · the anchor is NOT IN THE DOM AT ALL, because the step or tab that draws the field is not
//     mounted (the design studio shows ONE step at a time — a refusal over `fit` while the flat is
//     open has nothing to point at). Then there is no anchor to dispatch on, and the event goes to
//     the `document` instead. A listener that knows where the field lives (the studio's step
//     map, `core/chain.ts` `stepOfField`) CLAIMS the request with `preventDefault()` and brings the
//     step on; an unclaimed request is the old honest `false` — «no rendered field».
// In both cases the scroll+pulse then waits a bounded number of FRAMES for the anchor to appear
// shown. The event is the seam on purpose: this module knows nothing about how a step or a fold is
// drawn, and they know nothing about error paths — a new container inherits the door by adding one
// listener, not by teaching this reader its markup.
export const FIELD_REVEAL_EVENT = 'grbpwr:reveal-field';

export type FieldRevealDetail = { path: string };
...
export function revealField(path: string): boolean {
  ...
  // `dispatchEvent` is false exactly when a listener called `preventDefault()` — the claim.
  const claimed = !(el ?? document).dispatchEvent(ask);
  // A hidden anchor is answered by its ancestors whether or not they claim (the fold opens without
  // saying so); a missing anchor is a `false` unless somebody said «I will bring it».
  if (!el && !claimed) return false;
  // Frames, not milliseconds: what has to happen is a React commit — of a fold opening, or of a
  // whole step mounting — and on a slow machine a timer runs out before it has landed.
  let left = 90;
  const tick = (): void => {
    const now = find();
    if (now && isShown(now)) {
